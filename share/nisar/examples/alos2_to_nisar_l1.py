...
from alos_to_nisar_l0b import get_alos_orbit, getset_attitude, set_h5_orbit
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Package ALOS-2 L1 stripmap data into NISAR L1 HDF5"
    )
    parser.add_argument(
        "-i",
        "--indir",
    ...
    parser.add_argument(
        "-l",
        "--pols",
        dest="polarization_list",
        type=str,
        help="List of polarizations to process",
        nargs="*",
    )
    parser.add_argument(
        "--first-line", dest="first_line", type=int, help="First azimuth line to unpack"
    )
    parser.add_argument(
        "--last-line", dest="last_line", type=int, help="Last azimuth line to unpack"
    )
    parser.add_argument(
        "-o",
        "--outh5",
...
def process(args=None):
    ...
    if args.template_file:
        ...
        template_file = os.path.join(
            script_dir, "..", "templates", "L1_SingleLookComplex.h5"
        )
...
def parse_leader_file(filenames, args):
    ...
    if not args.polarization_list and numpol != ldr.summary.NumberOfSARChannels:
        print(
            f"WARNING Number of image files ({numpol}) discovered"
...
def construct_nisar_hdf5(outh5, ldr):
    ...
    ident_group.create_dataset(
        "orbitPassDirection",
        data=np.string_(ldr.summary.TimeDirectionIndicatorAlongLine),
    )
    ...
    del root_group["//science/LSAR/SLC/metadata/calibrationInformation/" "frequencyB"]

    return orbit


def add_imagery(args, ldr, imgfile, pol, orbit, metadata, flag_first_image):
    """
    ...
    bytesperpixel = (
        image.description.NumberOfBytesPerDataGroup
    ...
    if flag_first_image:
        ...
        freq_group.create_dataset(
            "chirpDuration", data=firstrec.ChirpLengthInns * 1.0e-9
        )
        freq_group.create_dataset(
            "chirpSlope",
            data=-(
                (freq_group["rangeBandwidth"][()]) / (freq_group["chirpDuration"][()])
            ),
        )
        ...
        assert (
            ldr.summary.SensorIDAndMode[7] == "R"
            or ldr.summary.SensorIDAndMode[7] == "L"
        )
        ...
        if not FLAG_REFORMAT_DOPPLER_ISCE2:
            ...
            doppler_coeffs = [
                ldr.summary.DopplerCenterFrequencyConstantTerm,
        ...
        freq_group.create_dataset(
            "validSamplesSubSwath1", dtype="i8", shape=(length, 2)
        )
    ...
    polimg = root_group.create_dataset(
        os.path.join(freq_str, pol), dtype=cpxtype, shape=(length, width), **compress
    )
    ...
    if flag_first_image:
        ...
        metadata["Scene Center Incidence Angle"] = ldr.summary.SceneCenterIncidenceAngle

        ref_epoch = DateTime(ref_epoch)
        timedelta_start = (DateTime(sensing_start) - ref_epoch).total_seconds()
        radar_grid = RadarGridParameters(
            timedelta_start, wavelength, prf, r0, dr, lookside, length, width, ref_epoch
        )
        metadata["Radar Grid"] = radar_grid


def populate_hdf5(metadata, outfile, orbit, pol_list, frequency="A", az_pad=10.0):
    """
    ...
    construct_calibration_grid(metadata, metadata["Mission"], orbit, az_pad=az_pad)
...
def update_metadata(fid, metadata, pol_list, frequency="A"):
    ...
    R = metadata["Image Starting Range"] + metadata[
        "Range Spacing per Bin"
    ...
    group["sceneCenterGroundRangeSpacing"] = metadata["Range Spacing per Bin"] / np.sin(
        inc
    )
...
def update_identification(fid, orbit, metadata, min_height=-500.0, max_height=9000.0):
    """
    ...
    box_min = get_geo_perimeter_wkt(radar_grid, orbit, dem=dem_min)
    box_max = get_geo_perimeter_wkt(radar_grid, orbit, dem=dem_max)
...
def update_doppler(fid, metadata, frequency):  # time, position, velocity,
    ...
    if frequency == "A":
        ...
        pgroup["zeroDopplerTime"].attrs["units"] = np.string_(
