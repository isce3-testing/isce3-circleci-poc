...
def run(cfg: dict, input_hdf5: str):
    ...
    if filter_type == "no_filter":
        ...
        kernel_rows = create_gaussian_kernel(
            kernel_length, filter_args["gaussian"]["sigma_azimuth"]
        )
        kernel_rows = np.reshape(kernel_rows, (kernel_length, 1))
        kernel_cols = create_gaussian_kernel(
            kernel_width, filter_args["gaussian"]["sigma_range"]
        )
    ...
    t_all_elapsed = time.time() - t_all
    info_channel.log(
        f"successfully ran filter_interferogram in {t_all_elapsed:.3f} seconds"
    )
