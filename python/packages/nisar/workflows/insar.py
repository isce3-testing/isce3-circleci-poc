...
import journal
from nisar.workflows import (bandpass_insar, crossmul, dense_offsets,
                             filter_interferogram, geo2rdr, geocode_insar,
                             h5_prep, rdr2geo, resample_slc, rubbersheet,
                             unwrap)
...
def run(cfg: dict, out_paths: dict, run_steps: dict):
    ...
    if (run_steps["dense_offsets"]) and (cfg["processing"]["dense_offsets"]["enabled"]):
        dense_offsets.run(cfg)

    if run_steps["rubbersheet"] and cfg["processing"]["rubbersheet"]["enabled"]:
        rubbersheet.run(cfg, out_paths["RIFG"])

    # If enabled, run fine_resampling
    if run_steps["fine_resample"] and cfg["processing"]["fine_resample"]["enabled"]:
    ...
    if (
        run_steps["filter_interferogram"]
        and cfg["processing"]["filter_interferogram"]["filter_type"] != "no_filter"
    ):
