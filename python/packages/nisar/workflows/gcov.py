...
def run(cfg):
    ...
    flag_symmetrize_cross_pol_channels = cfg["processing"]["input_subset"][
    ...
    for frequency in freq_pols.keys():
        ...
        if (
            ...
            isce3.polsar.symmetrize_cross_pol_channels(
                hv_raster_obj, vh_raster_obj, symmetrized_hv_obj
            )
        ...
        # create output raster
        temp_output = tempfile.NamedTemporaryFile(dir=scratch_path, suffix=".tif")
        ...
        if flag_save_nlooks:
            temp_nlooks = tempfile.NamedTemporaryFile(dir=scratch_path, suffix=".tif")
        ...
        if flag_save_rtc:
            temp_rtc = tempfile.NamedTemporaryFile(dir=scratch_path, suffix=".tif")
        ...
        with h5py.File(output_hdf5, "a") as hdf5_obj:
            ...
            if flag_save_dem:
                ...
                if output_mode == isce3.geocode.GeocodeOutputMode.AREA_PROJECTION:
                    ...
                    yds_dem, xds_dem = set_get_geo_info(hdf5_obj, root_ds, dem_geogrid)
                else:
                    yds_dem = yds
                    xds_dem = xds
...
def _save_hdf5_dataset(
    ...
    if compute_stats:
        ...
        if (
            raster.datatype() == gdal.GDT_CFloat32
            or raster.datatype() == gdal.GDT_CFloat64
        ):
            stats_real_imag_vector = isce3.math.compute_raster_stats_real_imag(raster)
    ...
    for band in range(nbands):
        ...
        if stats_vector is not None:
            ...
            dset.attrs.create(
                "sample_standard_deviation_real", data=stats_obj.sample_stddev_real
            )
            ...
            dset.attrs.create(
                "sample_standard_deviation_imag", data=stats_obj.sample_stddev_imag
            )
