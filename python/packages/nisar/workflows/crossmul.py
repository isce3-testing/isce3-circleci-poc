    ...
    with h5py.File(output_hdf5, "a", libver="latest", swmr=True) as dst_h5:
        for freq, pol_list in freq_pols.items():
            ...
            if flatten is not None:
                ...
                crossmul.set_rg_filter(
                    rg_sample_freq, rg_bandwidth, rg_pxl_spacing, wavelength
                )
            ...
            for pol in pol_list:
                ...
                if coregistered_is_file:
                    ...
                    raster_str = str(
                        coregistered_slc_path / f"{resample_type}_resample_slc/"
                ...
                igram_raster = isce3.io.Raster(
                    f"IH5:::ID={igram_dataset.id.id}".encode("utf-8"), update=True
                )
