...
class ResampleSlcRunConfig(RunConfig):
    ...
    def yaml_check(self, resample_type):
        ...
        if self.cfg["processing"][resample_key]["offsets_dir"] is None:
            self.cfg["processing"][resample_key]["offsets_dir"] = self.cfg[
                "ProductPathGroup"
            ]["ScratchPath"]
        ...
        if resample_type == "coarse":
            helpers.check_mode_directory_tree(offsets_dir, "geo2rdr", frequencies)
            for freq in frequencies:
                rg_off = os.path.join(
                    offsets_dir, "geo2rdr", f"freq{freq}", "range.off"
                )
            ...
            for freq in frequencies:
                for pol in set.intersection(set(["HH", "VV"]), set(freq_pols[freq])):
                    ...
                    if not os.path.exists(rg_off) or not os.path.exists(az_off):
                        err_str = (
                            f"{rg_off} and {az_off} files do not exists. HH and"
