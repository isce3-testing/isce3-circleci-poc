...
def run(cfg: dict, input_hdf5: str, output_hdf5: str):
    ...
    log_unwrap_attributes(unwrap_obj, info_channel, unwrap_args["algorithm"])

    t_all = time.time()

    with h5py.File(output_hdf5, "a", libver="latest", swmr=True) as dst_h5, h5py.File(
        crossmul_path, "r", libver="latest", swmr=True
    ) as src_h5:
        for freq, pol_list in freq_pols.items():
            ...
            for pol in pol_list:
                ...
                igram_path = (
                    f"HDF5:{crossmul_path}:/"
                    f"{src_pol_group_path}/wrappedInterferogram"
                )

                # Prepare correlation input raster
                corr_path = (
                    f"HDF5:{crossmul_path}:/" f"{src_pol_group_path}/coherenceMagnitude"
                )
                ...
                uigram_raster = isce3.io.Raster(
                    f"IH5:::ID={uigram_dataset.id.id}".encode("utf-8"), update=True
                )
                ...
                conn_comp_raster = isce3.io.Raster(
                    f"IH5:::ID={conn_comp_dataset.id.id}".encode("utf-8"), update=True
                )
                ...
                if unwrap_args["algorithm"] == "icu":
                    ...
                    if "seed" in unwrap_args["icu"]:
                        unwrap_obj.unwrap(
                            ...
                            igram_raster,
                            corr_raster,
                            seed=unwrap_args["seed"],
                        )
                    else:
                        unwrap_obj.unwrap(
                            uigram_raster, conn_comp_raster, igram_raster, corr_raster
                        )
                else:
                    ...
                    ds = None
                    with open(
                        os.path.join(unwrap_scratch, "wrapped_phase.vrt"), "w"
                    ) as fid:
                        fid.write(vrttmpl)

                    # Open phase_raster as ISCE3 raster
                    phase_raster = isce3.io.Raster(
                        os.path.join(unwrap_scratch, "wrapped_phase.vrt")
                    )
                    ...
                    if "power" in unwrap_args["phass"]:
                        ...
                        unwrap_obj.unwrap(
                            phase_raster, corr_raster, uigram_raster, conn_comp_raster
                        )
                ...
                for dataset_name, group_name in zip(dataset_names, group_names):
                    dst_path = (
                        f"{dst_freq_group_path}/{group_name}/{pol}/{dataset_name}"
                    )
                    src_path = (
                        f"{src_freq_group_path}/{group_name}/{pol}/{dataset_name}"
                    )
...
def log_unwrap_attributes(unwrap, info, algorithm):
    ...
    if algorithm == "icu":
        ...
        info.log(
            f"Maximum intensity correlation "
        ...
        info.log(f"Minimum size of an unwrapped region: {unwrap.min_pixels_region}")
...
def set_unwrap_attributes(unwrap, cfg: dict):
    ...
    if algorithm == "icu":
        ...
        if "neutron_phase_gradient_threshold" in icu_cfg:
            unwrap.neut_phase_grad_thr = icu_cfg["neutron_phase_gradient_threshold"]
        ...
        if "max_intensity_correlation_threshold" in icu_cfg:
            unwrap.neut_correlation_thr = icu_cfg["max_intensity_correlation_threshold"]
