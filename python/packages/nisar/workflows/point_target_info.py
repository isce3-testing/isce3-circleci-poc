...
def find_null_to_null(matched_output, num_nulls_main, fs_bw_ratio, main_peak_idx):
    ...
    if np.any(diffsign_left == 1):
        null_left_idx = (
            first_peak_left_idx
            - np.where(diffsign_left[:-1] + diffsign_left[1:] == 0)[0][0]
            - 1
        )
    else:
    ...
    if np.any(diffsign_right == 1):
        null_right_idx = (
            first_peak_right_idx
            + np.where(diffsign_right[:-1] + diffsign_right[1:] == 0)[0][0]
            + 1
        )
    else:
...
def islr_pslr(
    data_in_linear, fs_bw_ratio=1.2, num_nulls_main=2, num_lobes=12, search_null=False
):
    ...
    if search_null:
        null_main_left_idx, null_main_right_idx = find_null_to_null(
            data_in_pwr_dB, num_nulls_main, fs_bw_ratio, zmax_idx
        )
        null_first_left_idx, null_first_right_idx = find_null_to_null(
            data_in_pwr_dB, plsr_main_lobe, fs_bw_ratio, zmax_idx
        )
...
def analyze_point_target(
    ...
    dr_islr_dB, dr_pslr_dB = islr_pslr(
        rg_slice,
        fs_bw_ratio=fs_bw_ratio_ov,
        num_nulls_main=num_nulls_main,
        num_lobes=num_lobes,
        search_null=search_null,
    )
    da_islr_dB, da_pslr_dB = islr_pslr(
        az_slice,
        fs_bw_ratio=fs_bw_ratio_ov,
        num_nulls_main=num_nulls_main,
        num_lobes=num_lobes,
...
def main(argv):
    ...
    parser.add_argument(
        "--cuts", action="store_true", help="Add range/azimuth slices to output JSON."
    )
    ...
    parser.add_argument(
        ...
        default=2,
        required=False,
        help="number of nulls in mainlobe, default=2",
    )
    parser.add_argument(
        "--num-lobes",
        type=float,
        default=12,
        required=False,
        help="total number of lobes, including mainlobe, default=12",
    )
    parser.add_argument(
        "-s",
        "--search-null",
        action="store_true",
        default="False",
        help="Search for mainlobe null or use default mainlobe sample spacing",
    )
    ...
    info = analyze_point_target(
        ...
        num_nulls_main=args.mlobe_nulls,
        num_lobes=args.num_lobes,
        search_null=args.search_null,
    )
    if args.i:
        info = info[0]
