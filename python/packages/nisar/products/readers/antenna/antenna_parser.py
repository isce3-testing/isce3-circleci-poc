...
class AntennaParser:
    ...
    def __init__(self, filename):
        ...
        self._fid = h5py.File(filename, mode="r", libver="latest", swmr=True)
    ...
    def num_beams(self, pol="H"):
        ...
        return len(
            [
                rx
                for rx in self.rx_beams
                if "DBF" not in rx.upper() and pol in rx.upper()
            ]
        )
    ...
    def el_cut_all(self, pol="H"):
        ...
        num_ang = (
            int(
                np.ceil(
                    (beam_last.angle[-1] - beam_first.angle[0])
                    / (beam_first.angle[1] - beam_first.angle[0])
                )
            )
            + 1
        )
        ...
        out = {}
        out["angle"] = np.linspace(beam_first.angle[0], beam_last.angle[-1], num_ang)
        out["copol_pattern"] = np.zeros(
            (num_beam, num_ang), beam_first.copol_pattern.dtype
        )
        ...
        for nn in range(num_beam):
            ...
            out["copol_pattern"][nn, :] = np.interp(
                ...
                beam.angle,
                beam.copol_pattern,
                left=out_of_range_val,
                right=out_of_range_val,
            )
    ...
    def _from_cut_attrs(self, attr_name: str):
        ...
        cut_attr_obj = self._fid[self.rx_beams[0] + "/" + first_cut_name].attrs
        try:
    ...
    def _get_ang_cut(
        ...
        pol: str,
        cut_name: str,
        out_keys: tuple = ("angle", "copol_pattern", "cxpol_pattern"),
        ang_attr: str = "cut_angle",
    ) -> cl.namedtuple:
