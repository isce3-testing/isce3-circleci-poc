...
def open_product(filename: str, product_type: str = None, root_path: str = None):
    """
    ...
    product_type = get_hdf5_file_product_type(
        filename, product_type=product_type, root_path=root_path
    )
...
def get_hdf5_file_product_type(filename, product_type=None, root_path=None):
    """
    ...
    if root_path is None:
        root_path = get_hdf5_file_root_path(filename, root_path=root_path)
    NISAR_PRODUCT_LIST = ["RRSD", "RSLC", "SLC", "RIFG", "RUNW", "GCOV", "GSLC", "GUNW"]
    ...
    error_msg = "HDF5 could not find NISAR product group" f" in file: {filename}"
...
class GenericProduct(Base, family="nisar.productreader.product"):
    ...
    def __init__(self, **kwds):
        ...
        self.identification.productType = get_hdf5_file_product_type(
            self.filename, product_type=self.productType, root_path=self.RootPath
        )

        if (
            self.productValidationType != "BASE"
            and self.productValidationType != self.productType
        ):
