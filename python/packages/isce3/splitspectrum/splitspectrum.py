...
class SplitSpectrum:
    ...
    def __init__(
        self, rg_sample_freq, rg_bandwidth, center_frequency, slant_range, freq
    ):
    ...
    def bandpass_shift_spectrum(
        ...
        slc_demodulate = self.demodulate_slc(
            slc_bp[:, :width], diff_frequency, rg_sample_freq
        )
        ...
        if resampling:
            ...
            meta["slant_range"] = np.linspace(
                self.slant_range(0), self.slant_range(width), sub_width, endpoint=False
            )
            ...
            meta["slant_range"] = np.linspace(
                self.slant_range(0), self.slant_range(width), width, endpoint=False
            )

            return slc_demodulate, meta

    def bandpass_spectrum(
        self,
        slc_raster,
        low_frequency,
        high_frequency,
        ...
        spectrum_target = fft(slc_raster, n=fft_size) / window_target
        # apply new bandpass window to spectrum
        slc_bandpassed = ifft(
            spectrum_target * window_bandpass * np.sqrt(resampling_scale_factor),
            n=fft_size,
        )
    ...
    def demodulate_slc(self, slc_array, diff_frequency, rg_sample_freq):
        ...
        slc_shifted = slc_array * np.exp(1j * 2.0 * np.pi * diff_frequency * range_time)
        return slc_shifted
    ...
    def get_range_bandpass_window(
        ...
        frequency = self.freq_spectrum(
            cfrequency=center_frequency, dt=1.0 / sampling_frequency, fft_size=fft_size
        )
    ...
    def construct_range_bandpass_cosine(
        self, frequency_range, freq_low, freq_high, window_shape
    ):
        ...
        filter_1d = self._construct_range_bandpass_kaiser_cosine(
            frequency_range, freq_low, freq_high, cosine_window, window_shape
        )
        return filter_1d

    def construct_range_bandpass_kaiser(
        self, frequency_range, freq_low, freq_high, window_shape
    ):
        ...
        """
        filter_1d = self._construct_range_bandpass_kaiser_cosine(
            frequency_range, freq_low, freq_high, np.kaiser, window_shape
        )
        return filter_1d

    def _construct_range_bandpass_kaiser_cosine(
        self, frequency_range, freq_low, freq_high, window_function, window_shape
    ):
    ...
    def construct_range_bandpass_tukey(
        self, frequency_range, freq_low, freq_high, window_shape
    ):
        ...
        for i in range(0, fft_size):
            ...
            if (freq <= (freq_high - df)) and (freq >= (freq_low + df)):
                filter_1d[i] = 1
            # Transition region
            elif ((freq < freq_low + df) and (freq >= freq_low - df)) or (
                (freq <= freq_high + df) and (freq > freq_high - df)
            ):
                filter_1d[i] = 0.5 * (
                    1.0
                    + np.cos(
                        np.pi
                        / (subbandwidth * window_shape)
