...
class ImageFile(object):
    ...
    def parseFileDescriptor(self):
        ...
        """
        from isce3.parsers.CEOS.ImageFileDescriptorType import \
            ImageFileDescriptorType
        ...
        assert (
            record.NumberOfBytesPerDataGroup % record.NumberOfSamplesPerDataGroup == 0
        )
        assert (
            record.NumberOfBytesOfSARDataPerRecord
            % (record.NumberOfBytesPerDataGroup // record.NumberOfSamplesPerDataGroup)
            == 0
        )
    ...
    def readNextLine(self):
        ...
        from isce3.stripmap.readers.l1.ALOS2.CEOS.SignalDataRecordType import \
            SignalDataRecordType

        # Create record type with information from description
        bytesperpixel = (
            self.description.NumberOfBytesPerDataGroup
            // self.description.NumberOfSamplesPerDataGroup
        )
