...
void addbinding(py::class_<Geo2rdr>& pyGeo2Rdr)
{
    pyGeo2Rdr
            .def(py::init([](const isce3::product::RadarGridParameters&
                ...
                auto geo2rdr_obj =
                        Geo2rdr(radar_grid, orbit, ellipsoid, doppler);
                ...
                return geo2rdr_obj;
                    ...
                    py::arg("threshold") = 1e-8, py::arg("numiter") = 25,
                    py::arg("lines_per_block") = 1000)
            .def("geo2rdr",
                    py::overload_cast<isce3::io::Raster&, const std::string&,
                            double, double>(&Geo2rdr::geo2rdr),
                    py::arg("dem_raster"), py::arg("outdir"),
                    py::arg("az_shift") = 0.0, py::arg("rg_shift") = 0.0)
...
void addbinding_geo2rdr(pybind11::module& m)
{
    ...
    m.def(
            "geo2rdr",
            [](const Vec3& lon_lat_height, const Ellipsoid& ellipsoid,
                    const Orbit& orbit, const LUT2d<double>& doppler,
                    double wavelength, py::object py_side, double threshold,
                    int maxiter, double delta_range) {
                auto side = duck_look_side(py_side);
                double aztime, slant_range;
                int converged = geo2rdr(lon_lat_height, ellipsoid, orbit,
                        doppler, aztime, slant_range, wavelength, side,
                        threshold, maxiter, delta_range);
                if (!converged)
                    throw std::runtime_error("geo2rdr failed to converge");
                return std::make_pair(aztime, slant_range);
            },
            py::arg("lon_lat_height"), py::arg("elliposid") = Ellipsoid(),
            py::arg("orbit"), py::arg("doppler"), py::arg("wavelength"),
            py::arg("side"), py::arg("threshold") = defaults.threshold,
