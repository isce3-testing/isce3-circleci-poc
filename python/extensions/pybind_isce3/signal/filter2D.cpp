...
void addbinding_filter2D(py::module& m)
{
    m.def(
            ...
            [](Raster& output, Raster& input,
                ...
                if (kernel_rows.size() > input.length())
                    throw isce3::except::RuntimeError(
                            ISCE_SRCINFO(), "kernel rows width > input length");
                if (output.length() != input.length())
                    if (output.length() != input.length() / kernel_rows.size())
                        throw isce3::except::RuntimeError(ISCE_SRCINFO(),
                                "output length not equal to input length or not"
                                "equal to input length divided by kernel row "
                                "size");
                if (output.width() != input.width())
                    if (output.width() !=
                            output.width() / kernel_columns.size())
                        throw isce3::except::RuntimeError(ISCE_SRCINFO(),
                                "output width not equal to input width or not"
                                "equal to input width divided by kernel "
                                "columns size");
                // ensure types match
                switch (in_type) {
                case GDT_Float32:
                    filter2D<float>(output, input, kernel_columns, kernel_rows,
                            block_rows);
                    return;
                case GDT_Float64:
                    filter2D<double>(output, input, kernel_columns, kernel_rows,
                            block_rows);
                    return;
                case GDT_CFloat32:
                    filter2D<std::complex<float>>(output, input, kernel_columns,
                            kernel_rows, block_rows);
                    return;
                case GDT_CFloat64:
                    filter2D<std::complex<double>>(output, input,
            ...
            py::arg("output"), py::arg("input"), py::arg("kernel_columns"),
            py::arg("kernel_rows"), py::arg("block_rows"),
            "Filter real or complex data by convolving two separable 1D "
            "kernels");
}
