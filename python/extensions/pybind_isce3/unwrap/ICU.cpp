...
void addbinding(py::class_<ICU>& pyICU)
{
    ...
    pyICU
            // Constructors
            .def(py::init([](const size_t buffer_lines,
                                  const size_t overlap_lines,
                                  const bool use_phase_grad_neut,
                                  const bool use_intensity_neut,
                                  const int phase_grad_win_size,
                                  const float neut_phase_grad_thr,
                                  const float neut_intensity_thr,
                                  const float neut_correlation_thr,
                                  const int trees_number,
                                  const int max_branch_len,
                                  const float ratio_dxdy,
                                  const float init_corr_thr,
                                  const float max_corr_thr,
                                  const float corr_incr_thr,
                                  const float min_cc_area,
                                  const size_t num_bs_lines,
                                  const size_t min_overlap_area,
                                  const float phase_var_thr) {
                ICU icu;
                ...
                return icu;
                    ...
                    py::arg("ratio_dxdy") = 1.0, py::arg("init_corr_thr") = 0.1,
            ...
            .def("unwrap",
                    py::overload_cast<Raster&, Raster&, Raster&, Raster&,
                            unsigned int>(&ICU::unwrap),
                    py::arg("unw_igram"), py::arg("connected_components"),
                    py::arg("igram"), py::arg("corr"), py::arg("seed") = 0,
                    R"(
