...
{
    pyMode.value("Full", RangeComp::Mode::Full, R"(
...
void addbinding(py::class_<RangeComp>& pyRangeComp)
{
    ...
    pyRangeComp
            .def(py::init<const chirp_t&, int, int, RangeComp::Mode>(),
                    ...
                    py::arg("maxbatch") = 1,
                    py::arg("mode") = RangeComp::Mode::Full,
                    R"(
    ...
    mode      Convolution output mode
            ...
            .def(
                    ...
                    [](RangeComp& self, buf_t& out, const buf_t& in) {
                        ...
                        // Require 2D with matching slow dim if batched
                        // operation.
                        if (in.ndim() == 2) {
                            batch = in.shape(0);
                            if (in.shape(0) != out.shape(0))
                                throw std::length_error(
                                        "require equal batch size on input and "
                                        "output");
                            ...
                            throw std::invalid_argument(
                                    "require 1D or 2D data");
                        }
                        self.rangecompress(
                                out.mutable_data(), in.data(), batch);
                    },
    ...
    function.  Batch size inferred from first dimension of 2D data (1 for 1D).
            ...
            .def_property_readonly(
                    "first_valid_sample", &RangeComp::firstValidSample);
}
