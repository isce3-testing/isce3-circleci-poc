...
template<typename T>
static py::buffer_info toBuffer(Raster& raster)
{
    Buffer mmap = raster.memmap();

    std::string format = py::format_descriptor<T>::format();
    std::vector<ssize_t> shape = {
            ssize_t(mmap.length()), ssize_t(mmap.width())};
    std::vector<ssize_t> strides = {
            ssize_t(mmap.rowstride()), ssize_t(mmap.colstride())};
...
template<typename T>
static Raster toRaster(py::buffer buf)
{
    py::buffer_info info = buf.request();

    if (info.ndim != 2) {
        throw isce3::except::RuntimeError(
                ISCE_SRCINFO(), "buffer object must be 2-D");
    }

    constexpr static std::size_t max_int = std::numeric_limits<int>::max();
    for (int dim = 0; dim < 2; ++dim) {
        if (info.shape[dim] > max_int) {
            std::string errmsg = "buffer shape along axis " +
                                 std::to_string(dim) + " exceeds max size";
    ...
    return Raster(data, info.shape[1], info.shape[0], info.strides[1],
            info.strides[0], access);
}
...
Raster toRaster(py::buffer buf)
{
    ...
    if (info.format == py::format_descriptor<unsigned char>::format()) {
        return toRaster<unsigned char>(buf);
    }
    if (info.format == py::format_descriptor<std::uint16_t>::format()) {
        return toRaster<std::uint16_t>(buf);
    }
    if (info.format == py::format_descriptor<std::int16_t>::format()) {
        return toRaster<std::int16_t>(buf);
    }
    if (info.format == py::format_descriptor<std::uint32_t>::format()) {
        return toRaster<std::uint32_t>(buf);
    }
    if (info.format == py::format_descriptor<std::int32_t>::format()) {
        return toRaster<std::int32_t>(buf);
    }
    if (info.format == py::format_descriptor<float>::format()) {
        return toRaster<float>(buf);
    }
    if (info.format == py::format_descriptor<double>::format()) {
        return toRaster<double>(buf);
    }
    if (info.format == py::format_descriptor<std::complex<float>>::format()) {
        return toRaster<std::complex<float>>(buf);
    }
    if (info.format == py::format_descriptor<std::complex<double>>::format()) {
        return toRaster<std::complex<double>>(buf);
    }

    throw isce3::except::RuntimeError(ISCE_SRCINFO(),
            "unable to cast buffer format descriptor to GDALDataType");
}

void addbinding(py::class_<Raster>& pyRaster)
{
    pyRaster.def_buffer([](Raster& self) -> py::buffer_info {
                return toBuffer(self);
            })
            .def("default_driver", &Raster::defaultDriver,
                    "Default GDAL driver for raster creation")
            .def(py::init<const std::string&, GDALAccess>(),
                    "Open an existing file containing a single raster band as "
                    "a GDAL raster",
                    py::arg("path"), py::arg("access") = GA_ReadOnly)
            .def(py::init([](const std::string& path, char access) {
                return std::make_unique<Raster>(path, toGDALAccess(access));
            }),
                    "Open an existing file containing a single raster band as "
                    "a GDAL raster",
                    py::arg("path"), py::arg("access"))
            .def(py::init<const std::string&, int, GDALAccess>(),
                    "Open an existing file as a GDAL dataset and fetch the "
                    "specified raster band",
                    py::arg("path"), py::arg("band"),
                    py::arg("access") = GA_ReadOnly)
            .def(py::init([](const std::string& path, int band, char access) {
                return std::make_unique<Raster>(
                        path, band, toGDALAccess(access));
            }),
                    "Open an existing file as a GDAL dataset and fetch the "
                    "specified raster band",
                    py::arg("path"), py::arg("band"), py::arg("access"))
            .def(py::init<const std::string&, int, int, GDALDataType,
                         const std::string&>(),
                    "Create a new GDAL dataset containing a single raster band",
                    py::arg("path"), py::arg("width"), py::arg("length"),
                    py::arg("datatype"),
            ...
            .def(py::init([](const std::string& path, int width, int length,
                ...
                return std::make_unique<Raster>(
                        path, width, length, toGDALDataType(datatype), driver);
            }),
                    py::arg("path"), py::arg("width"), py::arg("length"),
                    py::arg("datatype"),
                    py::arg("driver") = Raster::defaultDriver())
            .def(py::init([](py::buffer buf) {
                return std::make_unique<Raster>(toRaster(buf));
            }))
            .def("dataset", py::overload_cast<>(&Raster::dataset, py::const_),
                    "Get the dataset containing the raster")
            .def_property_readonly(
                    "band", &Raster::band, "Band index (1-based)")
            .def_property_readonly(
                    "datatype", &Raster::datatype, "Datatype identifier")
            ...
            .def_property_readonly(
                    "dx", &Raster::dx, "Pixel width in projection coordinates")
            .def_property_readonly(
                    "dy", &Raster::dy, "Line height in projection coordinates")
            .def_property_readonly("data", [](Raster& self) {
                return py::roarray(toBuffer(self), py::cast(self));
            });
}
