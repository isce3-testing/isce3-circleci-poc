...
static std::string getInfo(Dataset& dataset)
{
    ...
    if (!tmp) {
        throw isce3::except::GDALError(
                ISCE_SRCINFO(), "failed to display dataset info");
    }
    std::string info(tmp);
    CPLFree(tmp);
...
{
    pyDataset
            .def("default_driver", &Dataset::defaultDriver,
                    "Default GDAL driver for dataset creation")
            .def(py::init<const std::string&, GDALAccess>(),
                    "Open an existing file as a GDAL dataset.", py::arg("path"),
            ...
            .def(py::init([](const std::string& path, char access) {
                return std::make_unique<Dataset>(path, toGDALAccess(access));
                    ...
                    "Open an existing file as a GDAL dataset.", py::arg("path"),
                    py::arg("access"))
            .def(py::init<const std::string&, int, int, int, GDALDataType,
                         const std::string&>(),
                    "Create a new GDAL dataset.", py::arg("path"),
                    py::arg("width"), py::arg("length"), py::arg("bands"),
                    py::arg("datatype"),
                    py::arg("driver") = Dataset::defaultDriver())
            .def(py::init([](const std::string& path, int width, int length,
                                  int bands, py::object datatype,
                                  const std::string& driver) {
                return std::make_unique<Dataset>(path, width, length, bands,
                        toGDALDataType(datatype), driver);
            }),
                    py::arg("path"), py::arg("width"), py::arg("length"),
                    py::arg("bands"), py::arg("datatype"),
                    py::arg("driver") = Dataset::defaultDriver())
            ...
            .def_property_readonly("x0", &Dataset::x0,
                    "Left edge of left-most pixel in projection coordinates")
            .def_property_readonly("y0", &Dataset::y0,
                    "Upper edge of upper-most line in projection coordinates")
            .def_property_readonly(
                    "dx", &Dataset::dx, "Pixel width in projection coordinates")
            .def_property_readonly(
                    "dy", &Dataset::dy, "Line height in projection coordinates")
