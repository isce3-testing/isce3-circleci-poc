...
static py::buffer_info toBuffer(const std::vector<isce3::core::Vec3>& buf)
{
    ...
    const std::vector<ssize_t> strides {
            sizeof(isce3::core::Vec3), sizeof(double)};
    const bool readonly = true;

    return {(void*) buf.data(), sizeof(double), format, 2, shape, strides,
            readonly};
}

void addbinding(py::class_<Orbit>& pyOrbit)
{
    pyOrbit.def(py::init<std::vector<StateVector>>())
            ...
            .def_property_readonly(
                    "time", py::overload_cast<>(&Orbit::time, py::const_))
            .def_property_readonly("position",
                    [](const Orbit& self) {
                        return py::roarray(
                                toBuffer(self.position()), py::cast(self));
                    })
            .def_property_readonly("velocity",
                    [](const Orbit& self) {
                        return py::roarray(
                                toBuffer(self.velocity()), py::cast(self));
                    })

            .def_static(
                    "load_from_h5",
                    [](py::object h5py_group) {
                        auto id =
            ...
            .def(
                    "save_to_h5",
                    [](const Orbit& self, py::object h5py_group) {
                        auto id =
                                h5py_group.attr("id").attr("id").cast<hid_t>();
                        isce3::io::IGroup group(id);
            ...
            .def(
                    "interpolate",
                    [](const Orbit& self, double t) {
                        isce3::core::Vec3 p, v;
                        self.interpolate(&p, &v, t);
                        return std::make_pair(p, v);
                    },
                    "Interpolate platform position and velocity", py::arg("t"));
}
