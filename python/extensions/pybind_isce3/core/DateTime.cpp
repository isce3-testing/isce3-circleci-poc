...
static DateTime fromPyDateTime(py::handle obj)
{
...
{
    pyDateTime.def(py::init<>())
            .def(py::init<double>(), py::arg("ord"), "Construct from ordinal")
            .def(py::init<int, int, int>(), py::arg("year"), py::arg("month"),
                    py::arg("day"))
            .def(py::init<int, int, int, int, int, int>(), py::arg("year"),
                    py::arg("month"), py::arg("day"), py::arg("hour"),
                    py::arg("minute"), py::arg("second"))
            .def(py::init<int, int, int, int, int, double>(), py::arg("year"),
                    py::arg("month"), py::arg("day"), py::arg("hour"),
                    py::arg("minute"), py::arg("second"))
            ...
            .def("is_close",
                    (bool(DateTime::*)(const DateTime&, const TimeDelta&)
                                    const) &
                            DateTime::isClose,
                    py::arg("other"), py::arg("tol") = TimeDelta(1e-10))
            //        .def("day_of_year", &DateTime::dayOfYear)  // XXX not
            //        implemented
            .def("seconds_of_day", &DateTime::secondsOfDay)
            //        .def("day_of_week", &DateTime::dayOfWeek)  // XXX not
            //        implemented .def("toordinal", &DateTime::ordinal)  // XXX
            //        not implemented
            .def("isoformat", &DateTime::isoformat)
            .def("__str__",
                    [](const DateTime& self) { return std::string(self); })
            .def("__repr__",
                    [](const DateTime& self) { return std::string(self); });
}
