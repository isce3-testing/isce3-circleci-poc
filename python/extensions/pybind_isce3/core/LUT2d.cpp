...
void addbinding(py::class_<LUT2d<T>>& pyLUT2d)
{
    ...
    pyLUT2d.def(py::init<>())
            .def(py::init([](double xstart, double ystart, double dx, double dy,
                                  py::array_t<T, py::array::c_style |
                                                         py::array::forcecast>&
                                          py_data,
                                  py::object method, bool b_error) {
                // memcpy because ndarray not auto converted to eigen type for
                // some reason
                if (py_data.ndim() != 2) {
                    throw isce3::except::RuntimeError(
                            ISCE_SRCINFO(), "buffer object must be 2-D");
                }
                isce3::core::Matrix<T> data(
                        py_data.shape()[0], py_data.shape()[1]);
                ...
                // return LUT2d object
                return isce3::core::LUT2d<T>(
                        xstart, ystart, dx, dy, data, interp_method, b_error);
            }),
                    py::arg("xstart"), py::arg("ystart"), py::arg("dx"),
                    py::arg("dy"), py::arg("data"),
                    py::arg("method") = "bilinear", py::arg("b_error") = true)
            .def(py::init([](py::array_t<double,
                                  ...
                                  py::array_t<T, py::array::c_style |
                                          ...
                                          py_data,
                                  py::object method, bool b_error) {
                // memcpy ndarrays because they're not auto converted to eigen
                ...
                // return LUT2d object
                return isce3::core::LUT2d<T>(
                        xcoord, ycoord, data, interp_method, b_error);
            }),
                    py::arg("xcoord"), py::arg("ycoord"), py::arg("data"),
                    py::arg("method") = "bilinear", py::arg("b_error") = true)

            .def_static(
                    "load_from_h5",
                    [](py::object h5py_group, const std::string& dataset_name) {
                        auto id =
                                h5py_group.attr("id").attr("id").cast<hid_t>();
                    ...
                    py::arg("h5py_group"), py::arg("dataset_name"))

            .def(
                    "save_to_h5",
                    [](const LUT2d<T>& self, py::object h5py_group,
                        ...
                        isce3::core::saveCalGrid(
                                group, name, self, epoch, units);
                    },
                    "Serialize LUT2d to h5py.Group object (axes assumed "
                    "range/time).",
                    py::arg("h5py_group"), py::arg("dataset_name"),
                    py::arg("epoch"), py::arg("units") = "")
            ...
            .def_property_readonly("data",
                    [](const LUT2d<T>& self) { return self.data().map(); })
            .def("eval", py::overload_cast<double, double>(
                                 &LUT2d<T>::eval, py::const_))
            .def("eval", py::overload_cast<double,
