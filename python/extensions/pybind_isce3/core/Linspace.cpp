...
void addbinding(py::class_<Linspace<T>>& pyLinspace)
{
    pyLinspace
            ...
            .def(py::init([](T first, T spacing, int size) {
                if (spacing == static_cast<T>(0)) {
                    throw InvalidArgument(
                            ISCE_SRCINFO(), "spacing must be non-zero");
                }
                if (size <= 0) {
                ...
                return Linspace<T>(first, spacing, size);
                    ...
                    py::arg("first"), py::arg("spacing"), py::arg("size"))

            // magic methods
            .def("__getitem__",
                    [](const Linspace<T>& self, int pos) {
                        // wrap index
                        if (pos < 0) {
                            pos += self.size();
                        }
                        ...
                        if (pos < 0 or pos >= self.size()) {
                            throw OutOfRange(
                                    ISCE_SRCINFO(), "index out of range");
                        }
                        return self[pos];
                    })
            .def("__getitem__",
                    [](const Linspace<T>& self, py::slice slice) {
                        ssize_t start, stop, step, len;
                        auto res = slice.compute(
                                self.size(), &start, &stop, &step, &len);
                        if (!res) {
            ...
            .def(
                    "resize",
                    [](Linspace<T>& self, int size) {
                        if (size <= 0) {
                            throw InvalidArgument(
                                    ISCE_SRCINFO(), "size must be > 1");
                        }
                        self.resize(size);
                    },
                    py::arg("size"))
            .def(
                    "search",
                    [](const Linspace<T>& self, T x) { return self.search(x); },
                    "Return the position where the specified value would be "
                    "inserted "
