...
#define DEF_PARAM_RENAME(T, pyname, cppname)                                   \
    def_property(                                                              \
            #pyname, [](const cls& self) -> T { return self.param->cppname; }, \
            [](cls& self, const T i) { self.param->cppname = i; })
...
#define DEF_PARAM(T, name) DEF_PARAM_RENAME(T, name, name)
            ...
            .DEF_PARAM_RENAME(
                    int, halfSearchRangeAcross, halfSearchRangeAcrossRaw)
            .DEF_PARAM_RENAME(int, halfSearchRangeDown, halfSearchRangeDownRaw)

            .DEF_PARAM_RENAME(int, referenceStartPixelAcrossStatic,
                    referenceStartPixelAcross0)
            .DEF_PARAM_RENAME(int, referenceStartPixelDownStatic,
                    referenceStartPixelDown0)

            .DEF_PARAM_RENAME(
                    int, corrSurfaceOverSamplingMethod, oversamplingMethod)
            .DEF_PARAM_RENAME(
                    int, corrSurfaceOverSamplingFactor, oversamplingFactor)
            ...
            .def("checkPixelInImageRange",
                    [](const cls& self) {
                        self.param->checkPixelInImageRange();
                    })

            .def("setupParams",
                    [](cls& self) { self.param->setupParameters(); })

            .def("setConstantGrossOffset",
                    [](cls& self, const int goDown, const int goAcross) {
                        self.param->setStartPixels(
                                self.param->referenceStartPixelDown0,
                                self.param->referenceStartPixelAcross0, goDown,
                                goAcross);
                    })
            .def("setVaryingGrossOffset", [](cls& self, std::vector<int> vD,
                                                  std::vector<int> vA) {
                self.param->setStartPixels(self.param->referenceStartPixelDown0,
                        self.param->referenceStartPixelAcross0, vD.data(),
                        vA.data());
            });
}
