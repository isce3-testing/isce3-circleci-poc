...
{
    m.def(name, &isce3::math::computeRasterStats<T>, py::arg("input_raster"),
...
template void addbinding_stats<std::complex<float>>(
        py::module& m, const char* name);
template void addbinding_stats<std::complex<double>>(
        py::module& m, const char* name);
...
void addbinding(py::class_<StatsRealImag<T>>& pyStatsRealImag)
{
    ...
    pyStatsRealImag.def_readonly(
            "sample_stddev_real", &StatsRealImag<T>::sample_stddev_real);
    ...
    pyStatsRealImag.def_readonly(
            "sample_stddev_imag", &StatsRealImag<T>::sample_stddev_imag);
