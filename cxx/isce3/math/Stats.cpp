...
template<class T>
T signedRealOrComplexModulus(T val)
{
    return val;
}

template<class T>
T signedRealOrComplexModulus(std::complex<T> val)
{
    return std::abs(val);
}
...
inline void _updateStats(BlockStats<T>* block_stats, T& geo_value)
{
    ...
    double delta = (geo_value_real - block_stats->real_valued_mean);
    block_stats->real_valued_mean += (delta / block_stats->n_valid);

    // Square diff sum
    block_stats->square_diff_sum +=
            (delta * (geo_value_real - block_stats->real_valued_mean));

    // Max
    if (isnan(block_stats->max) ||
            geo_value_real > signedRealOrComplexModulus(block_stats->max)) {
        block_stats->max = geo_value;
    }

    // Min
    if (isnan(block_stats->min) ||
            geo_value_real < signedRealOrComplexModulus(block_stats->min)) {
        block_stats->min = geo_value;
    }
}

template<class T>
void _writeStatsToStruct(Stats<T>& stats, T& min, T& max, T& mean,
        double stddev, long long n_valid)
{
    stats.min = min;
    stats.max = max;
    stats.mean = mean;
...
template<class T>
void _saveStats(
        Stats<T>& stats, std::vector<BlockStats<T>>& block_stats_vector_band)
{
    ...
    for (auto block_stats : block_stats_vector_band) {
        ...
        if (isnan(max) || signedRealOrComplexModulus(block_stats.max) >
                                  signedRealOrComplexModulus(max)) {
            max = block_stats.max;
        }

        // Real- or complex-valued min
        if (isnan(min) || signedRealOrComplexModulus(block_stats.min) <
        ...
        const long y0, const long block_length, const int band)
{
    ...
    {
        input_raster.getBlock(block_array.data(), x0, y0, block_width,
                block_length, band + 1);
    }
        ...
        const long y0, const long block_length, const int band)
{
    ...
    // Read band array from input_raster
    isce3::core::Matrix<std::complex<T_real>> block_array(
        ...
        isce3::core::MemoryModeBlockY memory_mode)
{
    ...
    if (memory_mode == isce3::core::MemoryModeBlockY::SingleBlockY) {
        ...
        isce3::geocode::getBlocksNumberAndLength(input_raster.length(),
                input_raster.width(), nbands,
                GDALGetDataTypeSizeBytes(input_raster.dtype()), &info,
    ...
    for (int band = 0; band < input_raster.numBands(); ++band) {
        ...
        _Pragma("omp parallel for") for (int block_count = 0;
                                         block_count < nblocks; ++block_count)
        {

            const long y0 = block_count * block_length;
            const long this_block_length =
                    std::min(static_cast<long>(block_length),
                            static_cast<long>(input_raster.length()) - y0);

            _runBlock(input_raster, block_stats_vector, block_count, x0,
                    block_width, y0, this_block_length, band);
        }
    }
    ...
    for (int band = 0; band < nbands; ++band) {
        ...
        info << "band: " << band + 1 << pyre::journal::newline
             ...
             << pyre::journal::newline << "    min: " << stats_vector[band].min
...
template std::vector<isce3::math::Stats<float>> computeRasterStats(
        isce3::io::Raster& input_raster,
        isce3::core::MemoryModeBlockY memory_mode);

template std::vector<isce3::math::Stats<double>> computeRasterStats(
        isce3::io::Raster& input_raster,
        isce3::core::MemoryModeBlockY memory_mode);

template std::vector<isce3::math::Stats<std::complex<float>>>
computeRasterStats(isce3::io::Raster& input_raster,
        isce3::core::MemoryModeBlockY memory_mode);
        ...
        isce3::core::MemoryModeBlockY memory_mode)
{
    ...
    std::vector<std::vector<isce3::math::BlockStats<T>>>
            block_stats_vector_real(
                    nbands, std::vector<isce3::math::BlockStats<T>>(nblocks));
    std::vector<std::vector<isce3::math::BlockStats<T>>>
            block_stats_vector_imag(
                    nbands, std::vector<isce3::math::BlockStats<T>>(nblocks));
    ...
    for (int band = 0; band < input_raster.numBands(); ++band) {
                                         ...
                                         block_count < nblocks; ++block_count)
        {
            ...
            _runBlockRealImag(input_raster, block_stats_vector_real,
                    block_stats_vector_imag, block_count, x0, block_width, y0,
                    this_block_length, band);
        }
    }
    ...
    for (int band = 0; band < nbands; ++band) {
        ...
        stats_vector[band].sample_stddev_real =
                stats_vector_real[band].sample_stddev;
        ...
        stats_vector[band].sample_stddev_imag =
                stats_vector_imag[band].sample_stddev;
        ...
        info << "band: " << band + 1 << pyre::journal::newline
             ...
             << ", sample stddev (real): "
             << stats_vector[band].sample_stddev_real << pyre::journal::newline

             << "    min (imag): " << stats_vector[band].min_imag
             << ", mean (imag): " << stats_vector[band].mean_imag
             << ", max (imag): " << stats_vector[band].max_imag
             << ", sample stddev (imag): "
             << stats_vector[band].sample_stddev_imag << pyre::journal::endl;
    }
    return stats_vector;
}

template std::vector<isce3::math::StatsRealImag<float>>
computeRasterStatsRealImag(isce3::io::Raster& input_raster,
        isce3::core::MemoryModeBlockY memory_mode);
