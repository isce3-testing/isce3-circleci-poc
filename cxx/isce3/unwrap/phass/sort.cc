...
void heapSort(int n, int* vals, int* indexes, int order)
{
    ...
    if (order == 0) {
        ...
        for (a = 0; a < n; a++)
            tmp_indexes[a] = a;
        for (a = n / 2; a > 0; a--)
            maxHeapify(vals, n, tmp_indexes, a);
        for (a = n - 1; a > 0; a--) {
            tmp = tmp_indexes[0];
...
float median_of_medians(int n, float* data, int rank)
{
    ...
    if (rank < low_count)
        return median_of_medians(low_count, low, rank);
    else if (rank > low_count)
        return median_of_medians(high_count, high, rank - low_count - 1);
    else
        return pivot;
}
...
{
    if (start_count == 0 && total_size == 0)
        total_size = n;
    // if(n <= 2) exit(0);
    int print_flag = 0;
    if (print_flag)
        cerr << "n: " << n << "  start_count: " << start_count
    ...
    if (step < 0.000001)
        return (float) ((maxv + minv) / 2.0);
    step += step * 0.01;

    if (print_flag)
        cerr << "minv: " << minv << "  maxv: " << maxv << "  step: " << step
             << "  levels: " << levels << endl;
    ...
    for (int i = 0; i < n; i++) {
        ...
        histogram[level]++;
        if (print_flag && level == 1 && n == 19)
            cerr << "level: " << level << "  x: " << x
                 << "  histo: " << histogram[level] << endl;
    }

    for (int level = 0; level < levels; level++) {
        if (print_flag)
            cerr << "level: " << level << "  histo: " << histogram[level]
                 << endl;
    }
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (level == level0) {
            new_data[count] = x;
            if (print_flag && (new_n == 19 || new_n == 3))
    ...
    if (level0 > 0)
        start_count = tot - histogram[level0];
