...
ChangeDetector::ChangeDetector(int nr_lines_, int nr_pixels_, float no_data_,
        DataPatch<float>* data_patch_, int change_type_, float change_th_,
        int window, int iter)
{
    basic_init();
    nr_lines = nr_lines_;
...
ChangeDetector::~ChangeDetector() { delete change_patch; }
...
void ChangeDetector::calculate()
{
    ...
    for (int iter = 0; iter < iterations; iter++) {
        ...
        for (int n = 0; n < N; n++) {
            DataPatch<float>* work_patch =
                    new DataPatch<float>(box_size, box_size);
            ...
            for (int m = 0; m < M; m++) {
                ...
                //        if(n == 0 && m == 60) cerr << "peak1: " << peak1 << "
                //        peak1_id: " << peak1_id << "  checked: " <<
                //        (int)checked[peak1_id] << endl;

                for (int ii = peak1_id + 1; ii < items; ii++) {
                    if (histo[ii] <= histo[ii - 1]) {

                        // if(n == 0 && m == 60) cerr << " +++++   ii: " << ii
                        // << "  histo[ii]: " << histo[ii] << endl;
                ...
                /*
                      ...
                      FILE *fp = fopen("0_60.dat", "w");
                          for(int ii = 0; ii < items; ii ++) {
                        fprintf(fp, "%d  %d  %d  %lf\n", ii, histo[ii],
                   (int)checked[ii], minv + delta * ii);
                      }
                          fclose(fp);
                          cerr << "n: " << n << "  m: " << m << "  peak1: " <<
                   peak1 << "  peak1_id: " << peak1_id << "  peak2: " << peak2
                   << "  peak2_id: " << peak2_id << endl;
                    }
                        if(n == 0 && m == 73) {
                ...
                */
                ...
                if (count > 1) {
                    if (change_type == 0) {
                        double ratio = (minv + delta * peak1_id) /
                                       (minv + delta * peak2_id);
                        if (ratio > change_th) {
                            for (int i = 0; i < box_size; i++) {
                                if (start_line + i >= nr_lines)
                                    continue;
                                for (int j = 0; j < box_size; j++) {
                                    if (start_pixel + j >= nr_pixels)
                                        continue;
                                    change_data[start_line + i]
                                               [start_pixel + j]++;
                                }
                            }
                        }
    ...
    for (int i = 0; i < nr_lines; i++) {
        for (int j = 0; j < nr_pixels; j++) {
            if (change_data[i][j] > max_change)
                max_change = change_data[i][j];
        }
    }
