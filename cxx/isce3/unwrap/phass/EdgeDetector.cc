...
void detect_edge(int nr_lines, int nr_pixels, float** data,
        unsigned char** edge_data, int window_length, double C_min,
        double R_edge, double R_line)
{
    ...
    cerr << "C_min: " << C_min << "  R_edge: " << R_edge
         << "  R_line: " << R_line << endl;
...
#pragma omp parallel for
    for (int line = start_line; line < stop_line; line++) {
        ...
        for (int pixel = start_pixel; pixel < stop_pixel; pixel++) {
            ...
            //     if(line > 5995 ) cerr << "  line: " << line << "  pixel: " <<
            //     pixel << "  mean: " << mean << "  deviation: " << deviation
            //     << endl;
            if (mean <= 1.0e-20 && deviation <= 1.0e-20)
                continue;
            ...
            if (Ca < C_min)
                continue; // Homogenous area with no edge
            ...
            if (R < R_edge)
                edge_data[line][pixel] =
                        (unsigned char) (255 * (R_edge - R) / R_edge);
            ...
            if (R < R_line)
                edge_data[line][pixel] =
                        (unsigned char) (255 * (R_line - R) / R_line);
        }
    }
    ...
    for (int line = 0; line < win; line++) {
        for (int pixel = 0; pixel < nr_pixels; pixel++) {
            ...
            for (int ii = line - win; ii <= line + win; ii++) {
                if (ii < 0)
                    continue;
                if (edge_data[ii][pixel] > maxValue)
                    maxValue = edge_data[ii][pixel];
            }
    ...
    for (int pixel = 0; pixel < win; pixel++) {
        for (int line = 0; line < nr_lines; line++) {
            ...
            for (int jj = pixel - win; jj <= pixel + win; jj++) {
                if (jj < 0)
                    continue;
                if (edge_data[line][jj] > maxValue)
                    maxValue = edge_data[line][jj];
            }
            edge_data[line][pixel] = maxValue;
        }
    }
...
void detect_edge(int nr_lines, int nr_pixels, float** data,
        unsigned char** horizontal_edge_data,
        unsigned char** vertical_edge_data, int window_length,
        double coefficient_variance_min, double max_edge_ratio)
{
    unsigned char Zero = (unsigned char) 0;
    // unsigned char One  = (unsigned char)1;
...
#pragma omp parallel for
    for (int line = start_line; line < stop_line; line++) {
        for (int pixel = start_pixel; pixel < stop_pixel; pixel++) {
            ...
            if (Ca < coefficient_variance_min)
                continue;
            double weight = (1.0 - Cu2 / Ca / Ca) / (1 + Cu2);
            ;
            mean_data[line][pixel] = mean_data[line][pixel] * weight +
                                     data[line][pixel] * (1.0 - weight);
        }
    }
    delete ca_patch;
    ...
    for (int line = start_line; line < stop_line; line++) {
        for (int pixel = start_pixel; pixel < stop_pixel; pixel++) {
            ...
            if (ii2 < nr_lines) {
                ...
                if (ratio > 0) {
                    horizontal_edge_data[line][pixel] =
