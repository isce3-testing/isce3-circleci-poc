...
CannyEdgeDetector::CannyEdgeDetector(int nr_lines_, int nr_pixels_,
        float no_data_, float** data, float low, float high, int gw,
        double gw_sigma_)
{
    basic_init();
    nr_lines = nr_lines_;
...
void CannyEdgeDetector::calculate(float** data)
{
    ...
    DataPatch<float>* tmp_edge_patch =
    ...
    if (gw_sigma > 0.00001) {
        ...
        DataPatch<float>* gaussian_patch =
                new DataPatch<float>(gw_size, gw_size);
...
#pragma omp parallel for
            ...
            for (int j = 0; j < nr_pixels; j++) {
                ...
                for (int ii = i - gw; ii <= i + gw; ii++) {
                    ...
                    for (int jj = j - gw; jj <= j + gw; jj++) {
                        if (jj < 0 || jj >= nr_pixels)
                            continue;
                        if (data[ii][jj] == no_data)
                            continue;
                        int k1 = ii - i + gw;
                        int k2 = jj - j + gw;
    ...
    // 2.  calculate gradient or edge strength and determine the direction of
    // the edge

    DataPatch<unsigned char>* dir_patch =
    ...
    for (int i = 2; i < nr_lines - 1; i++) {
        for (int j = 2; j < nr_pixels - 1; j++) {
            double gx = tmp_edge[i - 2][j] - tmp_edge[i - 2][j - 1] +
                        2.0 * tmp_edge[i - 1][j] -
                        2.0 * tmp_edge[i - 1][j - 1] + 2.0 * tmp_edge[i][j] -
                        2.0 * tmp_edge[i][j - 1] + tmp_edge[i + 1][j] -
                        tmp_edge[i + 1][j - 1];
            double gy = tmp_edge[i][j - 2] - tmp_edge[i - 1][j - 2] +
                        2.0 * tmp_edge[i][j - 1] -
                        2.0 * tmp_edge[i - 1][j - 1] + 2.0 * tmp_edge[i][j] -
                        2.0 * tmp_edge[i - 1][j] + tmp_edge[i][j + 1] -
                        tmp_edge[i - 1][j + 1];
            ...
            if (theta <= eighth || theta >= PI - eighth)
                dir[i][j] = 1;
            else if (fabs(theta - PI / 4.0) <= eighth)
                dir[i][j] = 2;
            else if (fabs(theta - PI / 2.0) <= eighth)
                dir[i][j] = 3;
            else if (fabs(theta - PI * 0.75) <= eighth)
                dir[i][j] = 4;
            else
                dir[i][j] = 0;
        }
    ...
    for (int i = 2; i < nr_lines - 1; i++) {
        for (int j = 2; j < nr_pixels - 1; j++) {
            if (dir[i][j] == 1) {
                if (tmp_edge[i][j - 1] >= tmp_edge[i][j] ||
                        tmp_edge[i][j + 1] >= tmp_edge[i][j]) {
                    edge[i][j] = 0;
                }
            } else if (dir[i][j] == 2) {
                if (tmp_edge[i - 1][j - 1] >= tmp_edge[i][j] ||
                        tmp_edge[i + 1][j + 1] >= tmp_edge[i][j]) {
                    edge[i][j] = 0;
                }
            } else if (dir[i][j] == 3) {
                if (tmp_edge[i - 1][j] >= tmp_edge[i][j] ||
                        tmp_edge[i + 1][j] >= tmp_edge[i][j]) {
                    edge[i][j] = 0;
                }
            } else if (dir[i][j] == 4) {
                if (tmp_edge[i + 1][j - 1] >= tmp_edge[i][j] ||
    ...
    // 3. non-maxima suppression
      ...
      for(int i = 1; i < nr_lines - 1; i ++) {
        for(int j = 1; j < nr_pixels - 1; j ++) {
          if(dir[i][j] == 1) {
        if(tmp_edge[i][j-1] >= tmp_edge[i][j] || tmp_edge[i][j+1] >=
    tmp_edge[i][j]) { edge[i][j] = 0;
        }
          }
          else if(dir[i][j] == 2) {
        if(tmp_edge[i-1][j-1] >= tmp_edge[i][j] || tmp_edge[i+1][j+1] >=
    tmp_edge[i][j]) { edge[i][j] = 0;
        }
          }
          else if(dir[i][j] == 3) {
        if(tmp_edge[i-1][j] >= tmp_edge[i][j] || tmp_edge[i+1][j] >=
    tmp_edge[i][j]) { edge[i][j] = 0;
        }
          }
          else if(dir[i][j] == 4) {
    ...
    */
    ...
    for (int line = 1; line < nr_lines - 1; line++) {
        for (int pixel = 1; pixel < nr_pixels - 1; pixel++) {
            if (edge[line][pixel] > T2)
                workq.push(Point(pixel, line));
        }
    }
