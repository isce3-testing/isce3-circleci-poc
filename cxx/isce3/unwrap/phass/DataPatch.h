...
public:
    ...
    void write(char* filename,
            int append = 0); // dumps the used part of patch to filename
};
...
template<class type>
inline type DataPatch<type>::operator()(int y, int x)
{
    //  DPRINT("operator()\n");
    if (y < 0 || y >= nr_lines)
        cerr << "DataPatch::operator()(int x,int y), Invalid par y: " << y
             << endl;
    if (x < 0 || x >= nr_pixels)
        cerr << "DataPatch::operator()(int x,int y), Invalid par x: " << x
             << endl;
    return data_lines[y][x];
}

template<class type>
inline type DataPatch<type>::operator[](int k)
{
    //  DPRINT("operator[]\n");
...
template<class type>
inline const type DataPatch<type>::operator[](int k) const
{
    //  DPRINT("operator[]\n");
    if (k < 0 || k >= nr_lines * nr_pixels)
        cerr << "DataPatch::operator[](int k), Invalid par k: " << k << endl;
...
inline DataPatch<type>::DataPatch(int pixels, int lines)
{
    ...
    if (data == NULL)
        cerr << "DataPatch: unable to allocate memory: "
...
template<class type>
inline void DataPatch<type>::set_actual_lines(
        int act_start_line, int act_nr_lines)
{
    if (act_start_line < 0)
        cerr << "DataPatch:invalid actual_start_line: " << act_start_line
             << endl;
    if (act_nr_lines <= 0)
        cerr << "DataPatch:invalid actual_nr_lines: " << act_nr_lines << endl;
    if (act_start_line + act_nr_lines > nr_lines)
        cerr << "DataPatch:invalid actual_lines: "
             << act_start_line + act_nr_lines << endl;
...
template<class type>
inline void DataPatch<type>::set_actual_pixels(
        int act_start_pixel, int act_nr_pixels)
{
    if (act_start_pixel < 0)
        cerr << "DataPatch:invalid actual_start_pixel: " << act_start_pixel
             << endl;
    if (act_nr_pixels <= 0)
        cerr << "DataPatch:invalid actual_nr_pixels: " << act_nr_pixels << endl;
    if (act_start_pixel + act_nr_pixels > nr_pixels)
        cerr << "DataPatch:invalid actual_pixels: "
             << act_start_pixel + act_nr_pixels << endl;
...
template<class type>
inline void DataPatch<type>::set_extern_start_line(int start_line)
{
    if (start_line < 0)
...
template<class type>
inline void DataPatch<type>::set_extern_start_pixel(int start_pixel)
{
    if (start_pixel < 0)
...
inline std::ostream &operator<< (std::ostream &stream, DataPatch<type>& patch)
{
  ...
  stream << "actual_start_line         " << patch.actual_start_line  <<
std::endl; stream << "actual_start_pixel        " << patch.actual_start_pixel <<
std::endl; stream << "actual_nr_lines           " << patch.actual_nr_lines  <<
std::endl; stream << "actual_nr_pixels          " << patch.actual_nr_pixels <<
std::endl; return stream;
}
*/
//------------------------------------------------------------------------------
template<class type>
inline void DataPatch<type>::dump(char* filename)

{
    ...
    for (int line = actual_start_line; line < actual_nr_lines; line++) {
        for (int pixel = actual_start_pixel; pixel < actual_nr_pixels; pixel++)
            if (fwrite((void*) &data_lines[line][pixel], sizeof(type), 1, fp) !=
                    1) {
                cerr << "DataPatch::dump: error on writing: " << filename
                     << endl;
                goto cleanup;
            }
    }
...
//------------------------------------------------------------------------------
template<class type>
inline void DataPatch<type>::write(char* filename, int append)

{
    ...
    for (int line = actual_start_line; line < actual_nr_lines; line++) {
        for (int pixel = actual_start_pixel; pixel < actual_nr_pixels; pixel++)
            if (fwrite((void*) &data_lines[line][pixel], sizeof(type), 1, fp) !=
                    1) {
                cerr << "DataPatch::write: error on writing: " << filename
                     << endl;
                goto cleanup;
            }
    }
...
//------------------------------------------------------------------------------
template<class type>
inline void DataPatch<type>::dumpall(char* filename)
{
    FILE* fp;
    if (!(fp = fopen(filename, "w"))) {
        cerr << "DataPatch::dump: error on opening: " << filename << endl;
...
inline type DataPatch<type>::mean()
{
    int cnt = 0;
    type sum = 0.0;
    for (int line = actual_start_line;
            line < actual_start_line + actual_nr_lines; line++) {
        for (int pixel = actual_start_pixel;
                pixel < actual_start_pixel + actual_nr_pixels; pixel++) {
