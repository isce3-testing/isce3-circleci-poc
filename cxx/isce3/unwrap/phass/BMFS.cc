...
void moving_window_average(
        int items, double* input, int window_length, double ped)
{
    ...
    for (int i = 0; i < 2 * window_length + 1; i++) {
        weight[i] = 0.5 + ped / 2.0 -
                    (0.5 - ped / 2.0) *
                            cos(PI * (double) i / (double) window_length);
    }
...
double* standard_deviation(
        int items, double* input, int window_length, double* mean)
{
    ...
    for (int line = 0; line < items; line++) {
        ...
        if (count > 0)
            dev[line] = sqrt(sum / (double) count);
        else
            dev[line] = 0;
    }
    return dev;
