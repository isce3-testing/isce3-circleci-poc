...
void make_node_patch(char* int_file, char* corr_file, char* amp_file,
        char* layover_file, int start_line, int nr_lines, int nr_pixels,
        double /* corr_th */, double phase_th, double /* amp_th */,
        DataPatch<Node>** return_node_patch,
        DataPatch<float>** return_phase_patch, double minimum_corr,
        double good_corr, double const_phase_to_remove, int sq)
{
    // if corr_file does not exist, the magnitude of int_file is the correlation
    // coefficients
    ...
    if (fopen(corr_file, "r") == NULL) { // no separate corr_file
        ...
        long long byte_offset =
                (long long) 8 * (long long) nr_pixels * (long long) start_line;
        ...
        if (fopen(amp_file, "r") != NULL) {
            ...
            fp = fopen(amp_file, "r");
            long long byte_offset = (long long) 4 * (long long) nr_pixels *
    ...
    } else if (fopen(amp_file, "r") ==
               NULL) { // corr_file separate but amp_file not exist
        FILE* fp = fopen(corr_file, "r");
        long long byte_offset =
                (long long) 4 * (long long) nr_pixels * (long long) start_line;
        fseek(fp, byte_offset, SEEK_SET);
        for (int line = 0; line < nr_lines; line++) {
    ...
    /*
      ...
      if(amp_data) {
        ...
        detect_edge(nr_lines, nr_pixels, amp_data, edge_data, window_length,
      C_min, R_edge, R_line);
    ...
    */
    ...
    if (1) {
        ...
        DataPatch<unsigned char>* tmp_patch =
                new DataPatch<unsigned char>(nr_pixels, nr_lines);
        ...
        for (int line = 0; line < nr_lines; line++) {
            for (int pixel = 0; pixel < nr_pixels; pixel++) {
                float x = (float) ((double) tmp_data[line][pixel] /
                                   (double) cost_scale);
                if (corr_data[line][pixel] > 0) {
                    corr_data[line][pixel] = x;
                    // if(x < corr_data[line][pixel]) corr_data[line][pixel] =
                    // x; if(corr_data[line][pixel] > 0.5)
                    // corr_data[line][pixel] = 0.5;
                }
            }
        }
    ...
    /*
      ...
      if(0){
        DataPatch<unsigned char> *tmp_patch = new DataPatch<unsigned
    char>(nr_pixels, nr_lines); unsigned char **tmp_data =
    tmp_patch->get_data_lines_ptr(); double max_phase_std = 1.2;
        compute_corr(nr_lines, nr_pixels, phase_data, tmp_data, max_phase_std);
        ...
        for(int line = 0; line < nr_lines; line ++) {
          for(int pixel = 0; pixel < nr_pixels; pixel ++) {
            ...
            double phase_std = phase_deviation + max_phase_std *
    edge_data[line][pixel]/255.0;
    ...
    //	if(x > 1.0) x = 1.0;
            ...
            x = 1 - x;
            if(edge_data) {
          if(edge_data[line][pixel] > 0) x = 1.0; // x +=
    edge_data[line][pixel]/255.0;
        }
    ...
    */
    ...
    if (fp_lay) { // yes layover_file
        ...
        long long byte_offset = (long long) sizeof(char) *
                                (long long) nr_pixels * (long long) start_line;
        fseek(fp_lay, byte_offset, SEEK_SET);

        DataPatch<unsigned char>* layover_patch =
                new DataPatch<unsigned char>(nr_pixels, nr_lines);
      ...
      if(amp_data) {
        ...
        for(int i = 0; i < nr_lines; i++) {
          for(int j = 0; j < nr_pixels; j++) {
            amp_data[i][j] = 10.0*log10(amp_data[i][j]+1.0e-20);  // for power
      inputs
          }
        }
        CannyEdgeDetector *edgeDetector = new CannyEdgeDetector(nr_lines,
      nr_pixels, -200.0, amp_data, low, high, gw, gws);

        float **edge = edgeDetector->get_edge();


        float corr_th_plus = corr_th + 1.01/(200.0 * corr_th); // 0.05; //
      corr_th + (sqrt(corr_th*corr_th + 0.01) - corr_th) + 0.01;

        cerr << "corr_th: " << corr_th << "  corr_th_plus: " << corr_th_plus <<
      endl;

        for(int i = 0; i < nr_lines; i ++) {
          for(int j = 0; j < nr_pixels; j ++) {
        //if(edge[i][j] > low) corr_data[i][j] = 0;
        if(edge[i][j] > low && corr_data[i][j] > corr_th_plus) corr_data[i][j] =
      corr_th_plus; if(edge[i][j] >= mid) corr_data[i][j] = 0;
          }
        }
        delete edgeDetector;
      }
    ...
    /*
      for(int line = 0; line < nrows; line++) {
        ...
        if(pixel > 0 && pixel < ncols - 1) {
          node_data[line][pixel].dc =
      Vweight_patch->get_data_lines_ptr()[line][pixel - 1];
        }
          }
          else if(line < nrows - 1) {    // For middle rows ......
        node_data[line][pixel].rc = Hweight_patch->get_data_lines_ptr()[line -
      1][pixel]; if(pixel > 0 && pixel < ncols - 1) { node_data[line][pixel].dc
      = Vweight_patch->get_data_lines_ptr()[line - 1][pixel - 1];
        }
          }
    ...
    */  // Oct 21, 2015 commented by XWU
    ...
    for (int line = 0; line < nrows; line++) {
        for (int pixel = 0; pixel < ncols; pixel++) {
            if (line == 0) { // For the first row ......
                ...
                //	  node_data[line][pixel].rc = 255;
                //	  x = min(corr_data[line][pixel - 1],
                //corr_data[line][pixel]); 	  node_data[line][pixel].dc = (uchar)(x
                //* cost_scale);
                //	}
                //	else if(pixel < ncols - 2) {
                else {
            ...
            if (node_data[line][pixel].dc > mask_th)
                node_data[line][pixel].dc = 255;
            if (node_data[line][pixel].rc > mask_th)
                node_data[line][pixel].rc = 255;
        }
    }
    ...
    if (amp_data) {
        ...
        for (int i = 0; i < nr_lines; i++) {
            for (int j = 0; j < nr_pixels; j++) {
                amp_data[i][j] = 10.0 * log10(amp_data[i][j] +
                                                1.0e-20); // for power inputs
            }
        }
        CannyEdgeDetector* edgeDetector = new CannyEdgeDetector(
                nr_lines, nr_pixels, -200.0, amp_data, low, high, gw, gws);
    ...
    */
    ...
    // scale the rc (right cohearence) based on the pixel index, near or far
    // range
    /*
      for(int line = 0; line < nrows; line++) {
        for(int pixel = 0; pixel < ncols; pixel ++) {
    ...
    */
    ...
    /*
      if(amp_data) {
        ...
        DataPatch<unsigned char> *Hedge_patch = new DataPatch<unsigned
    char>(nr_pixels, nr_lines); unsigned char **Hedge_data =
    Hedge_patch->get_data_lines_ptr();

        DataPatch<unsigned char> *Vedge_patch = new DataPatch<unsigned
    char>(nr_pixels, nr_lines); unsigned char **Vedge_data =
    Vedge_patch->get_data_lines_ptr();

        detect_edge(nr_lines, nr_pixels, amp_data, Hedge_data, Vedge_data,
    window_length, Ca_min, edge_ratio_max);
        ...
        if(line == 0) {        // For the first row ......
          if(pixel > 0 && pixel < ncols - 1) {
                if(Vedge_data[line][pixel - 1] > edge_th)
    node_data[line][pixel].dc = i_min(10, node_data[line][pixel].dc);
            //int r = node_data[line][pixel].dc - Vedge_data[line][pixel - 1] *
    255/100.0; // *(1.0 - Vedge_data[line][pixel - 1]/255.0);
            //if(r < 0) r = 0;
          ...
          if(pixel == 0) {
            if(Hedge_data[line - 1][pixel] > edge_th) node_data[line][pixel].rc
    = i_min(node_data[line][pixel].rc, 10);
            //int r = node_data[line][pixel].rc - Hedge_data[line - 1][pixel] *
    255/100.0; // *(1.0 - Hedge_data[line - 1][pixel]/255.0);
            //if(r < 0) r = 0;
    ...
    255/100.0; // *(1.0 - Hedge_data[line - 1][pixel]/255.0);
            ...
            //node_data[line][pixel].rc = (unsigned char)r;
                ...
                if(Hedge_data[line][pixel - 1] > edge_th)
    node_data[line][pixel].dc = i_min(node_data[line][pixel].dc, 10);
           // r = node_data[line][pixel].dc - Vedge_data[line][pixel - 1]
    *255/100.0; // * (1.0 - Vedge_data[line][pixel - 1]/255.0);

    //if(abs(line - 1108) < 3 && abs(pixel - 936) < 3) cerr << "line: " << line
    << "  r: " << r << "  dc: " << (int)node_data[line][pixel].dc << "  Vedge: "
    << (int)Vedge_data[line][pixel - 1] << endl;
            //if(r < 0) r = 0;
            //node_data[line][pixel].dc = (unsigned char)r;
          }
        }
  ...
  { dx = fabs(20.0*(log10(amp_data[line][pixel]) - log10(amp_data[line][pixel -
  1])));

            //cerr << "line: " << line << "  pixel: " << pixel << "  dx: " << dx
  << endl;

            if(dx >= amp_th) node_data[line][pixel].dc = 0;
            //else node_data[line][pixel].dc = (uchar)(node_data[line][pixel].dc
  * (amp_th - dx)/amp_th);
          }
        }
      }
  ...
  { dy = fabs(20.0*(log10(amp_data[line][pixel]) - log10(amp_data[line -
  1][pixel])));
            //cerr << "line: " << line << "  pixel: " << pixel << "  dy: " << dy
  << endl; if(dy >= amp_th) node_data[line][pixel].rc = 0;
            //else node_data[line][pixel].rc = (uchar)(node_data[line][pixel].rc
  * (amp_th - dy)/amp_th);
          }
        }
        else if(pixel < ncols - 1) {
  ...
  * (amp_th - dy)/amp_th);
  ...
  1])));
            //cerr << "line: " << line << "  pixel: " << pixel << "  dx: " << dx
  << endl; if(dx >= amp_th) node_data[line][pixel].dc = 0;
            //else node_data[line][pixel].dc = (uchar)(node_data[line][pixel].dc
  * (amp_th - dx)/amp_th);
          }
        }
  ...
  */
    ...
    for (int line = 0; line < nrows; line++) {
        for (int pixel = 0; pixel < ncols; pixel++) {
            if (line == 0) { // For the first row ......
                if (pixel > 0 && pixel < ncols - 1) {
                    if (corr_data[line][pixel] > small &&
                            corr_data[line][pixel - 1] > small) {
                        dx = phase_data[line][pixel] -
                             phase_data[line][pixel - 1];
                        dx = fabs(dx);
                        if (dx > PI)
                            dx = two_pi - dx;
                        if (dx >= max_dph)
                        ...
                        //            else {
                        //	      node_data[line][pixel].dc = (unsigned
                        //char)((1.0 - dx/max_dph) *
                        //(double)node_data[line][pixel].dc);
                        //	    }
                        // if(dx > phase_difference_threshold) dx =
                        // phase_difference_threshold; node_data[line][pixel].dc
                        // = (uchar)(x * (phase_difference_threshold -
                        // dx)/phase_difference_threshold * cost_scale);
                    }
                }
            } else if (line < nrows - 1) { // For middle rows ......
                if (pixel == 0) {
                    if (corr_data[line][pixel] > small &&
                            corr_data[line - 1][pixel] > small) {
                        dx = phase_data[line][pixel] -
                             phase_data[line - 1][pixel];
                        dx = fabs(dx);
                        if (dx > PI)
                            dx = two_pi - dx;
                        if (dx >= max_dph)
                        ...
                        //            else {
                        //	      node_data[line][pixel].rc = (unsigned
                        //char)((1.0 - dx/max_dph) *
                        //(double)node_data[line][pixel].rc);
                        //	      //node_data[line][pixel].rc *= 1.0 -
                        //dx/max_dph;
                        //	    }
                        // if(dy > phase_difference_threshold) dy =
                        // phase_difference_threshold; node_data[line][pixel].rc
                        // = (uchar)(y * (phase_difference_threshold -
                        // dy)/phase_difference_threshold *cost_scale);
                    }
                } else if (pixel < ncols - 1) {
                    if (corr_data[line][pixel] > small &&
                            corr_data[line - 1][pixel] > small) {
    ...
    */
    ...
    if (Hweight_patch)
        delete Hweight_patch;
    if (Vweight_patch)
        delete Vweight_patch;
...
DataPatch<Node>* make_node_patch(DataPatch<fcomplex>* int_patch, double qthresh)
{
    ...
    DataPatch<Node>* node_patch =
            make_node_patch(nr_lines, nr_pixels, amp_data, phase_data, qthresh);
...
DataPatch<Node>* make_node_patch(int nr_lines, int nr_pixels, float** corr_data,
        float** phase_data, double /* qthresh */)
{
    int nrows = nr_lines + 1;
    int ncols = nr_pixels + 1;
    ...
    */
    ...
    for (int line = 0; line < nrows; line++) {
        for (int pixel = 0; pixel < ncols; pixel++) {
            if (line == 0) { // For the first row ......
                if (pixel > 0 && pixel < ncols - 1) {
                    // x = (corr_data[line][pixel - 1] + corr_data[line][pixel]
                    // )/2.0;
                    x = min(corr_data[line][pixel - 1], corr_data[line][pixel]);
                    // x *= x;
                    // node_data[line][pixel].dc = (uchar)(x * cost_scale);
                    dx = fabs(phase_data[line][pixel] -
                              phase_data[line][pixel - 1]);
                    if (dx > pi)
                        dx = 2 * pi - dx;
                    if (dx > phase_difference_threshold)
                        dx = phase_difference_threshold;
                    node_data[line][pixel].dc = (uchar) (x * cost_scale);
                    // node_data[line][pixel].dc = (uchar)(x *
                    // (phase_difference_threshold -
                    // dx)/phase_difference_threshold * cost_scale);
                }
            } else if (line < nrows - 1) { // For middle rows ......
                if (pixel == 0) {
                    // y = ( corr_data[line - 1][pixel] + corr_data[line][pixel]
                    // )/2.0;
                    y = min(corr_data[line - 1][pixel], corr_data[line][pixel]);
                    // y *= y;
                    // node_data[line][pixel].rc = (uchar)(y *cost_scale);
                    dy = fabs(phase_data[line][pixel] -
                              phase_data[line - 1][pixel]);
                    if (dy > pi)
                        dy = 2 * pi - dy;
                    if (dy > phase_difference_threshold)
                        dy = phase_difference_threshold;
                    node_data[line][pixel].rc = (uchar) (y * cost_scale);
                    // node_data[line][pixel].rc = (uchar)(y *
                    // (phase_difference_threshold -
                    // dy)/phase_difference_threshold *cost_scale);
                } else if (pixel < ncols - 1) {
                    // y = (corr_data[line - 1][pixel] + corr_data[line][pixel]
                    ...
                    // if(line % 100 == 0) cerr << line << " " << pixel << "  x:
                    // " << x << "  dc: " << (int)node_data[line][pixel].dc << "
                    // cost_scale: " << cost_scale << endl;
                }
            }
        }
    }
...
DataPatch<char>* unwrap_assp(DataPatch<NodeFlow>* flows_patch,
        float** phase_data, int nr_seeds, Seed* seeds)
{
    ...
    for (int seed_id = 0; seed_id < nr_seeds; seed_id++) {
        ...
        // cerr << "Seed: " << Point(seed_x, seed_y) << "  nr_2pi: " <<
        // seeds[seed_id].nr_2pi << "  seed phase: " <<
        // phase_data[seed_y][seed_x] << endl;
...
void create_seeds(DataPatch<NodeFlow>* flows_patch, int minimum_nr_pixels,
        int& nr_seeds, Seed** seeds)
{
    int nr_lines = flows_patch->get_nr_lines() - 1;
    int nr_pixels = flows_patch->get_nr_pixels() - 1;
    int line, pixel, line_plus, line_minus, pixel_plus, pixel_minus;
    ...
    for (int ii = 0; ii < nr_lines; ii++) {
        ...
        // reversing the order so that farther out pixesl are used for the seed,
        // reducing the sensitivity to bad dem
        for (int jj = nr_pixels - 1; jj >= 0; jj--) {

            // cerr << "ii: " << ii << "  jj: " << jj << "  visit: " <<
            // visit[jj][ii] << "  region_id: " << region_id << "  visit: " <<
            // (int)visit[ii][jj]<< endl;

            if (visit[ii][jj] == unwrapped)
                continue;
            ...
            while (!workq.empty()) {
                ...
                if (line < nr_lines - 1) { // facing down ......
                    if (flows[line_plus][pixel].toRight == 0 &&
                            visit[line_plus][pixel] == not_unwrapped) {
                        workq.push(Point(pixel, line_plus));
                        visit[line_plus][pixel] = unwrapped;
                    }
            ...
            // if(count > 1) cerr << "ii: " << ii << "  jj: " << jj << "
            // region_id: " << region_id << "  count: " << count << "  mini: "
            // << minimum_nr_pixels<<  endl;
            if (count >= minimum_nr_pixels) {

                pixels_of_region[region_id] = count;
                region_id++;

                if (region_id >= max_seeds)
                    break;
            }
        }
        if (region_id >= max_seeds)
    ...
    for(int i = 0; i < 10; i++) {
      int index = indexes[i];
      cerr << "i: " << i << "  index: " << index << "  pixels_of_region: " <<
    pixels_of_region[ index ] << "  seed: " << tmp_seeds[index].x << " : " <<
    tmp_seeds[index].y << endl;
    }
    */
    ...
    for (int i = 0; i < nr_regions; i++) {
        ...
        //  cerr << "i: " << i << "  index: " << index << "  seed: " <<
        //  return_seeds[i].x << " " << return_seeds[i].y << " " <<
        //  return_seeds[i].nr_2pi << "  pixels_of_region: " <<
        //  pixels_of_region[index] << endl;
    }
...
DataPatch<char>* unwrap_adjust_seeds(DataPatch<NodeFlow>* flows_patch,
        float** phase_data, int nr_seeds, Seed* seeds)
{
    int patch_start = flows_patch->get_extern_start_line();
    int nr_lines = flows_patch->get_nr_lines() - 1;
    ...
    for (int seed_id = 0; seed_id < nr_seeds; seed_id++) {
        ...
        if (visit[seed_y][seed_x] == unwrapped)
            continue;
        ...
        while (!workq.empty()) {
            ...
            for (int aid = 0; aid < nr_amb; aid++) {
                if (seed_phase >= lower_bound[aid] &&
                        seed_phase < upper_bound[aid]) {
                    histogram[aid]++;
                }
            }
...
DataPatch<int>* generate_regions(
        DataPatch<NodeFlow>* flows_patch, int nr_seeds, Seed* seeds)
{
    int patch_start = flows_patch->get_extern_start_line();
    int nr_lines = flows_patch->get_nr_lines() - 1;
    int nr_pixels = flows_patch->get_nr_pixels() - 1;
    ...
    for (int seed_id = 0; seed_id < nr_seeds; seed_id++) {
        ...
        if (visit[seed_y][seed_x] != not_unwrapped)
            continue;
...
void generate_regions(DataPatch<NodeFlow>* flows_patch, int nr_seeds,
        Seed* seeds, int** regions)
{
    int patch_start = flows_patch->get_extern_start_line();
    int nr_lines = flows_patch->get_nr_lines() - 1;
    int nr_pixels = flows_patch->get_nr_pixels() - 1;
    ...
    for (int seed_id = 0; seed_id < nr_seeds; seed_id++) {
        ...
        if (regions[seed_y][seed_x] != not_unwrapped)
            continue;
        ...
        while (!workq.empty()) {
            ...
            if (line > 0) { // facing up ......
                if (flows[line][pixel].toRight == 0 &&
                        regions[line_minus][pixel] == not_unwrapped) {
                    workq.push(Point(pixel, line_minus));
            ...
            if (line < nr_lines - 1) { // facing down ......
                if (flows[line + 1][pixel].toRight == 0 &&
                        regions[line_plus][pixel] == not_unwrapped) {
                    workq.push(Point(pixel, line_plus));
            ...
            if (pixel > 0) { // facing left ......
                if (flows[line][pixel].toDown == 0 &&
                        regions[line][pixel_minus] == not_unwrapped) {
                    workq.push(Point(pixel_minus, line));
            ...
            if (pixel < nr_pixels - 1) { // facing right ......
                if (flows[line][pixel_plus].toDown == 0 &&
                        regions[line][pixel_plus] == not_unwrapped) {
                    workq.push(Point(pixel_plus, line));
...
DataPatch<NodeFlow>* solve(DataPatch<Node>* node_patch)
{
    ...
    DataPatch<NodeFlow>* node_flow_patch =
            new DataPatch<NodeFlow>(ncols, nrows);
    ...
    for (int line = 0; line < nrows; line++) {
        for (int pixel = 0; pixel < ncols; pixel++) {
            ...
            if (nodes[line][pixel].supply > 0)
                pcount++;
            else if (nodes[line][pixel].supply < 0)
                ncount++;
        }
    }
    ...
    if (abs(pcount - ncount) == 1)
        nodes[0][0].supply = -(pcount - ncount);
    ...
    */
    ...
    //  initialize_flows_PI(nrows, ncols, nodes, supplys, S, demands, T,
    //  node_flow, pot);
    ...
    */
    ...
    while (supplys_left > 0) {
        ...
        for (int s = 0; s < supplys; s++) {
            ...
            pixel = S[s].x;
            if (nodes[line][pixel].supply == 0)
                continue; // if Residue discharged
            dists[line][pixel] =
                    0; // Otherwise set all left-over supplys to zero distance
            visit[line][pixel] = labeled;
            point.x = pixel;
            point.y = line;
            dist_queues[0].push(point);

            // cerr << "s: " << s << "  point: " << point << "  dist: " <<
            // dists[ line ][ pixel ] << endl;
        }
        ...
        int min_dist = 0;
        int max_dist = 0;
        while (!dist_queues[min_dist]
                        .empty()) { // as long as the labeled_set is not empty,
                                    // do the following ......
            ...
            //	if(pixel == 3 && line == 3) cerr << "iter: " << iter << "
            //min_dist: " << min_dist << "  scanned: " << point << "  dist: " <<
            //dists[line][pixel] << endl;

            // if(iter == 2) cerr << "min_dist: " << min_dist << "  point: " <<
            // point << endl;
            ...
            if (visit[line][pixel] == scanned) {
                ...
                while (dist_queues[min_dist].empty()) {
                    min_dist++;
                    if (min_dist >= nr_queues)
                        break;
                }
                //	if(pixel == 3 && line == 3) cerr << "after   min_dist: " <<
                //min_dist << endl;
            ...
            if (pixel > 0 && visit[line][pixel - 1] != scanned) {
                //       if(node_flow[line][pixel - 1].toRight >
                //       -flow_limit_per_arc) {
                if (node_flow[line][pixel - 1].toRight != 0)
                    reduced_cost = 0;
                else
                    reduced_cost = nodes[line][pixel - 1].rc +
                                   pot[line][pixel] - pot[line][pixel - 1];
                d = curr_dist + reduced_cost;
                // uchar cost = nodes[line][pixel - 1].rc;
                // if(node_flow[line][pixel - 1].toRight > 0) d = curr_dist -
                // cost; else d = curr_dist + cost;
                ...
                if (d < dists[line][pixel - 1]) {
                    ...
                    //	if(pixel - 1 == 3 && line == 3) cerr << "iter: " << iter
                    //<< "  Left: scanned: " << Point(pixel, line) << "  dist: "
                    //<< dists[line][pixel-1] << endl;
                    // cerr << "LEFT  d : " << d << endl;
                }
                //       }
            }
            ...
            if (pixel < ncols - 1 && visit[line][pixel + 1] != scanned) {
                ...
                //       {
                if (node_flow[line][pixel].toRight != 0)
                    reduced_cost = 0;
                else
                    reduced_cost = nodes[line][pixel].rc + pot[line][pixel] -
                                   pot[line][pixel + 1];
                d = curr_dist + reduced_cost;
                ...
                if (d < dists[line][pixel + 1]) {
                    ...
                    // cerr << "Right  d : " << d << endl;
                    //	if(pixel + 1 == 3 && line == 3) cerr << "iter: " << iter
                    //<< "  Right scanned: " << Point(pixel, line) << "  dist: "
                    //<< dists[line][pixel+1] << endl;
                }
                //       }
            }
            ...
            if (line > 0 && visit[line - 1][pixel] != scanned) {
                //       if(node_flow[line - 1][pixel].toDown >
                //       -flow_limit_per_arc) {
                if (node_flow[line - 1][pixel].toDown != 0)
                    reduced_cost = 0;
                else
                    reduced_cost = nodes[line - 1][pixel].dc +
                                   pot[line][pixel] - pot[line - 1][pixel];
                d = curr_dist + reduced_cost;
                // uchar cost = nodes[line - 1][pixel].dc;
                // if(node_flow[line - 1][pixel].toDown > 0) d = curr_dist -
                // cost; else d = curr_dist + cost;
                ...
                if (d < dists[line - 1][pixel]) {
                    ...
                    if (d < tmp_mind)
                        tmp_mind = d;
                    // cerr << "UP  d : " << d << endl;
                    //	if(pixel == 3 && line - 1 == 3) cerr << "iter: " << iter
                    //<< "  UP scanned: " << Point(pixel, line) << "  dist: " <<
                    //dists[line - 1][pixel] << endl;
                }
                //       }
            }
            ...
            if (line < nrows - 1 && visit[line + 1][pixel] != scanned) {
                ...
                //       {
                if (node_flow[line][pixel].toDown != 0)
                    reduced_cost = 0;
                else
                    reduced_cost = nodes[line][pixel].dc + pot[line][pixel] -
                                   pot[line + 1][pixel];
                d = curr_dist + reduced_cost;
                ...
                if (d < dists[line + 1][pixel]) {
                    ...
                    //	if(pixel == 3 && line + 1 == 3) cerr << "iter: " << iter
                    //<< "  Down scanned: " << Point(pixel, line) << "  dist: "
                    //<< dists[line + 1][pixel] << endl;
                    // cerr << "Down  d : " << d << endl;
                }
            ...
            if (min_dist > max_dist)
                break;
            // if(iter > 0 && scanned_count == supplys_left) cerr <<
            // "total_scanned: " << total_scanned << endl; if(iter > 0 &&
            // scanned_count == supplys_left) break;
        }
        ...
        */
        ...
        for (int line = 0; line < nrows; line++) {
            for (int pixel = 0; pixel < ncols; pixel++) {
                //	  if(dists[line][pixel] > 0 && visit[line][pixel] ==
                //scanned) {
        ...
        */
        ...
        //    cerr << "iter: " << iter << "  supplys_left: " << supplys_left <<
        //    "  scanned_count: "  << scanned_count << "  total_scanned: " <<
        //    total_scanned << endl; cerr << "iter: " << iter << " supplys_left:
        //    " << supplys_left << "  min_dist: " << min_dist << "  max_dist: "
        //    << max_dist << "  nr_queues: " << nr_queues << endl; //"
        //    scanned_count: "  << scanned_count << endl;
        ...
        for (int i = 0; i < demands; i++) {
            ...
            // cerr << "count: " << count << "  point: " << Point(pixel, line)
            // << "  supply: " << (int)nodes[line][pixel].supply << "  dd: " <<
            // dd[count] << endl;
            count++;
        }
        ...
        for (int i = 0; i < count; i++) {
            ...
            do {
                ...
                if (branches[line][pixel] == flow_up) {
                    // if(pixel != 0 && pixel != ncols - 1 && new_flow[line -
                    // 1][pixel].toDown != 0) {
                    ...
                    // if(line != 0 && line != nrows - 1 && new_flow[line][pixel
                    // - 1].toRight != 0) {
            ...
            //     cerr << "for flow start at: " << flow.xstart << ", " <<
            //     flow.ystart  << "  branch_taken: " << branch_taken << "  End
            //     at supply " << pixel << " " << line << "  supply: " <<
            //     (int)nodes[line][pixel].supply << endl;
            ...
            if (nodes[line][pixel].supply == 0)
                continue; // if the supply node is balanced terminate !!
            supplys_left--;
            nodes[line][pixel].supply -= supply;              // supply
            nodes[flow.ystart][flow.xstart].supply -= demand; // demand

            // cerr << "   start supply: " <<
            // (int)nodes[flow.ystart][flow.xstart].supply << "  end supply: "
            // << (int)nodes[line][pixel].supply << endl;
            ...
            for (int j = 0; j < flow.flowdir.size(); j++) {
                if (flow.flowdir[j] == flow_up) {
                    if (pixel > 0 && pixel < ncols - 1)
                        new_flow[line - 1][pixel].toDown++;
                    line--;
                } else if (flow.flowdir[j] == flow_down) {
                    if (pixel > 0 && pixel < ncols - 1)
                        new_flow[line][pixel].toDown--;
                    line++;
                } else if (flow.flowdir[j] == flow_left) {
                    if (line > 0 && line < nrows - 1)
                        new_flow[line][pixel - 1].toRight++;
                    pixel--;
                } else if (flow.flowdir[j] == flow_right) {
                    if (line > 0 && line < nrows - 1)
                        new_flow[line][pixel].toRight--;
                    pixel++;
                }
            } // for loop
        ...
        for (int line = 0; line < nrows; line++) {
            for (int pixel = 0; pixel < ncols; pixel++) {
                if (new_flow[line][pixel].toRight != 0) {
                    node_flow[line][pixel].toRight +=
                            new_flow[line][pixel].toRight;
                    new_flow[line][pixel].toRight = 0;
                }
                if (new_flow[line][pixel].toDown != 0) {
                    node_flow[line][pixel].toDown +=
                            new_flow[line][pixel].toDown;
                    new_flow[line][pixel].toDown = 0;
                }
                /*
                if(abs(new_flow[line][pixel].toRight) > 1 ||
                abs(new_flow[line][pixel].toDown) > 1) cerr << " New Flow  line:
                " << line << "  pixel: " << pixel << "  To Right: " <<
                abs(new_flow[line][pixel].toRight) << "  To Down : " <<
                abs(new_flow[line][pixel].toDown) << endl;
                if(abs(node_flow[line][pixel].toRight) > 1 ||
                abs(node_flow[line][pixel].toDown) > 1) cerr << " total Flow
                line: " << line << "  pixel: " << pixel << "  ToRight: " <<
                abs(node_flow[line][pixel].toRight) << "  To Down: " <<
                abs(node_flow[line][pixel].toDown)<< endl;
                */
                /*
                        if(abs((int)node_flow[line][pixel].toRight) >
                   max_flows_per_arc) { max_flows_per_arc =
                   abs((int)node_flow[line][pixel].toRight);
                      max_flows_location.set(pixel, line);
                    }
                        if(abs((int)node_flow[line][pixel].toDown) >
                   max_flows_per_arc) { max_flows_per_arc =
                   abs((int)node_flow[line][pixel].toDown);
                      max_flows_location.set(pixel, line);
                    }
                */
            }
        ...
        //    cerr << "iter: " << iter << "  supplys_left: " << supplys_left <<
        //    "  max_flows_per_arc: " << max_flows_per_arc << "
        //    max_flows_location: " << max_flows_location << endl; cerr << "(0,
        //    499) toRight: " << (int)node_flow[499][0].toRight << "  toDown: "
        //    << (int)node_flow[499][0].toDown << endl;
        iter++;
    }
