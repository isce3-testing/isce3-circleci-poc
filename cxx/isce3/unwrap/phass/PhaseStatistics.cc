...
DataPatch<unsigned char>* compute_Hweight(
        int nr_lines, int nr_pixels, float** data)
{
    DataPatch<float>* Hdphase_patch =
            compute_H_delta_phase(nr_lines, nr_pixels, data);
    ...
    DataPatch<unsigned char>* Hweight_patch =
            new DataPatch<unsigned char>(nr_pixels, nr_lines - 1);
    ...
    for (int line = 0; line < nr_lines - 1; line++) {
        for (int pixel = 0; pixel < nr_pixels; pixel++) {
            ...
            if (x > upper_limit)
                x = upper_limit;
            Hweight[line][pixel] =
                    255 - 254 * (x - lower_limit) / (upper_limit - lower_limit);
        }
    }
...
DataPatch<unsigned char>* compute_Vweight(
        int nr_lines, int nr_pixels, float** data)
{
    DataPatch<float>* Vdphase_patch =
            compute_V_delta_phase(nr_lines, nr_pixels, data);
    float** Vdphase = Vdphase_patch->get_data_lines_ptr();

    double upper_limit = dphase_uppler_limit; // 1.0;  // radians
    double lower_limit = dphase_lower_limit;  // radians

    DataPatch<unsigned char>* Vweight_patch =
            new DataPatch<unsigned char>(nr_pixels - 1, nr_lines);
...
DataPatch<float>* compute_H_delta_phase(
        int nr_lines, int nr_pixels, float** data)
{
    DataPatch<float>* delta_line_patch =
            new DataPatch<float>(nr_pixels, nr_lines - 1);
    ...
    for (int line = 0; line < nr_lines - 1; line++) {
        ...
        for (int pixel = 0; pixel < nr_pixels; pixel++) {
            ...
            for (int ii = pixel - win; ii <= pixel + win; ii++) {
                if (ii < 0 || ii >= nr_pixels)
                    continue;
                if (tmp_data[ii] == void_phase)
                    continue;
                check_sum += fabs(tmp_data[ii]);
                count++;
                sum += tmp_data[ii];
            }
            if (count == 0 || check_sum == 0)
                delta_line[line][pixel] = PI;
            else
                delta_line[line][pixel] = sum / (double) count;
        }
    }
    delete[] tmp_data;
...
DataPatch<float>* compute_V_delta_phase(
        int nr_lines, int nr_pixels, float** data)
{
    cerr << "compute_V_delta_phase() ...... " << endl;
    DataPatch<float>* delta_pixel_patch =
            new DataPatch<float>(nr_pixels - 1, nr_lines);
    ...
    for (int pixel = 0; pixel < nr_pixels - 1; pixel++) {
        ...
        for (int line = 0; line < nr_lines; line++) {
            ...
            if (count == 0 || check_sum == 0)
                delta_pixel[line][pixel] = PI;
            else
                delta_pixel[line][pixel] = sum / (double) count;
        }
    }
    delete[] tmp_data;
...
void compute_corr(int nr_lines, int nr_pixels, float** data,
        unsigned char** corr_data, double /* max_phase_std */)
{
    DataPatch<float>* delta_pixel_patch =
            new DataPatch<float>(nr_pixels, nr_lines);
    float** delta_pixel = delta_pixel_patch->get_data_lines_ptr();
    //  DataPatch<float> *delta_pixel_mean_patch = new
    //  DataPatch<float>(nr_pixels, nr_lines); float **delta_pixel_mean =
    //  delta_pixel_mean_patch->get_data_lines_ptr();

    DataPatch<float>* delta_line_patch =
            new DataPatch<float>(nr_pixels, nr_lines);
    float** delta_line = delta_line_patch->get_data_lines_ptr();
    //  DataPatch<float> *delta_line_mean_patch = new
    //  DataPatch<float>(nr_pixels, nr_lines); float **delta_line_mean =
    //  delta_line_mean_patch->get_data_lines_ptr();
...
#pragma omp parallel for
    for (int line = 0; line < nr_lines - 1; line++) {
        for (int pixel = 0; pixel < nr_pixels - 1; pixel++) {
            ...
            int count = 0;
            for (int ii = line - win; ii <= line + win; ii++) {
                if (ii < 0 || ii >= nr_lines - 1)
                    continue;
                for (int jj = pixel - win; jj <= pixel + win; jj++) {
                    if (jj < 0 || jj >= nr_pixels - 1)
                        continue;
                    count++;
...
#pragma omp parallel for
    ...
    for (int line = 0; line < nr_lines; line++) {
        for (int pixel = 0; pixel < nr_pixels; pixel++) {
            if (std_data[line][pixel] == 0)
                corr_data[line][pixel] = (unsigned char) cost_scale;
            else {
                // double x = delta_pixel[line][pixel];// -
                // delta_pixel_mean[line][pixel]; double y =
                // delta_line[line][pixel];// - delta_pixel_mean[line][pixel];
                // x = 0.5*(x*x + y*y)/phi_max/phi_max/(std_data[line][pixel] +
                // 0.1); x = std_data[line][pixel];
