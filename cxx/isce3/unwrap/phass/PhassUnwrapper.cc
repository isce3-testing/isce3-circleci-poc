...
void phass_unwrap(int nr_lines, int nr_pixels, float** phase_data,
        float** corr_data, float** power_data, int** region_map, double corr_th,
        double good_corr, int min_pixels_per_region)
{
    ...
    /*
      {
        ...
        for(int line = 0; line < nr_lines; line ++) {
          for(int pixel = 0; pixel < nr_pixels; pixel ++) {
            int_data[pixel] = corr_data[line][pixel] *
      fcomplex(cos(phase_data[line][pixel]), sin(phase_data[line][pixel]));
          }
    ...
    */
    ...
    if (power_data != NULL) { // set cuts based on the detected edges
        ...
        for (int i = 0; i < nr_lines; i++) {
            for (int j = 0; j < nr_pixels; j++) {
                power_data[i][j] = 10.0 * log10(power_data[i][j] +
                                                  1.0e-20); // for power inputs
            }
        }
        CannyEdgeDetector* edgeDetector = new CannyEdgeDetector(
                nr_lines, nr_pixels, -200.0, power_data, low, high, gw, gws);
            ...
            if (line == 0) { // For the first row ......
                if (pixel > 0 && pixel < ncols - 1) {
                    if (corr_data[line][pixel] > small &&
                        ...
                        //  node_data[line][pixel].dc = (unsigned char)((1.0 -
                        //  dx/max_dph) * (double)node_data[line][pixel].dc);
                        //}
                    }
                }
            } else if (line < nrows - 1) { // For middle rows ......
                if (pixel == 0) {
                    if (corr_data[line][pixel] > small &&
                        ...
                        // else if(dx > max_dph/2) {
                        //  node_data[line][pixel].rc = (unsigned char)((1.0 -
                        //  dx/max_dph) * (double)node_data[line][pixel].rc);
                        //}
                    }
    ...
    if (corr_th > 0) {
        ...
        for (int line = 0; line < nrows; line++) {
            for (int pixel = 0; pixel < ncols; pixel++) {
                if (node_data[line][pixel].rc < th &&
                        flow_data[line][pixel].toRight == 0) {
                    if (!isEdge[line][pixel])
                        flow_data[line][pixel].toRight = 1;
                }
                if (node_data[line][pixel].dc < th &&
                        flow_data[line][pixel].toDown == 0) {
                    if (!isEdge[line][pixel])
                        flow_data[line][pixel].toDown = 1;
                }
            }
        }
        for (int line = 0; line < nr_lines + 1; line++) {
    ...
    Seed* seeds = NULL;
    create_seeds(flows, min_pixels_per_region, nr_seeds,
            &seeds); // create seeds only
    //  DataPatch<char> *visit_patch = unwrap_assp(flows, phase_data, nr_seeds,
    //  seeds); // unwrap only
    DataPatch<char>* visit_patch = unwrap_adjust_seeds(
            flows, phase_data, nr_seeds, seeds); // unwrap only
    delete visit_patch;
