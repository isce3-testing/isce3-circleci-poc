...
void growConnComp(float* unw, bool* currcc, size_t* ccsize, const float* phase,
        const float* corr, const bool* tree, const idx2_t& seed,
        const float corrthr, const size_t length, const size_t width)
{
    constexpr float twopi = 2.f * M_PI;

    // Single pixel offsets in each cardinal direction
    constexpr const offset2_t searchpts[4] = {{1, 0}, {0, -1}, {-1, 0}, {0, 1}};
    ...
    while (nold > 0) {
        ...
        for (size_t p = 0; p < nold; ++p) {
            ...
            for (int s = 0; s < 4; ++s) {
                ...
                if (currcc[inewpix] || corr[inewpix] < corrthr) {
                    continue;
                }
        ...
        *ccsize += nnew;
        ...
        // New list becomes old list (reuse old old list buffer for new new
        // list).
...
BootstrapStatus_t estimBootstrapPhase(float* bsphase, const float* unw,
        const bool* currcc, const float* bsunw, const uint8_t* bsccl,
        const size_t width, const size_t numBsLines, const size_t minBsPts,
...
uint8_t bootstrapLabel(LabelMap& labelmap, const bool* currcc,
        const uint8_t* bsccl, const size_t width, const size_t numBsLines)
{
...
void ICU::growGrass(float* unw, uint8_t* ccl, bool* currcc, float* bsunw,
        uint8_t* bsccl, LabelMap& labelmap, const float* phase,
        const bool* tree, const float* corr, float corrthr, const size_t length,
        const size_t width)
{
    ...
    for (size_t sj = seedRowSpcng / 2; sj < length; sj += seedRowSpcng) {
        for (size_t si = seedColSpcng / 2; si < width; si += seedColSpcng) {
            ...
            if (tree[iseed] || corr[iseed] < corrthr || ccl[iseed] != 0) {
                continue;
            }

            // Grow connected component from seed.
            size_t ccsize;
            growConnComp(unw, currcc, &ccsize, phase, corr, tree, seed, corrthr,
                    length, width);

            // Check if connected component is large enough.
            if (ccsize < _MinCCAreaFrac * tilesize) {
                continue;
            }
            ...
            if (DO_BOOTSTRAP) {
                ...
                float bsphase;
                BootstrapStatus_t status = estimBootstrapPhase(&bsphase,
                        &unw[bsoff], &currcc[bsoff], bsunw, bsccl, width,
                ...
                case BootstrapSuccess: {
                    ...
                    uint8_t bslabel = bootstrapLabel(labelmap, &currcc[bsoff],
                            bsccl, width, _NumBsLines);
                ...
                case BootstrapFailure: {
                    ...
                    if (corrthr < _MaxCorrThr) {
                        corrthr += _CorrThrInc;
                        return growGrass<DO_BOOTSTRAP>(unw, ccl, currcc, bsunw,
                                bsccl, labelmap, phase, tree, corr, corrthr,
                                length, width);
                    } else {
                        throw std::runtime_error("failed to unwrap tile at max "
...
template void ICU::growGrass<true>(float* unw, uint8_t* ccl, bool* currcc,
        float* bsunw, uint8_t* bsccl, LabelMap& labelmap, const float* phase,
        const bool* tree, const float* corr, float corrthr, const size_t length,
        const size_t width);

template void ICU::growGrass<false>(float* unw, uint8_t* ccl, bool* currcc,
        float* bsunw, uint8_t* bsccl, LabelMap& labelmap, const float* phase,
        const bool* tree, const float* corr, float corrthr, const size_t length,
        const size_t width);
