...
void _calcPhaseGrad(float* phasegradx, float* phasegrady,
        ...
        for (size_t i = WINSIZE / 2 + 1; i < width - WINSIZE / 2; ++i) {
            ...
            for (int jj = -WINSIZE / 2; jj <= WINSIZE / 2; ++jj) {
                for (int ii = -WINSIZE / 2; ii <= WINSIZE / 2; ++ii) {
                    float w = weights[(jj + WINSIZE / 2) * WINSIZE +
                                      (ii + WINSIZE / 2)];

                    std::complex<float> z_11 =
                            intf[(j + jj) * width + (i + ii)];
                    std::complex<float> z_10 =
                            intf[(j + jj) * width + (i + ii - 1)];
                    std::complex<float> z_01 =
                            intf[(j + jj - 1) * width + (i + ii)];
