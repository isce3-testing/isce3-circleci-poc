...
void permute(T* shuffled, const T* input, const size_t size, URNG& generator)
{
...
Twig::Twig(const size_t length, const size_t width) : _width(width)
{
...
const idx2_t& Twig::operator[](const size_t pos) const { return _nodes[pos]; }
...
void branchcutHoriz(
        bool* tree, size_t i1, size_t i2, size_t j, const size_t width)
{
    if (i1 > i2) {
        std::swap(i1, i2);
    }
...
void branchcutVert(
        bool* tree, size_t i, size_t j1, size_t j2, const size_t width)
{
    if (j1 > j2) {
        std::swap(j1, j2);
    }
...
void branchcutShallow(bool* tree, size_t i1, size_t j1, size_t i2, size_t j2,
        const size_t width)
{
    if (i1 > i2) {
        std::swap(i1, i2);
...
void branchcutSteep(bool* tree, size_t i1, size_t j1, size_t i2, size_t j2,
        const size_t width)
{
    if (j1 > j2) {
        std::swap(i1, i2);
...
void branchcut(bool* tree, size_t i1, size_t j1, size_t i2, size_t j2,
        const size_t width)
{
    ...
    if (dj == 0) {
        branchcutHoriz(tree, i1, i2, j1, width);
    } else if (di == 0) {
        branchcutVert(tree, i1, j1, j2, width);
    } else if (di >= dj) {
        branchcutShallow(tree, i1, j1, i2, j2, width);
    } else {
        branchcutSteep(tree, i1, j1, i2, j2, width);
    }
}

void growTwig(bool* tree, Twig& twig, bool* visited,
        const SearchTable& searchtable, const signed char* charge,
        const bool* neut, const idx2_t& root, const size_t length,
        const size_t width, const int maxbranchlen)
{
    // Reset twig.
    twig.clear();
    ...
    for (int l = 1; l <= maxbranchlen; ++l) {
        ...
        for (size_t t = 0; t < twig.size(); ++t) {
            ...
            for (size_t s = 0; s < nsearchpts; ++s) {
                ...
                // Check if search point is residue or neutron not already on
                // twig.
                ...
                if ((charge[inewnode] != 0 || neut[inewnode]) &&
                    ...
                    branchcut(tree, node[0], node[1], newnode[0], newnode[1],
                            width);
...
void ICU::growTrees(bool* tree, const signed char* charge, const bool* neut,
        const size_t length, const size_t width, const unsigned int seed)
{
    // Init branch cuts.
    const size_t tilesize = length * width;
    for (size_t i = 0; i < tilesize; ++i) {
...
#pragma omp parallel for shared(searchtable)
    for (int t = 0; t < _NumTrees; ++t) {
        ...
        for (int r = 0; r < nresid; ++r) {
            ...
            growTwig(tree, twig, visited, searchtable, charge, neut, root,
                    length, width, _MaxBranchLen);
        }
