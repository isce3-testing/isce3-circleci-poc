...
public:
    ...
    static std::string defaultDriver() { return "ENVI"; }
     ...
     * \param[in] access    Access mode
     */
    Dataset(const isce3::io::IDataSet& dataset,
     ...
     */
    Dataset(const std::string& path, int width, int length, int bands,
     ...
     */
    template<typename T>
    Dataset(T* data, int width, int length, int bands,
     ...
     * adjacent rows \param[in] bandstride Stride in bytes between the start of
     * adjacent bands
     */
    template<typename T>
    Dataset(const T* data, int width, int length, int bands,
            std::size_t colstride, std::size_t rowstride,
     ...
     * columns \param[in] rowstride Stride in bytes between the start of
     * adjacent rows \param[in] bandstride Stride in bytes between the start of
     * adjacent bands \param[in] access Access mode
     */
    template<typename T>
    Dataset(T* data, int width, int length, int bands, std::size_t colstride,
            std::size_t rowstride, std::size_t bandstride,
            GDALAccess access = GA_Update);
...
private:
    ...
    // XXX need to keep track of access mode ourselves rather than use
    // `_dataset->GetAccess()`
    // XXX GDAL silently ignores the requested access mode when using "MEM"
    // driver
    // XXX and sets access mode to GA_Update
