...
static std::string getOGRErrString(OGRErr status)
{
    switch (status) {
...
SpatialReference::SpatialReference(int epsg)
{
    ...
    if (status != OGRERR_NONE) {
        std::string errmsg = "failed to initialize spatial reference "
                             "from EPSG code '" +
                             std::to_string(epsg) + "' - " +
...
SpatialReference::SpatialReference(const std::string& wkt)
{
    ...
    if (status != OGRERR_NONE) {
        std::string errmsg =
                "failed to initialize spatial reference from WKT string - " +
                getOGRErrString(status);
        throw isce3::except::GDALError(ISCE_SRCINFO(), errmsg);
    }
    ...
    if (status != OGRERR_NONE) {
        throw isce3::except::GDALError(
                ISCE_SRCINFO(), "failed to identify EPSG authority code");
    }
}

SpatialReference::SpatialReference(const OGRSpatialReference& srs) : _srs(srs)
{
...
int SpatialReference::toEPSG() const
{
    ...
    if (!code) {
        throw isce3::except::GDALError(
                ISCE_SRCINFO(), "unable to fetch EPSG code");
    }
...
std::string SpatialReference::toWKT() const
{
    ...
    if (status != OGRERR_NONE) {
        std::string errmsg = "unable to export spatial reference to WKT - " +
                             getOGRErrString(status);
        throw isce3::except::GDALError(ISCE_SRCINFO(), errmsg);
    }
    std::string wkt = tmp;
