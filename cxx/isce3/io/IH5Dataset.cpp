...
IH5RasterBand::IH5RasterBand(IH5Dataset* ds, int bandIn, GDALDataType eTypeIn)
    : GDALPamRasterBand(FALSE), bNoDataSet(FALSE), dfNoData(0.0)
{
...
IH5RasterBand::~IH5RasterBand() { IH5RasterBand::FlushCache(); }

/************************************************************************/
/*                             IReadBlock()                             */
/************************************************************************/

CPLErr IH5RasterBand::IReadBlock(int nBlockXOff, int nBlockYOff, void* pImage)
{
    ...
    counts[offset] =
            std::min(nBlockYSize, poGDS->GetRasterYSize() - starts[offset]);
    counts[offset + 1] =
            std::min(nBlockXSize, poGDS->GetRasterXSize() - starts[offset + 1]);
    ...
    H5::DataSpace dspace =
            poGDS->_dataset->getDataSpace(starts, counts, nullptr);
...
CPLErr IH5RasterBand::IWriteBlock(int nBlockXOff, int nBlockYOff, void* pImage)
{
    ...
    poGDS->_dataset->H5::DataSet::write(pImage, writeType, mspace, dspace);
    if (!H5::IdComponent::isValid(poGDS->_dataset->getId())) {
        CPLError(CE_Failure, CPLE_AppDefined,
...
void* IH5Dataset::GetInternalHandle(const char*) { return _dataset; }
...
const GDAL_GCP* IH5Dataset::GetGCPs() { return pasGCPList; }
...
CPLErr IH5Dataset::populateFromDataset()
{
    ...
    if (!H5::IdComponent::isValid(_dataset->getId())) {
        CPLError(CE_Failure, CPLE_AppDefined, "IH5Dataset->getRank() failure");
        return CE_Failure;
    }
    if ((ndims != 2) && (ndims != 3)) {
        CPLError(CE_Failure, CPLE_AppDefined,
    ...
    if (ndims == 2) {
        std::stringstream ss;
        ss << _dataset->getId() << ": " << dimensions[axisOffset] << " L x "
           << dimensions[axisOffset + 1] << " P";
        CPLDebug("GDAL_IH5", "%s", ss.str().c_str());
    } else {
        std::stringstream ss;
        ss << _dataset->getId() << ": " << nBands << " H x "
           << dimensions[axisOffset] << " L x " << dimensions[axisOffset + 1]
           << " P";
        CPLDebug("GDAL_IH5", "%s", ss.str().c_str());
    }
    ...
    chunks[0] = 1;
    chunks[axisOffset] =
            (chunkSize[axisOffset] == 0) ? 1 : chunkSize[axisOffset];
    chunks[axisOffset + 1] = (chunkSize[axisOffset] == 0)
                                     ? nRasterXSize
                                     : chunkSize[axisOffset + 1];
    ...
    if (nativeType.getClass() != H5T_COMPOUND) {
        ...
        if (compoundType.getNmembers() == 2) {
            ...
            if (firstType == secondType) {
                ...
                f_DataType_setId(
                        &nativeFirstType, IH5GetNativeType(firstType).getId());
