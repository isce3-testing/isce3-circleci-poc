...
std::vector<std::complex<float>> formLinearChirp(double chirprate,
        double duration, double samplerate, double centerfreq, double amplitude,
        double phi)
{
    // sanity checks
    if (duration <= 0.) {
        throw isce3::except::DomainError(
                ISCE_SRCINFO(), "chirp duration must be > 0");
    }
    if (samplerate <= 0.) {
        throw isce3::except::DomainError(
                ISCE_SRCINFO(), "sampling rate must be > 0");
    }
    if (amplitude <= 0.) {
        throw isce3::except::DomainError(
                ISCE_SRCINFO(), "amplitude must be > 0");
    }
    ...
    if (d_size > d_maxsize) {
        throw isce3::except::OverflowError(
                ISCE_SRCINFO(), "chirp size exceeds max int value");
    }
    ...
    for (int i = 0; i < size; ++i) {
        ...
        double phase =
                phi + 2. * M_PI * (centerfreq + 0.5 * chirprate * tau) * tau;
