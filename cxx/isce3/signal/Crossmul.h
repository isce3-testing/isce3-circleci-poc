...
/** \brief Intereferogram generation by cross-multiplication of reference and
 * secondary SLCs.
 *
...
public:
    ...
    */
    ...
    void crossmul(isce3::io::Raster& referenceSLC,
            isce3::io::Raster& secondarySLC, isce3::io::Raster& rngOffset,
            isce3::io::Raster& interferogram, isce3::io::Raster& coherence);
    ...
    void lookdownShiftImpact(size_t oversample, size_t fft_size,
            size_t blockRows, std::valarray<std::complex<float>>& shiftImpact);
    ...
    void rangeCommonBandFilter(std::valarray<std::complex<float>>& refSlc,
            ...
            isce3::signal::Filter<float>& rngFilter, size_t blockRows,
            size_t ncols);

    /** Set doppler LUTs for reference and secondary SLCs*/
    inline void doppler(isce3::core::LUT1d<double>, isce3::core::LUT1d<double>);

    /** Set dopplers LUT for reference SLC */
    inline void refDoppler(isce3::core::LUT1d<double> refDopp)
    {
        _refDoppler = refDopp;
    }

    /** Get doppler LUT for reference SLC */
    inline const isce3::core::LUT1d<double>& refDoppler() const
    {
        return _refDoppler;
    }

    /** Set dopplers LUT for secondary SLC */
    inline void secDoppler(isce3::core::LUT1d<double> secDopp)
    {
        _secDoppler = secDopp;
    }

    /** Get doppler LUT for secondary SLC */
    inline const isce3::core::LUT1d<double>& secDoppler() const
    {
        return _secDoppler;
    }
    ...
    inline void rangeSamplingFrequency(double rgSamplingFreq)
    {
        _rangeSamplingFrequency = rgSamplingFreq;
    }

    /** Get range sampling frequency  */
    inline double rangeSamplingFrequency() const
    {
        return _rangeSamplingFrequency;
    }
    ...
    inline void rangePixelSpacing(double rgPxlSpacing)
    {
        _rangePixelSpacing = rgPxlSpacing;
    }
    ...
    /** Compute the avergae frequency shift in range direction between two
     * SLCs*/
    inline void rangeFrequencyShift(
            std::valarray<std::complex<float>>& refAvgSpectrum,
            std::valarray<std::complex<float>>& secAvgSpectrum,
            std::valarray<double>& rangeFrequencies, size_t blockRowsData,
            size_t fft_size, double& frequencyShift);

    /** estimate the index of the maximum of a vector of data */
    inline void getPeakIndex(std::valarray<float> data, size_t& peakIndex);
