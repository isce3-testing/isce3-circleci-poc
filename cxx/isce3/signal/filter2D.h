 ...
 * \param[in] kernel_columns 1-D kernel in columns direction
 * \param[in] kernel_rows 1-D kernel in rows direction
 * \param[in] do_decimate flag to indicate if the output data will be decimated
 * proportional to the kernel size \param[in] mask flag to indicate if the
 * output data will be masked before filtering \param[in] block_rows number of
 * lines (rows) per block
 * */
template<typename T>
void filter2D(isce3::io::Raster& output_raster, isce3::io::Raster& input_raster,
        isce3::io::Raster& mask_raster,
