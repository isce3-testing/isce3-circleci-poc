...
void isce3::signal::decimate(std::valarray<T>& out, const std::valarray<T>& in,
        ...
        size_t ncols_decimated, size_t rows_decimation, size_t cols_decimation,
        size_t rows_offset, size_t cols_offset)
{
    ...
    if (ncols_decimated != width_decimated) {
        throw isce3::except::DomainError(ISCE_SRCINFO(),
                "The input number of columns in the decimated "
                "data does not seem correct");
    }
    if (nrows_decimated != length_decimated) {
        throw isce3::except::DomainError(ISCE_SRCINFO(),
                "The input number of rows in the decimated "
                "data does not seem correct");
    }
...
#define SPECIALIZE_DECIMATE(T)                                                 \
    template void isce3::signal::decimate(std::valarray<T>& out,               \
            const std::valarray<T>& in, size_t nrows, size_t ncols,            \
            size_t nrows_decimated, size_t ncols_decimated,                    \
