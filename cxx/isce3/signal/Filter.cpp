...
template<class T>
void isce3::signal::Filter<T>::initiateRangeFilter(
        std::valarray<std::complex<T>>& signal,
        std::valarray<std::complex<T>>& spectrum, size_t ncols, size_t nrows)
{
 ...
 */
template<class T>
void isce3::signal::Filter<T>::initiateAzimuthFilter(
        std::valarray<std::complex<T>>& signal,
        std::valarray<std::complex<T>>& spectrum, size_t ncols, size_t nrows)
{
 ...
 * @param[in] subBandCenterFrequencies a vector of center frequencies for each
 * band
 ...
 */
template<class T>
void isce3::signal::Filter<T>::constructRangeBandpassFilter(
        ...
        std::string filterType)
{
    constructRangeBandpassFilter(rangeSamplingFrequency,
            subBandCenterFrequencies, subBandBandwidths, ncols, nrows,
            filterType);
        ...
        std::string filterType)
{
    ...
    if (filterType == "boxcar") {
        constructRangeBandpassBoxcar(subBandCenterFrequencies,
                subBandBandwidths, dt, fft_size, _filter1D);

    } else if (filterType == "cosine") {
        double beta = 0.25;
        constructRangeBandpassCosine(subBandCenterFrequencies,
                subBandBandwidths, dt, frequency, beta, _filter1D);

    } else {
        std::cout << filterType << " filter has not been implemented"
                  << std::endl;
    }
 ...
 * @param[out] _filter1D one dimensional boxcar bandpass filter in frequency
 * domain
 */
template<class T>
void isce3::signal::Filter<T>::constructRangeBandpassBoxcar(
        std::valarray<double> subBandCenterFrequencies,
        ...
        std::valarray<std::complex<T>>& _filter1D)
{
    ...
    for (size_t i = 0; i < subBandCenterFrequencies.size(); ++i) {
        ...
        std::cout << "fL: " << fL << " , fH: " << fH << " indL: " << indL
                  << " , indH: " << indH << std::endl;
        if (fL < 0 && fH >= 0) {
 ...
 */
template<class T>
void isce3::signal::Filter<T>::constructRangeBandpassCosine(
        std::valarray<double> subBandCenterFrequencies,
        std::valarray<double> subBandBandwidths, double,
    ...
    for (size_t i = 0; i < subBandCenterFrequencies.size(); ++i) {
        ...
        for (size_t i = 0; i < frequency.size(); ++i) {
            ...
            if (freq <= (0.5 * bandwidth - df)) {
                ...
                // Transition region
            } else if (freq > (0.5 * bandwidth - df) &&
                       freq <= (0.5 * bandwidth + df)) {
                _filter1D[i] = std::complex<T>(
                        norm * 0.5 *
 ...
 */
template<class T>
void isce3::signal::Filter<T>::constructAzimuthCommonbandFilter(
        ...
        for (size_t i = 0; i < frequency.size(); ++i) {
            ...
            if (freq <= (0.5 * bandwidth - df)) {
                ...
                // Transition region
                       ...
                       freq <= (0.5 * bandwidth + df)) {
                _filter[i * ncols + j] = std::complex<T>(
                        norm * 0.5 *
                                (1.0 + std::cos(M_PI / (bandwidth * beta) *
                                                (freq - 0.5 * (1.0 - beta) *
                                                                bandwidth))),
 ...
 */
void isce3::signal::fftfreq(double dt, std::valarray<double>& freq)
{
 ...
 */
template<class T>
void isce3::signal::Filter<T>::indexOfFrequency(
        double dt, int N, double f, int& n)
...
template<class T>
void isce3::signal::Filter<T>::writeFilter(size_t ncols, size_t nrows)
{
    isce3::io::Raster filterRaster(
            "filter.bin", ncols, nrows, 1, GDT_CFloat32, "ENVI");
