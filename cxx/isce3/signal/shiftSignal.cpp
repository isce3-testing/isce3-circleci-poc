...
void isce3::signal::shiftSignal(std::valarray<T>& data,
        std::valarray<T>& dataShifted, std::valarray<std::complex<U>>& spectrum,
        size_t ncols, size_t nrows, const double shiftX, const double shiftY,
        isce3::signal::Signal<U>& sigObj)
{
    ...
    if (not isce3::core::compareFloatingPoint(shiftX, 0.0) ||
        ...
        shiftSignal(data, dataShifted, spectrum, phaseRamp, sigObj);
 ...
 */
template<typename T>
void isce3::signal::frequencyResponseRange(size_t ncols, size_t nrows,
        const double shift, std::valarray<std::complex<T>>& shiftImpact)
{
    ...
    // The imapct is the same for each range line. Therefore copying the line
    // for the block
 ...
 */
template<typename T>
void isce3::signal::frequencyResponseAzimuth(size_t ncols, size_t nrows,
        const double shift, std::valarray<std::complex<T>>& shiftImpact)
{
    ...
    // The imapct is the same for each column. Therefore copying for all columns
    // of the block
...
template void isce3::signal::shiftSignal(std::valarray<float>& data,
        std::valarray<float>& dataShifted,
        std::valarray<std::complex<float>>& spectrum, size_t ncols,
        size_t nrows, const double shiftX, const double shiftY,
        isce3::signal::Signal<float>& sigObj);

template void isce3::signal::shiftSignal(std::valarray<double>& data,
        std::valarray<double>& dataShifted,
        std::valarray<std::complex<double>>& spectrum, size_t ncols,
        size_t nrows, const double shiftX, const double shiftY,
        isce3::signal::Signal<double>& sigObj);

template void isce3::signal::shiftSignal(
        std::valarray<std::complex<float>>& data,
...
template void isce3::signal::frequencyResponseRange(size_t fft_size,
        size_t blockRows, const double shift,
...
template void isce3::signal::frequencyResponseAzimuth(size_t fft_size,
        size_t blockRows, const double shift,
        std::valarray<std::complex<float>>& shiftImpact);
