...
template<class T>
isce3::signal::Signal<T>::Signal() : pimpl(new impl, [](impl* p) { delete p; })
{}
 ...
 *  @param[in] rank rank of the transform (1: for one dimensional and 2: for two
 * dimensional transform)
 *  @param[in] size size of each transform (ncols: for range FFT, nrows: for
 * azimuth FFT)
 *  @param[in] howmany number of FFT transforms for a block of data (nrows: for
 * range FFT, ncols: for azimuth FFT)
 *  @param[in] inembed
 *  @param[in] istride
 *  @param[in] idist
 *  @param[in] onembed
 ...
 */
template<class T>
void isce3::signal::Signal<T>::fftPlanForward(
        std::valarray<std::complex<T>>& input,
        std::valarray<std::complex<T>>& output, int rank, int* n, int howmany,
        int* inembed, int istride, int idist, int* onembed, int ostride,
        int odist, int sign)
{

    fftPlanForward(&input[0], &output[0], rank, n, howmany, inembed, istride,
            idist, onembed, ostride, odist, sign);
}
...
void isce3::signal::Signal<T>::fftPlanForward(std::complex<T>* input,
        std::complex<T>* output, int rank, int* n, int howmany, int* inembed,
        int istride, int idist, int* onembed, int ostride, int odist, int sign)
{

    _fwd_configure(
            rank, n, howmany, inembed, istride, idist, onembed, ostride, odist);

    pimpl->_plan_fwd = fftw3cxx::plan<T>::plan_many_dft(rank, n, howmany, input,
            inembed, istride, idist, output, onembed, ostride, odist, sign,
            FFTW_ESTIMATE);
}
 ...
 */
template<class T>
void isce3::signal::Signal<T>::fftPlanForward(T* input, std::complex<T>* output,
        ...
        int* onembed, int ostride, int odist)
{
    ...
    pimpl->_plan_fwd = fftw3cxx::plan<T>::plan_many_dft_r2c(rank, n, howmany,
            input, inembed, istride, idist, output, onembed, ostride, odist,
            FFTW_ESTIMATE);
}
 ...
 */
template<class T>
void isce3::signal::Signal<T>::fftPlanBackward(
        std::valarray<std::complex<T>>& input,
        std::valarray<std::complex<T>>& output, int rank, int* n, int howmany,
        int* inembed, int istride, int idist, int* onembed, int ostride,
        int odist, int sign)
{
    ...
    fftPlanBackward(&input[0], &output[0], rank, n, howmany, inembed, istride,
            idist, onembed, ostride, odist, sign);
}
        ...
        int istride, int idist, int* onembed, int ostride, int odist, int sign)
{
    ...
    pimpl->_plan_inv = fftw3cxx::plan<T>::plan_many_dft(rank, n, howmany, input,
            inembed, istride, idist, output, onembed, ostride, odist, sign,
            FFTW_ESTIMATE);
}
...
template<class T>
void isce3::signal::Signal<T>::fftPlanBackward(std::complex<T>* input,
        T* output, int rank, int* n, int howmany, int* inembed, int istride,
 ...
 */
template<class T>
void isce3::signal::Signal<T>::forward(
        std::complex<T>* input, std::complex<T>* output)
{
 ...
 */
template<class T>
void isce3::signal::Signal<T>::forward(T* input, std::complex<T>* output)
{
...
/** unnormalized inverse transform.*/
template<class T>
void isce3::signal::Signal<T>::inverse(
        std::complex<T>* input, std::complex<T>* output)
{
...
/** unnormalized inverse transform.*/
template<class T>
void isce3::signal::Signal<T>::inverse(std::complex<T>* input, T* output)
{
 ...
 */
template<class T>
void isce3::signal::Signal<T>::forwardRangeFFT(
        std::valarray<std::complex<T>>& signal,
        std::valarray<std::complex<T>>& spectrum, int ncolumns, int nrows)
{

    forwardRangeFFT(&signal[0], &spectrum[0], ncolumns, nrows);
}
...
void isce3::signal::Signal<T>::forwardRangeFFT(std::complex<T>* signal,
        std::complex<T>* spectrum, int ncolumns, int nrows)
{

    _fwd_configureRangeFFT(ncolumns, nrows);

    fftPlanForward(signal, spectrum, _fwd_rank, _fwd_n, _fwd_howmany,
            _fwd_inembed, _fwd_istride, _fwd_idist, _fwd_onembed, _fwd_ostride,
            _fwd_odist, FFTW_FORWARD);
}
 ...
 */
template<class T>
void isce3::signal::Signal<T>::forwardAzimuthFFT(
        std::valarray<std::complex<T>>& signal,
        std::valarray<std::complex<T>>& spectrum, int ncolumns, int nrows)
{

    forwardAzimuthFFT(&signal[0], &spectrum[0], ncolumns, nrows);
}
 ...
 */
template<class T>
void isce3::signal::Signal<T>::forward2DFFT(
        std::valarray<std::complex<T>>& signal,
        std::valarray<std::complex<T>>& spectrum, int ncolumns, int nrows)
{

    forward2DFFT(&signal[0], &spectrum[0], ncolumns, nrows, ncolumns, nrows);
}
 ...
 *  @param[in] oncolumns number of columns of the block of data in output
 * container
...
{
    forward2DFFT(
            &signal[0], &spectrum[0], incolumns, inrows, oncolumns, onrows);
}
 ...
 *  @param[in] signal input block of data
 *  @param[out] spectrum output block of spectrum
 *  @param[in] incolumns number of columns of the block of data in input
 * container
...
template<class T>
void isce3::signal::Signal<T>::forward2DFFT(std::complex<T>* signal,
        std::complex<T>* spectrum, int incolumns, int inrows, int oncolumns,
        int onrows)
{
...
void isce3::signal::Signal<T>::forward2DFFT(std::valarray<T>& signal,
        std::valarray<std::complex<T>>& spectrum, int ncolumns, int nrows)
{
    forward2DFFT(&signal[0], &spectrum[0], ncolumns, nrows);
}
...
void isce3::signal::Signal<T>::inverseRangeFFT(std::complex<T>* spectrum,
        std::complex<T>* signal, int ncolumns, int nrows)
{
    _rev_configureRangeFFT(ncolumns, nrows);

    fftPlanBackward(spectrum, signal, _rev_rank, _rev_n, _rev_howmany,
            _rev_inembed, _rev_istride, _rev_idist, _rev_onembed, _rev_ostride,
            _rev_odist, FFTW_BACKWARD);
}
 ...
 */
template<class T>
void isce3::signal::Signal<T>::inverseRangeFFT(
...
template<class T>
void isce3::signal::Signal<T>::inverseRangeFFT(
        std::valarray<std::complex<T>>& spectrum, std::valarray<T>& signal,
 ...
 */
template<class T>
void isce3::signal::Signal<T>::inverseAzimuthFFT(
        std::valarray<std::complex<T>>& spectrum,
        std::valarray<std::complex<T>>& signal, int ncolumns, int nrows)
{
 ...
 */
template<class T>
void isce3::signal::Signal<T>::inverse2DFFT(
        std::valarray<std::complex<T>>& spectrum,
        std::valarray<std::complex<T>>& signal, int ncolumns, int nrows)
{
...
void isce3::signal::Signal<T>::upsample(std::valarray<std::complex<T>>& signal,
        std::valarray<std::complex<T>>& signalUpsampled, int rows, int fft_size,
        int upsampleFactor)
{
    ...
    upsample(signal, signalUpsampled, rows, fft_size, upsampleFactor,
            shiftImpact);
}
 ...
 *   @param[in] fft_size number of columns of the block of input data
 *   @param[in] upsampleFactor upsampling factor
 *   @param[out] shiftImpact a linear phase term equivalent to a constant shift
 * in time domain
 */
template<class T>
void isce3::signal::Signal<T>::upsample(std::valarray<std::complex<T>>& signal,
        ...
        int upsampleFactor, std::valarray<std::complex<T>> shiftImpact)
{
    ...
    // spectrum /= fft_size;
    // shift the spectrum
    //  The spectrum has values from begining to fft_size index for each line.
    //  We want to put the spectrum in correct ouput locations such that the
    //  spectrum of the upsampled data has values from 0 to fft_size/2 and from
...
#pragma omp parallel for
    for (size_t column = 0; column < (fft_size + 1) / 2; ++column)
        spectrumShifted[std::slice(column, rows, columns)] =
                spectrum[std::slice(column, rows, fft_size)];

#pragma omp parallel for
    for (size_t i = 0; i < fft_size / 2; ++i) {
        size_t j = upsampleFactor * fft_size - fft_size / 2 + i;
        spectrumShifted[std::slice(j, rows, columns)] =
                spectrum[std::slice(i + fft_size / 2, rows, fft_size)];
    }
        ...
        const isce3::core::EArray2D<std::complex<T>>& shiftImpact)
{
    ...
    // inverse fft to get the upsampled signal
    pimpl->_plan_inv.execute_dft(
            spectrumShifted.data(), signalUpsampled.data());
 ...
 *   @param[in] upsampleFactor upsampling factor
 *
 *   When doing out-of-place upsampling, i.e., when the container of the input
 * to upsample is different to the one that will contain the upsampled data, the
 * FFT forward plan must be set to out-of-place (with the input and output
 * containers) and the FFT reverse plan to in-place (with the output container).
 *   In both case (in-place or out-of-place 2D upsampling), it is the user
 * responsability to provide an output container that is padded (for in-place)
 * or filled (for out-of-place) with zeros.
 */
template<class T>
void isce3::signal::Signal<T>::upsample2D(
        std::valarray<std::complex<T>>& signal,
        std::valarray<std::complex<T>>& signalUpsampled, int oversampleFactor)
{

    upsample2D(&signal[0], &signalUpsampled[0], oversampleFactor, nullptr);
}
...
void isce3::signal::Signal<T>::upsample2D(
        std::valarray<std::complex<T>>& signal,
        std::valarray<std::complex<T>>& signalUpsampled, int upsampleFactor,
        std::valarray<std::complex<T>>& shiftImpact)
{

    upsample2D(
            &signal[0], &signalUpsampled[0], upsampleFactor, &shiftImpact[0]);
}
...
void isce3::signal::Signal<T>::upsample2D(std::complex<T>* signal,
        std::complex<T>* signalUpsampled, int upsampleFactor)
{
    upsample2D(signal, signalUpsampled, upsampleFactor, nullptr);
}
        ...
        std::complex<T>* shiftImpact)
{
    ...
    if (upsampleFactor * _fwd_n[0] != _rev_n[0] ||
            upsampleFactor * _fwd_n[1] != _rev_n[1]) {
        std::cout << "Discrepency between upsampling factor and reverse FFT "
                     "rank values"
                  << std::endl;
        return;
    }
    ...
    // [1] Forward fft
    // If doing out-of-place transform, i.e., input signal is a different
    // container than output container, the forward FFT is done out-of-place and
    // the reverse FFT will be done in-place.
...
// [2] Spectrum shuffling - Moving the 4 quarts to the corners of the output
// (larger) container. In-place operation
#pragma omp parallel for
    for (size_t p = 0; p < _fwd_howmany; p++) {
        ...
        for (size_t r = 0; r < rows1; r++) {
            loc = &signalUpsampled[p * _fwd_odist + r * _fwd_onembed[0] +
                                   cols1];
            std::swap_ranges(loc, loc + cols2,
                    &signalUpsampled[p * _rev_idist + r * _rev_onembed[0] +
                                     _rev_n[0] - cols2]);
        }

        for (size_t r = 0; r < rows2; r++) {
            loc = &signalUpsampled[p * _fwd_odist +
                                   (r + rows1) * _fwd_onembed[0]];
            std::swap_ranges(loc, loc + cols1,
                    &signalUpsampled[p * _rev_idist + (_rev_n[1] - rows2 + r) *
                                                              _rev_onembed[0]]);
            loc = &signalUpsampled[p * _fwd_odist +
                                   (r + rows1) * _fwd_onembed[0] + cols1];
            std::swap_ranges(loc, loc + cols2,
                    &signalUpsampled[p * _rev_idist +
                                     (_rev_n[1] - rows2 + 1 + r) *
                                             _rev_onembed[0] -
                                     cols2]);
        }
        ...
        for (size_t p = 0; p < _fwd_howmany; p++)
            for (size_t j = 0; j < _rev_n[1]; j++)
                for (size_t i = 0; i < _rev_n[0]; i++)
                    signalUpsampled[p * _rev_idist + j * _rev_onembed[0] + i] *=
                            shiftImpact[j * _rev_n[0] + i];
    }
