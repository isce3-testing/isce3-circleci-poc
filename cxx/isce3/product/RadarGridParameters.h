...
public:
     ...
     */
    RadarGridParameters(
            const isce3::product::Swath& swath, isce3::core::LookSide lookSide);
    ...
    inline RadarGridParameters(double sensingStart, double wavelength,
            double prf, double startingRange, double rangePixelSpacing,
     ...
     *
     * Other dependent parameters like sensingStart are not modified. Use with
     * caution.*/
    ...
    /** Get pulse repetition frequency in Hz - inverse of azimuth time
     * interval*/
    inline double prf() const { return _prf; }

    /** Set pulse repetition frequency in Hz - inverse of azimuth time
     * interval*/
    ...
    inline double endingRange() const { return slantRange(_rwidth - 1.0); }
    ...
    /** Crop/ Expand while keeping the spacing the same with top left offset and
     * size */
    inline RadarGridParameters offsetAndResize(
            double yoff, double xoff, size_t ysize, size_t xsize) const
    {
        return RadarGridParameters(sensingTime(yoff), wavelength(), prf(),
                slantRange(xoff), rangePixelSpacing(), lookSide(), ysize, xsize,
                refEpoch());
    }
    ...
    inline RadarGridParameters multilook(size_t azlooks, size_t rglooks) const
    {
        ...
        return RadarGridParameters(sensingTime(0.5 * (azlooks - 1)),
                wavelength(), prf() / (1.0 * azlooks),
                slantRange(0.5 * (rglooks - 1)), rangePixelSpacing() * rglooks,
                lookSide(), length() / azlooks, width() / rglooks, refEpoch());
    }

    /** Upsample */
    inline RadarGridParameters upsample(
            size_t az_upsampling_factor, size_t rg_upsampling_factor) const
    {
        // Check for number of points on edge
        ...
        return RadarGridParameters(sensingTime(0.0), wavelength(),
                prf() * az_upsampling_factor, slantRange(0.0),
                rangePixelSpacing() / (1.0 * rg_upsampling_factor), lookSide(),
                length() * az_upsampling_factor, width() * rg_upsampling_factor,
                refEpoch());
    }
...
isce3::product::RadarGridParameters::RadarGridParameters(
    ...
    : _lookSide(meta.lookSide),
      ...
      _wavelength(meta.radarWavelength), _prf(meta.prf),
      _startingRange(meta.rangeFirstSample),
      _rangePixelSpacing(meta.slantRangePixelSpacing), _rlength(meta.length),
      _rwidth(meta.width), _refEpoch(refEpoch)
{
    validate();
}
...
isce3::product::RadarGridParameters::RadarGridParameters(
        const RadarGridParameters& rgparams)
    : _lookSide(rgparams.lookSide()), _sensingStart(rgparams.sensingStart()),
      _wavelength(rgparams.wavelength()), _prf(rgparams.prf()),
      _startingRange(rgparams.startingRange()),
      _rangePixelSpacing(rgparams.rangePixelSpacing()),
      _rlength(rgparams.length()), _rwidth(rgparams.width()),
...
isce3::product::RadarGridParameters::operator=(
        const RadarGridParameters& rgparams)
{
    ...
    _startingRange = rgparams.startingRange();
    _rangePixelSpacing = rgparams.rangePixelSpacing();
    _lookSide = rgparams.lookSide(), _rlength = rgparams.length();
...
isce3::product::RadarGridParameters::RadarGridParameters(double sensingStart,
        double wavelength, double prf, double startingRange,
        double rangePixelSpacing, isce3::core::LookSide lookSide, size_t length,
        size_t width, isce3::core::DateTime refEpoch)
    : _lookSide(lookSide), _sensingStart(sensingStart), _wavelength(wavelength),
      _prf(prf), _startingRange(startingRange),
      _rangePixelSpacing(rangePixelSpacing), _rlength(length), _rwidth(width),
      _refEpoch(refEpoch)
{
    validate();
}
...
void isce3::product::RadarGridParameters::validate() const
{
