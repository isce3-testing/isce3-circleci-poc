...
public:
     ...
     * \param[in] azimuth_time          Time of each azimuth line relative to
     * reference epoch (s). Timepoints may be non-uniformly sampled but must be
     *                                  monotonically increasing.
     * \param[in] range_window_start    Time of first range sample relative to
     * Tx pulse start (s) \param[in] range_sampling_rate   Range sampling rate
     * (Hz) \param[in] look_side             Radar look side
     */
    RSD(const isce3::io::gdal::Raster& signal_data,
            const isce3::core::DateTime& reference_epoch,
            const std::vector<double>& azimuth_time, double range_window_start,
            double range_sampling_rate, isce3::core::LookSide look_side);
    ...
    {
        return 0.5 * azimuthStartTime() + 0.5 * azimuthEndTime();
    }
    ...
    void readLines(
            std::complex<float>* dst, int first_line, int num_lines) const;
