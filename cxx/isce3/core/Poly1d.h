 ...
 * \f[
 *     f\left( x \right) = \sum_{i=0}^{N} a_i \cdot \left( \frac{x-\mu}{\sigma}
 * \right)^i \f]
 *
 * where \f$a_i\f$ represents the coefficients, \f$\mu\f$ represents the mean
 * and \f$\sigma\f$ represents the norm*/
...
public:
    ...
    Poly1d(int ord, double mn, double nm)
        : order(ord), mean(mn), norm(nm), coeffs(ord + 1)
    {}
    ...
    Poly1d(const Poly1d& p)
        : order(p.order), mean(p.mean), norm(p.norm), coeffs(p.coeffs)
    {}
...
isce3::core::Poly1d& isce3::core::Poly1d::operator=(const Poly1d& rhs)
{
    order = rhs.order;
    mean = rhs.mean;
    norm = rhs.norm;
...
void isce3::core::Poly1d::setCoeff(int idx, double val)
{
    if ((idx < 0) || (idx > order)) {
        std::string errstr = "Poly1d::setCoeff - Trying to set coefficient " +
                             std::to_string(idx + 1) + " out of " +
...
double isce3::core::Poly1d::getCoeff(int idx) const
{
    if ((idx < 0) || (idx > order)) {
