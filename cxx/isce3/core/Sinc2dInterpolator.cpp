...
template<typename U>
isce3::core::Sinc2dInterpolator<U>::Sinc2dInterpolator(int sincLen, int sincSub)
    : isce3::core::Interpolator<U>(SINC_METHOD), _kernelLength {sincSub},
      _kernelWidth {sincLen}, _sincHalf {sincLen / 2}
{
    ...
    for (size_t i = 0; i < sincSub; ++i) {
        ...
        // Normalize the filter coefficients and copy to transposed member
        // kernel
        for (size_t j = 0; j < sincLen; ++j) {
...
template<class U>
U isce3::core::Sinc2dInterpolator<U>::interp_impl(
...
template<class U>
U isce3::core::Sinc2dInterpolator<U>::_sinc_eval_2d(
        const Map& arrin, int intpx, int intpy, double frpx, double frpy) const
{
    ...
    int ifracx =
            std::min(std::max(0, int(frpx * _kernelLength)), _kernelLength - 1);
    int ifracy =
            std::min(std::max(0, int(frpy * _kernelLength)), _kernelLength - 1);
...
template<class U>
void isce3::core::Sinc2dInterpolator<U>::_sinc_coef(double beta, double,
        int decfactor, double pedestal, int weight,
