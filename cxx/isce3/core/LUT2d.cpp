...
template<typename T>
isce3::core::LUT2d<T>::LUT2d(double xstart, double ystart, double dx, double dy,
...
isce3::core::LUT2d<T>::LUT2d(const std::valarray<double>& xcoord,
        const std::valarray<double>& ycoord, const isce3::core::Matrix<T>& data,
        isce3::core::dataInterpMethod method, bool boundsError)
    : _haveData(true), _boundsError(boundsError), _refValue(data(0, 0))
{
    // Set the data
...
template<typename T>
T isce3::core::LUT2d<T>::eval(double y, double x) const
{
    /*
    ...
    if (_boundsError && not contains(y, x)) {
        pyre::journal::error_t errorChannel("isce.core.LUT2d");
        errorChannel << "Out of bounds LUT2d evaluation at " << y << " " << x
                     << pyre::journal::newline << " - bounds are " << _ystart
                     << " " << _ystart + _dy * _data.length() << " " << _xstart
                     << " " << _xstart + _dx * _data.width()
                     << pyre::journal::endl;
    }
...
template<typename T>
Eigen::Matrix<T, Eigen::Dynamic, 1> isce3::core::LUT2d<T>::eval(
...
template<typename T>
void isce3::core::LUT2d<T>::_setInterpolator(
        isce3::core::dataInterpMethod method)
{
    ...
    if (method == isce3::core::BIQUINTIC_METHOD) {
        _interp = isce3::core::createInterpolator<T>(
                isce3::core::BIQUINTIC_METHOD, 6);

        // If sinc, set the window sizes
    } else if (method == isce3::core::SINC_METHOD) {
        _interp = isce3::core::createInterpolator<T>(isce3::core::SINC_METHOD,
