 ...
 *
 * Only the semi-major axis and the eccentricity^2 parameters are stored. All
 * other quantities are derived on the fly*/
...
public:
    ...
    void getImagingAnglesAtPlatform(const cartesian_t& pos,
 ...
 *
 * See <a href="https://en.wikipedia.org/wiki/Earth_radius#Prime_vertical">Prime
 * vertical radius</a>*/
CUDA_HOSTDEV
 ...
 *
 * See <a
 * href="https://en.wikipedia.org/wiki/Earth_radius#Meridional">Meridional
 * radius</a> */
CUDA_HOSTDEV
double isce3::core::Ellipsoid::rNorth(double lat) const
{
    ...
    return (_a * (1.0 - _e2)) /
           std::pow((1.0 - (_e2 * std::pow(std::sin(lat), 2))), 1.5);
}
 ...
 *  See <a
 * href="https://en.wikipedia.org/wiki/Earth_radius#Directional">Directional
 * Radius</a> */
CUDA_HOSTDEV
 ...
 *  @param[in] lat Latitude in radians
 *  @param[out] vec Unit vector of normal pointing outwards in ECEF cartesian
 * coordinates
 *
 *  See <a href="https://en.wikipedia.org/wiki/N-vector">N-vector</a> */
CUDA_HOSTDEV
void isce3::core::Ellipsoid::nVector(
        double lon, double lat, cartesian_t& vec) const
{
 ...
 *
 *  See <a
 * href="https://en.wikipedia.org/wiki/Ellipsoid#Parametric_representation">parametric
 * representation of ellipsoid</a>*/
CUDA_HOSTDEV
void isce3::core::Ellipsoid::xyzOnEllipse(
        double lon, double lat, cartesian_t& vec) const
{
    nVector(lon, lat, vec);
    vec[0] *= _a;
...
CUDA_HOSTDEV inline void isce3::core::Ellipsoid::lonLatToXyz(
        const cartesian_t& llh, cartesian_t& xyz) const
{
    /*
...
CUDA_HOSTDEV inline void isce3::core::Ellipsoid::xyzToLonLat(
        const cartesian_t& xyz, cartesian_t& llh) const
{
    /*
     * Given a geocentric XYZ, produces a lat, lon, and height above the
     * reference ellipsoid. VERMEILLE IMPLEMENTATION
     */
    ...
    double d = (k * std::sqrt(std::pow(xyz[0], 2) + std::pow(xyz[1], 2))) /
               (k + _e2);
