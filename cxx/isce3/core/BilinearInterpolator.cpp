...
template<class U>
U isce3::core::BilinearInterpolator<U>::interp_impl(
        double x, double y, const Map& z) const
{
    ...
    // Future work:
    // static_cast<> was applied below bc the compiler complained about things
    // (complex dtype probably), but the complex operators are overloaded to
    // work with non-complex values on lhs and rhs, so not sure why this wasn't
    // working. In the future need to pull these out (mostly just because of
    // kludginess).
    if ((y1 == y2) && (x1 == x2)) {
        return q11;
    } else if (y1 == y2) {
