...
template<class U>
U isce3::core::Spline2dInterpolator<U>::interp_impl(
...
U isce3::core::Spline2dInterpolator<U>::_spline(double x,
        const std::valarray<U>& Y, int n, const std::valarray<U>& R) const
{
    ...
    if (x < 1.0) {
        ...
        return Y[n - 1] + (static_cast<U>(x - n) *
                                  (Y[n - 1] - Y[n - 2] + (R[n - 2] / denom)));
    } else {
        int j = int(std::floor(x));
        U xx = static_cast<U>(x - j);
        auto t0 = Y[j] - Y[j - 1] - (R[j - 1] / static_cast<U>(3.0)) -
                  (R[j] / denom);
        auto t1 = xx * ((R[j - 1] / static_cast<U>(2.0)) +
                               (xx * ((R[j] - R[j - 1]) / denom)));
...
template<typename U>
void isce3::core::Spline2dInterpolator<U>::_initSpline(
        const std::valarray<U>& Y, int n, std::valarray<U>& R,
        std::valarray<U>& Q) const
{
    ...
    for (int i = 1; i < n - 1; ++i) {
        ...
        R[i] = (static_cast<U>(3.0) * (Y[i + 1] - static_cast<U>(2.0) * Y[i] +
                                              Y[i - 1]) -
                       static_cast<U>(0.5) * R[i - 1]) *
               p;
    }
    R[n - 1] = U(0.0);
    for (int i = (n - 2); i > 0; --i)
