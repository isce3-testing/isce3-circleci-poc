...
/** Inline function for input checking on vector lengths (primarily to check to
 * see if 3D vector has the correct number of inputs, but is generalized to any
 * length). 'vec_name' is passed in by the wrapper macro (stringified vector
 * name), and 'parent_func' is passed in the same way through
 * __PRETTY_FUNCTION__ */
template<typename T>
inline void checkVecLenDebug(const std::vector<T>& vec, size_t len,
...
inline void check2dVecLenDebug(const std::vector<std::vector<T>>& vec,
        size_t len, size_t width, const char* vec_name, const char* parent_func)
{
    if ((vec.size() != len) && (vec[0].size() != width)) {
        std::string errstr = "In '" + std::string(parent_func) + "': Vector '" +
                             std::string(vec_name) + "' has size (" +
                             std::to_string(vec.size()) + " x " +
                             std::to_string(vec[0].size()) +
 ...
 * */
template<typename T>
inline bool compareFloatingPoint(T first, T second)
{
    const T scale =
            std::max({static_cast<T>(1.0), std::abs(first), std::abs(second)});
    return std::abs(first - second) <=
           std::numeric_limits<T>::epsilon() * scale;
}
 ...
 * @param[out] returns comparison result (true or false)*/
template<typename T>
inline bool compareComplex(std::complex<T> first, std::complex<T> second)
{
...
template<typename T>
inline std::valarray<bool> makeMask(
 ...
 * @param[out] returns a boolean mask */
template<typename T>
inline std::valarray<bool> makeMask(const std::valarray<T>& v, T noDataValue)
{
    std::valarray<bool> mask(v.size());
    for (size_t i = 0; i < v.size(); ++i) {
...
inline void insertionSort(std::valarray<double>& a, std::valarray<double>& b,
...
/** Clip a number between an upper and lower range (implements std::clamp for
 * older GCC) */
template<class T>
