...
public:
    ...
    Eigen::Matrix<T, Eigen::Dynamic, 1> eval(
            double y, const Eigen::Ref<const Eigen::VectorXd>& x) const;
...
private:
    ...
    // BVR: I'm placing the comparison operator implementations inline here
    // because it wasn't clear to me how to handle the template arguments
    // out-of-line
public:
    ...
    inline bool operator==(const isce3::core::LUT2d<T>& other) const
    {
        ...
        for (size_t i = 0; i < _data.length(); ++i) {
            for (size_t j = 0; j < _data.width(); ++j) {
                equal *= isce3::core::compareFloatingPoint(
                        _data(i, j), otherMat(i, j));
            }
        }
        return equal;
    }
    ...
    inline bool operator==(const isce3::core::LUT2d<T>& other) const
    {
        ...
        for (size_t i = 0; i < _data.length(); ++i) {
            for (size_t j = 0; j < _data.width(); ++j) {
                equal *= isce3::core::compareComplex(
                        _data(i, j), otherMat(i, j));
            }
        }
        return equal;
...
template<typename T>
isce3::core::LUT2d<T>::LUT2d()
    : _haveData(false), _boundsError(true), _refValue(0.0)
{
...
template<typename T>
isce3::core::LUT2d<T>::LUT2d(isce3::core::dataInterpMethod method)
    : _haveData(false), _boundsError(true), _refValue(0.0)
{
...
// Deep assignment operator
template<typename T>
isce3::core::LUT2d<T>& isce3::core::LUT2d<T>::operator=(const LUT2d<T>& lut)
{
