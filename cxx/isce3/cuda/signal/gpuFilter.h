...
public:
    ...
    // same name wrappers for filter init and construction functions with
    // HostToDevice cp
    /** constructs forward abd backward FFT plans for filtering a block of data
     * in range direction. */
    ...
    void constructRangeBandpassBoxcar(
            std::valarray<double> subBandCenterFrequencies,
            std::valarray<double> subBandBandwidths, double dt, int nfft,
    ...
    size_t rangeFrequencyShiftMaxIdx(
            thrust::complex<T>* spectrum, int n_rows, int n_cols);
...
template<class T>
__global__ void phaseShift_g(thrust::complex<T>* slc, T* range, double pxlSpace,
        T conj, double wavelength, T wave_div, int n_elements);

template<class T>
__global__ void sumSpectrum_g(
        thrust::complex<T>* spectrum, T* spectrum_sum, int n_rows, int n_cols);
