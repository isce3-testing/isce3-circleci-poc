 ...
 * The interpolation zone may also be shifted, if the max location is not at the
 * center.
 *
...
private:
    ...
    ///< the oversampling is only performed within \pm i_sincwindow*i_covs
    ///< around the peak
    ...
    static const int i_decfactor = 4096; ///< max decimals between original grid
                                         ///< to set up the sinc kernel
...
public:
    ...
    void execute(cuArrays<float>* imagesIn, cuArrays<float>* imagesOut,
            cuArrays<int2>* center, int oversamplingFactor);
    // destructor
