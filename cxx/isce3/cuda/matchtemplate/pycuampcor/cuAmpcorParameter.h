...
/// 2. Provide/set constant parameters, including numberWindows such as :
/// param->numberWindowDown = 100
/// 3. Call setupParameters() to determine related parameters and allocate
/// starting pixels for each window: param->setupParameters()
/// 4. Provide/set Reference window starting pixel(s), and gross offset(s):
/// param->setStartPixels(referenceStartDown, referenceStartAcross,
/// grossOffsetDown, grossOffsetAcross) 4a. Optionally, check the range of
/// windows is within the SLC image range: param->checkPixelInImageRange() Steps
/// 1, 3, 4 are mandatory. If step 2 is missing, default values will be used

class cuAmpcorParameter {
public:
    // TODO this is to avoid memory issues due to copying the non-owning
    // pointers we should use RAII ownership so these can be trivially defaulted
    ...
    int algorithm;    ///< Cross-correlation algorithm: 0=freq domain (default)
                      ///< 1=time domain
    int deviceID;     ///< Targeted GPU device ID: use -1 to auto select
    int nStreams;     ///< Number of streams to asynchonize data transfers and
                      ///< compute kernels
    ...
    int searchWindowSizeHeightRawZoomIn; ///< search window height used for zoom
                                         ///< in
    int searchWindowSizeWidthRawZoomIn;  ///< search window width used for zoom
                                         ///< in

    int corrStatWindowSize;  ///< correlation surface size used to estimate snr
    int corrRawZoomInHeight; ///< correlation surface height used for
                             ///< oversampling
    int corrRawZoomInWidth; ///< correlation surface width used for oversampling

    // chip or window size after oversampling
    int rawDataOversamplingFactor; ///< Raw data overampling factor (from
                                   ///< original size to oversampled size)
    ...
    int skipSampleDownRaw;   ///< Skip size between neighboring windows in Down
                             ///< direction (original size)
    int skipSampleAcrossRaw; ///< Skip size between neighboring windows in
                             ///< across direction (original size)

    // Zoom in region near location of max correlation
    int zoomWindowSize; ///< Zoom-in window size in correlation surface (same
                        ///< for down and across directions)
    int halfZoomWindowSizeRaw; ///<  half of
                               ///<  zoomWindowSize/rawDataOversamplingFactor

    int oversamplingFactor; ///< Oversampling factor for interpolating
                            ///< correlation surface
    int oversamplingMethod; ///< correlation surface oversampling method 0 = fft
                            ///< (default)  1 = sinc

    float thresholdSNR; ///< Threshold of Signal noise ratio to remove noisy
                        ///< data

    // reference image
    std::string referenceImageName; ///< reference SLC image name
    int imageDataType1; ///< reference image data type, 2=cfloat=complex=float2
                        ///< 1=float
    ...
    int imageDataType2; ///< secondary image data type, 2=cfloat=complex=float2
                        ///< 1=float
    ...
    int numberWindowDownInChunk;   ///< number of windows processed in a chunk
                                   ///< (down)
    int numberWindowAcrossInChunk; ///< number of windows processed in a chunk
                                   ///< (across)
    ...
    int mmapSizeInGB; ///< size for mmap buffer(useMmap=1) or a cpu memory
                      ///< buffer (useMmap=0)

    int referenceStartPixelDown0;   ///< first starting pixel in reference image
                                    ///< (down)
    int referenceStartPixelAcross0; ///< first starting pixel in reference image
                                    ///< (across)
    std::vector<int> referenceStartPixelDown; ///< reference starting pixels for
                                              ///< each window (down)
    std::vector<int> referenceStartPixelAcross; ///< reference starting pixels
                                                ///< for each window (across)
    std::vector<int> secondaryStartPixelDown; ///< secondary starting pixels for
                                              ///< each window (down)
    std::vector<int> secondaryStartPixelAcross; ///< secondary starting pixels
                                                ///< for each window (across)
    int grossOffsetDown0;   ///< gross offset static component (down)
    int grossOffsetAcross0; ///< gross offset static component (across)
    std::vector<int> grossOffsetDown;   ///< Gross offsets between reference and
                                        ///< secondary windows (down)
    std::vector<int> grossOffsetAcross; ///< Gross offsets between reference and
                                        ///< secondary windows (across)
    int mergeGrossOffset; ///< whether to merge gross offsets into the final
                          ///< offsets

    std::vector<int>
            referenceChunkStartPixelDown; ///< reference starting pixels for
                                          ///< each chunk (down)
    std::vector<int>
            referenceChunkStartPixelAcross; ///< reference starting pixels for
                                            ///< each chunk (across)
    std::vector<int>
            secondaryChunkStartPixelDown; ///< secondary starting pixels for
                                          ///< each chunk (down)
    std::vector<int>
            secondaryChunkStartPixelAcross; ///< secondary starting pixels for
                                            ///< each chunk (across)
    ...
    int maxReferenceChunkHeight,
            maxReferenceChunkWidth; ///< max reference chunk size
    int maxSecondaryChunkHeight,
            maxSecondaryChunkWidth; ///< max secondary chunk size
    ...
    // Three methods to set reference/secondary starting pixels and gross
    // offsets from input reference start pixel(s) and gross offset(s) 1 (int *,
    // int *, int *, int *): varying reference start pixels and gross offsets 2
    // (int, int, int *, int *): fixed reference start pixel (first window) and
    // varying gross offsets 3 (int, int, int, int): fixed reference start
    // pixel(first window) and fixed gross offsets
