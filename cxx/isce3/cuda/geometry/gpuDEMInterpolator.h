...
public:
    ...
    CUDA_HOSTDEV inline gpuDEMInterpolator()
        : _haveRaster(false), _refHeight(0.0),
          _interpMethod(isce3::core::BILINEAR_METHOD), _xstart(0.0),
          _ystart(0.0), _deltax(1.0), _deltay(1.0), _owner(false)
    {}
    ...
    CUDA_DEV inline double midX() const
    {
        return _xstart + 0.5 * _width * _deltax;
    }
    CUDA_DEV inline double midY() const
    {
    ...
    CUDA_HOSTDEV inline isce3::cuda::core::gpuInterpolator<float>**
    interp() const
    {
        return _interp;
    }
