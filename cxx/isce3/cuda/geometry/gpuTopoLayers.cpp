...
void gpuTopoLayers::copyToHost(isce3::geometry::TopoLayers& layers)
{
    checkCudaErrors(cudaMemcpy(
            &layers.x()[0], _x, _nbytes_double, cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaMemcpy(
            &layers.y()[0], _y, _nbytes_double, cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaMemcpy(
            &layers.z()[0], _z, _nbytes_double, cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaMemcpy(
            &layers.inc()[0], _inc, _nbytes_float, cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaMemcpy(
            &layers.hdg()[0], _hdg, _nbytes_float, cudaMemcpyDeviceToHost));
    ...
    checkCudaErrors(cudaMemcpy(&layers.localPsi()[0], _localPsi, _nbytes_float,
            cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaMemcpy(
            &layers.sim()[0], _sim, _nbytes_float, cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaMemcpy(&layers.crossTrack()[0], _crossTrack,
            _nbytes_double, cudaMemcpyDeviceToHost));
}
