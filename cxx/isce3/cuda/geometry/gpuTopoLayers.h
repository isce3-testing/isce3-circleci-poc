...
public:
    ...
    gpuTopoLayers(const gpuTopoLayers& layers)
        : _x(layers._x), _y(layers._y), _z(layers._z), _inc(layers._inc),
          _hdg(layers._hdg), _localInc(layers._localInc),
          _localPsi(layers._localPsi), _sim(layers._sim),
          _crossTrack(layers._crossTrack), _length(layers.length()),
          _width(layers.width()), _nbytes_double(layers.nbytes_double()),
    ...
    CUDA_DEV inline void x(size_t index, double value) { _x[index] = value; }

    CUDA_DEV inline void y(size_t index, double value) { _y[index] = value; }

    CUDA_DEV inline void z(size_t index, double value) { _z[index] = value; }

    CUDA_DEV inline void inc(size_t index, float value) { _inc[index] = value; }

    CUDA_DEV inline void hdg(size_t index, float value) { _hdg[index] = value; }
    ...
    CUDA_DEV inline void sim(size_t index, float value) { _sim[index] = value; }
    ...
    // copy construction on the device; still use the underbar convention to
    // discourage interfacing with pointers directly
    double* _x;
    double* _y;
    double* _z;
    float* _inc;
