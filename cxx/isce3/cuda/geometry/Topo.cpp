 ...
 * </ul>*/
void isce3::cuda::geometry::Topo::topo(
        Raster& demRaster, const std::string& outdir)
{
...
void isce3::cuda::geometry::Topo::topo(Raster& demRaster, Raster& xRaster,
        Raster& yRaster, Raster& heightRaster, Raster& incRaster,
        Raster& hdgRaster, Raster& localIncRaster, Raster& localPsiRaster,
 ...
 */
void isce3::cuda::geometry::Topo::topo(Raster& demRaster, TopoLayers& layers)
{
    ...
    // Cache ISCE objects (use public interface of parent isce3::geometry::Topo
    // class)
    ...
    for (size_t block = 0; block < nBlocks; ++block) {
        ...
        isce3::cuda::geometry::runGPUTopo(ellipsoid, orbit, doppler, demInterp,
                layers, lineStart, radarGrid.lookSide(), this->epsgOut(),
                radarGrid.sensingStart(), radarGrid.wavelength(),
                radarGrid.prf(), radarGrid.startingRange(),
                radarGrid.rangePixelSpacing(), this->threshold(),
                this->numiter(), this->extraiter(), totalconv);
...
void isce3::cuda::geometry::Topo::computeLinesPerBlock(
        isce3::io::Raster& demRaster, TopoLayers& layers)
{
    ...
    const size_t nDEMBytes =
            demRaster.width() * demRaster.length() * sizeof(float);
...
void isce3::cuda::geometry::Topo::_setLayoverShadowWithOrbit(const Orbit& orbit,
        TopoLayers& layers, DEMInterpolator& demInterp, size_t lineStart)
{
    ...
    for (size_t i = 0; i < layers.length(); ++i) {
        ...
        const double tline =
                this->radarGridParameters().sensingTime(i + lineStart);

        // Get state vector
        cartesian_t xyzsat;
        orbit.interpolate(
                &xyzsat, nullptr, tline, OrbitInterpBorderMode::FillNaN);
