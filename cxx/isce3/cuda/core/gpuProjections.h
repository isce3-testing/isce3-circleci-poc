/**
 * Source Author: Paulo Penteado, based on Projections.h by Piyush Agram /
 * Joshua Cohen Copyright 2018
 */
...
struct ProjectionBase {
    ...
    CUDA_HOSTDEV ProjectionBase(int code)
        : ellipse(6378137., .0066943799901), _epsgcode(code)
    {}
    CUDA_HOSTDEV virtual ~ProjectionBase() {}

    /** \brief Host function for transforming from LLH. This is similar to fwd
     * or fwd3d in PROJ.4
     *
    ...
    /** Host function for transforming to LLH. This is similar to inv or inv3d
     * in PROJ.4
     *
...
CUDA_DEV int projTransform(ProjectionBase* in, ProjectionBase* out,
        const double* inpts, double* outpts);
...
CUDA_DEV int projInverse(
        int code, const isce3::core::Vec3& in, isce3::core::Vec3& out_llh);
