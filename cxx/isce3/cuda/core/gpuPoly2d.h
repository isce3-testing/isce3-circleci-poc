...
struct gpuPoly2d {
    ...
    // Shallow-copy copy constructor only allowed on device, not host, but not
    // allowed to free own memory (host copy of gpuPoly2d is only one allowed)
    CUDA_DEV gpuPoly2d(
            int xo, int yo, double xm, double ym, double xn, double yn)
        : xOrder(xo), yOrder(yo), xMean(xm), yMean(ym), xNorm(xn), yNorm(yn),
          owner(false)
    {}
    CUDA_HOSTDEV gpuPoly2d() : gpuPoly2d(-1, -1, 0., 0., 1., 1.) {}
    // Advanced "copy constructor only allowed on host (manages deep copies from
    // host to device)
    CUDA_DEV gpuPoly2d(const gpuPoly2d& p)
        : xOrder(p.xOrder), yOrder(p.yOrder), xMean(p.xMean), yMean(p.yMean),
          xNorm(p.xNorm), yNorm(p.yNorm), coeffs(p.coeffs), owner(false)
    {}
    ...
    // Host function to test underlying device function in a single-threaded
    // context
