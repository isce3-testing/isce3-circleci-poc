...
void _migrate(const pixel_t* coarse, std::size_t pairId,
        std::size_t cellsPerPair, std::size_t cellsPerRefinedPair,
        std::size_t refCells, std::size_t refRefinedCells, std::size_t tdim,
        std::size_t edim, std::size_t trdim, const int* locations,
        pixel_t* refined);

// implementation. Moving reference and target tiles to new (enlarged)
// container for upcoming upsampling.
void ampcor::kernels::migrate(const std::complex<float>* coarse,
        std::size_t pairs, std::size_t refDim, std::size_t tgtDim,
        std::size_t expDim, std::size_t refRefinedDim,
        std::size_t tgtRefinedDim, const int* locations,
        std::complex<float>* refined)
{
    ...
    channel << pyre::journal::at(__HERE__)
            ...
            << "tiles to the refinement arena" << pyre::journal::endl;
...
#pragma omp parallel for
    for (std::size_t pairId = 0; pairId < pairs; pairId++)
        _migrate(reinterpret_cast<const std::complex<float>*>(coarse), pairId,
                cellsPerPair, cellsPerRefinedPair, refCells, refRefinedCells,
                tgtDim, expDim, tgtRefinedDim, locations,
        ...
        pixel_t* refined)
{
    ...
    for (std::size_t jdx = 0; jdx < edim; ++jdx) {
        ...
        src = coarse + pairId * cellsPerPair + refCells + (row + jdx) * tdim +
              col;
        // destination moves by a whole row in the refined target tile
        dest = refined + pairId * cellsPerRefinedPair + refRefinedCells +
               jdx * trdim;
        // migrate data
