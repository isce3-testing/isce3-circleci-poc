...
void _correlate(const value_t* arena, std::size_t pairId,
        const value_t* refStats, const value_t* tgtStats, std::size_t rdim,
        std::size_t rcells, std::size_t tdim, std::size_t tcells,
        std::size_t cdim, std::size_t ccells, value_t* correlation);

// implementation
void ampcor::kernels::correlate(const float* dArena, const float* refStats,
        ...
        std::size_t tgtDim, std::size_t corDim, float* dCorrelation)
{
    ...
    // show me
    channel << pyre::journal::at(__HERE__) << "launching multithreading on "
            << pairs << " pairs of tiles, with one thread "
            << "per pair " << pyre::journal::endl;

#pragma omp parallel for
    for (std::size_t pairId = 0; pairId < pairs; pairId++)
        _correlate(dArena, pairId, refStats, tgtStats, refDim, refCells, tgtDim,
                tgtCells, corDim, corCells, dCorrelation);
...
void _correlate(const value_t* arena, // the dataspace
