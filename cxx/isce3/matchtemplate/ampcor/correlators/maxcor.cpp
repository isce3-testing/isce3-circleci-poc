...
static void _maxcor(const value_t* gamma, std::size_t pairs,
        std::size_t corCells, std::size_t corDim, int* loc);

// run through the correlation matrix for each, find its maximum value and
// record its location
void ampcor::kernels::maxcor(const float* gamma, std::size_t pairs,
        std::size_t corCells, std::size_t corDim, int* loc)
{
    // make a channel
...
void _maxcor(const value_t* gamma,
        std::size_t pairId, // the pair index to process
        std::size_t
                corCells, // the number of cells in the correlation hyper-matrix
