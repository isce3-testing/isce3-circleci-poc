...
static void _offsetField(const int* fine, // the fine offsets
...
// record its location
void ampcor::kernels::offsetField(const int* fine, // the fine offsets
...
)
{
    ...
    // do the math
    myField[0] = (one * myField[0] - margin) +
                 (one * myFine[0] / zoom - refineMargin);
    myField[1] = (one * myField[1] - margin) +
                 (one * myFine[1] / zoom - refineMargin);
