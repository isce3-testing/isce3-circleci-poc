...
def cmdLineParse():
    ...
    # Required arguments
    parser.add_argument("master", type=str, help="Filename for master HDF5 product.")
    parser.add_argument(
        "slave", type=str, help="Filename for co-registered slave HDF5 product."
    )
    parser.add_argument("frequency", type=str, help="Frequency of SLC.")
    parser.add_argument("polarization", type=str, help="Polarization of SLC.")
...
def main(opts):
    ...
    igramDataset = driver.Create(
        opts.intPathAndPrefix, width, length, 1, gdal.GDT_CFloat32
    )
    igramRaster = Raster("", dataset=igramDataset)
    cohDataset = driver.Create(
        opts.cohPathAndPrefix, width, length, 1, gdal.GDT_Float32
    )
