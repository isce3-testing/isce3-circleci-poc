...
def cmdLineParse():
    ...
    # Required arguments
    parser.add_argument("product", type=str, help="Input master HDF5 product.")
    parser.add_argument("dem", type=str, help="Input DEM raster (GDAL compatible).")
    ...
    parser.add_argument(
        ...
        action="store",
        default="rdr2geo",
        help="Output directory. Default: topo.",
        dest="outdir",
    )
    parser.add_argument(
        "-f,--freq",
    ...
    parser.add_argument(
        "-mask", action="store_true", help="Generate layover/shadow mask."
    )
...
def main(opts):
    ...
    rdr2geo = isce3.geometry.rdr2geo(
        radarGrid=radarGrid, orbit=orbit, ellipsoid=ellipsoid, computeMask=opts.mask
    )
