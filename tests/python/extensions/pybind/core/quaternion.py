...
class SetUp:
    ...
    quat_ypr = [0.99996815847, 0.00131306830238, 0.00051330142620, -0.0078545784512]

    # 3-D Vectors , Rotation Matrix and Quaternion Vectors
    sc_pos = np.asarray(
        [-2434573.80388191110, -4820642.06528653484, 4646722.94036952127]
    )

    sc_vel = np.asarray([522.99592536068, 5107.80853161647, 5558.15620986960])
    ...
    quat_rotmat = [0.66878324, 0.22962791, 0.21909479, -0.67230757]

    quat_ant2ecf = np.asarray(
        [0.14889715185, 0.02930644114, -0.90605724862, -0.39500763650]
    )
...
class TestConstructors(SetUp):
    def test_default(self):
        ...
        npt.assert_allclose(q(), [1, 0, 0, 0], rtol=self.rtol, atol=self.atol)

    def test_vector4(self):
        q = isce3.core.Quaternion(2.0 * self.quat_ant2ecf)
        npt.assert_allclose(q(), self.quat_ant2ecf, rtol=self.rtol, atol=self.atol)
    ...
    def test_rotmat(self):
        ...
        npt.assert_allclose(q(), self.quat_rotmat, rtol=self.rtol, atol=self.atol)

    def test_ypr(self):
        q = isce3.core.Quaternion(*self.ypr_ang)
        npt.assert_allclose(q(), self.quat_ypr, rtol=self.rtol, atol=self.atol)
...
class TestMethods(SetUp):
    def test_to_ypr(self):
        q = isce3.core.Quaternion(*self.ypr_ang)
        npt.assert_allclose(q.to_ypr(), self.ypr_ang, rtol=self.rtol, atol=self.atol)

    def test_to_euler_angles(self):
        q = isce3.core.Quaternion(*self.ypr_ang)
        elr = q.to_euler_angles()
        npt.assert_allclose(
            [elr.yaw, elr.pitch, elr.roll], self.ypr_ang, rtol=self.rtol, atol=self.atol
        )
    ...
    def test_conjugate(self):
        ...
        quat_conj[1:] *= -1
        npt.assert_allclose(q.conjugate()(), quat_conj, rtol=self.rtol, atol=self.atol)
