...
def test_run_filter2D():
    ...
    # Filter data
    filt_data_real = isce3.io.Raster(
        "data_real.filt", width, length, 1, gdal.GDT_Float64, "ENVI"
    )
    filt_data_cpx = isce3.io.Raster(
        "data_cpx.filt", width, length, 1, gdal.GDT_CFloat64, "ENVI"
    )

    data_raster_real = isce3.io.Raster("data.real")
    data_raster_cpx = isce3.io.Raster("data.cpx")

    isce3.signal.filter2D(filt_data_real, data_raster_real, kernel1d, kernel1d, block)
    isce3.signal.filter2D(filt_data_cpx, data_raster_cpx, kernel1d, kernel1d, block)
