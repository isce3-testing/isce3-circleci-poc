...
from pybind_isce3.core import Linspace, Poly1d

# functions for conversion from (rad) to (mdeg) and vice versa


def r2md(ang_rad):
    return 1000 * np.rad2deg(ang_rad)


def md2r(ang_mdeg):
    return 1e-3 * np.deg2rad(ang_mdeg)
...
class TestEdgeMethodCostFunc:
    ...
    lka_deg = [32.0, 32.5, 33.0, 33.5, 34.1, 34.5, 35.0, 35.5, 36.0]

    # Calculated parameters from input parameters:
    num_lka_edge = int((max_lka_edge_deg - min_lka_edge_deg) / prec_lka_edge_deg) + 1
    ...
    # and echo objects
    lka_edge_rad = np.linspace(
        np.deg2rad(min_lka_edge_deg), np.deg2rad(max_lka_edge_deg), num_lka_edge
    )
    ...
    def _validate_estimation(
        self, est: tuple, roll_true_mdeg: float, err_msg: str = ""
    ):
        ...
        # validate
        npt.assert_(
            n_iter <= self.max_iter, msg="Exceed max number of iteration " + err_msg1
        )
        npt.assert_(flag, msg="Wrong convergence flag " + err_msg1)
        npt.assert_allclose(
            abs_err,
            0.0,
            atol=self.max_abs_err_mdeg,
            err_msg=("Too large residual roll offset " + err_msg1),
        )
        npt.assert_allclose(
            f_val,
            0.0,
    ...
    def test_roll_angle_offset_from_edge_scalars(self):
        ...
        for pf_ant, roll_true_mdeg in zip(self.pf_ant_vec, self.roll_ofs_ant_mdeg):
            ...
            self._validate_estimation(
                est_tuple, roll_true_mdeg, err_msg="w/o weighting"
            )
            ...
            # validate
            self._validate_estimation(
                est_wgt_tuple, roll_true_mdeg, err_msg="w/ weighting"
            )
    ...
    def test_roll_angle_offset_from_edge_linspace(self):
        ...
        for pf_ant, roll_true_mdeg in zip(self.pf_ant_vec, self.roll_ofs_ant_mdeg):
            # estimate roll angle offset w/o weighting
            est_tuple = ant.roll_angle_offset_from_edge(self.pf_echo, pf_ant, lka_lsp)
            # validate
            self._validate_estimation(
                est_tuple, roll_true_mdeg, err_msg="w/o weighting"
            )
