...
class TestTransformations:
    ...
    _est_xyz[0, :] = [-0.121864326797, -0.015668270588, 0.992423090799]

    _est_xyz[1, :] = [-0.034899213187, -0.006979842637, 0.999366462673]

    _est_xyz[2, :] = [0.000000000000, 0.000000000000, 1.000000000000]

    _est_xyz[3, :] = [0.017452264667, 0.006980905867, 0.999823327099]

    _est_xyz[4, :] = [0.069753604227, 0.015694560951, 0.997440782931]
    ...
    def test_scalars(self):
        ...
        for idx, (el, az, est_xyz) in enumerate(
            ...
            npt.assert_allclose(
                ...
                rtol=self._rtol,
                atol=self._atol,
                err_msg=err_msg_f.format(self._el_deg[idx], self._az_deg[idx]),
            )
            ...
            npt.assert_allclose(
                ...
                rtol=self._rtol,
                atol=self._atol,
                err_msg=err_msg_b.format(self._el_deg[idx], self._az_deg[idx]),
            )
    ...
    def test_vectorscalar(self):
        ...
        for idx, az in enumerate(self._az_rad):
            ...
            v_elaz = self._frame.cart2sph(self._frame.sph2cart(self._el_rad, az))
            ...
            npt.assert_allclose(
                ...
                atol=self._atol,
                rtol=self._rtol,
                err_msg="EL-cut failed @ {:.3f} (deg)".format(self._az_deg[idx]),
            )

        # AZ cut
        for idx, el in enumerate(self._el_rad):
            # Forward-Backward transformation
            v_elaz = self._frame.cart2sph(self._frame.sph2cart(el, self._az_rad))
            ...
            npt.assert_allclose(
                ...
                atol=self._atol,
                rtol=self._rtol,
                err_msg="AZ-cut failed @ {:.3f} (deg)".format(self._el_deg[idx]),
            )
