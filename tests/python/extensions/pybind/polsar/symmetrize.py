...
def test_run():
    ...
    hv_raster = _create_raster(hv_file, hv_array, width, length, nbands, dtype, "ENVI")
    vh_raster = _create_raster(vh_file, vh_array, width, length, nbands, dtype, "ENVI")
    ...
    for memory_mode in memory_mode_list:
        ...
        output_raster = isce3.io.Raster(
            output_file, width, length, nbands, dtype, "ENVI"
        )
        ...
        # compute max error
        symmetrization_max_error = np.max(
            np.abs(output_array[i, j] - (hv_array[i, j] + vh_array[i, j]) / 2.0)
        )
