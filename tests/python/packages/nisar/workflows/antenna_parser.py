...
class TestAntennaParser:
    ...
    def test_num_beams(self):
        npt.assert_equal(
            self.prs.num_beams("H"), 5, err_msg="Wrong number of beams for 'H' Pol"
        )
        npt.assert_equal(
            self.prs.num_beams("V"), 5, err_msg="Wrong number of beams for 'V' Pol"
        )
    ...
    def test_el_cut(self):
        ...
        npt.assert_allclose(
            np.rad2deg([elcut.angle[0], elcut.angle[-1]]),
            [-9.2, -1.6],
            atol=self.atol,
            rtol=self.rtol,
    ...
    def test_az_cut(self):
        ...
        npt.assert_allclose(
            np.rad2deg([azcut.angle[0], azcut.angle[-1]]),
            [-9.937, 9.712],
            atol=self.atol,
            rtol=self.rtol,
            err_msg="Wrong 'angle' values",
    ...
    def test_el_cut_all(self):
        ...
        num_ang = (
            int(
                np.ceil(
                    (ang_last - ang_first) / (beam_first.angle[1] - beam_first.angle[0])
                )
            )
            + 1
        )
        ...
        npt.assert_allclose(
            elcut.angle, angles, atol=self.atol, rtol=self.rtol, err_msg="Wrong 'angle'"
        )
        ...
        npt.assert_allclose(
            ...
            atol=0.01,
            rtol=0.1,
            err_msg="Wrong mean 'cut_ang'",
        )

        pattern = np.interp(
            angles, beam_first.angle, beam_first.copol_pattern, left=0.0, right=0.0
        )
        npt.assert_allclose(
            elcut.copol_pattern[0, :],
            pattern,
            atol=self.atol,
            rtol=self.rtol,
        ...
        pattern = np.interp(
            angles, beam_last.angle, beam_last.copol_pattern, left=0.0, right=0.0
        )
        npt.assert_allclose(
            elcut.copol_pattern[-1, :],
            pattern,
            atol=self.atol,
            rtol=self.rtol,
