...
TYPED_TEST(IH5Test, nochunk)
{
    ...
    // Create a HDF5 file
    std::string wfilename = "castread_" +
    ...
    isce3::io::IDataSet dset =
            grp.createDataSet(std::string("data"), _inmatrix.data(), shp);
    {
        ...
        int hsum;
        {
            ...
            hsum = GDALChecksumImage(readRaster.dataset()->GetRasterBand(1), 0,
                    0, width, length);
        }
...
TYPED_TEST(IH5Test, chunk)
{
    ...
    std::array<int, 2> shp = {length, width};
    isce3::io::IDataSet dset =
            grp.createDataSet<FirstParam>(std::string("data"), shp, 1);
