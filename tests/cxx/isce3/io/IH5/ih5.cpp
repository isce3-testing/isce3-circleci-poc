...
TEST_F(IH5Test, findRegularExpression)
{
    ...
    std::string searchedName(
            ".*calibrationInformation.*(HH$|HV$|RH$|RV$|VH$|VV$)");
...
TEST_F(IH5Test, readVariableLengthString2) {
    ...
    ASSERT_EQ(strVal, "Complex backscatter for primary mode (HH pol). Focused
SLC image. All channels are registered");
}
...
TEST_F(IH5Test, readVariableLengthString2)
{
    ...
    ASSERT_EQ(strVal, "POLYGON ((-115.507 34.822, -115.634 34.845, -115.639 "
                      "34.827, -115.512 34.805, -115.507 34.822))");
}
...
TEST_F(IH5Test, datasetReadComplexWithRawPointer)
{
    ...
    std::complex<float>* dval =
            new (std::nothrow) std::complex<float>[500 * 500];
    dset.read(dval);
...
TEST_F(IH5Test, datasetReadFloatWithVector)
{
    ...
    std::string datasetName(
            "/science/LSAR/SLC/metadata/attitude/angularVelocity");
    isce3::io::IDataSet dset = file.openDataSet(datasetName);
...
TEST_F(IH5Test, createGroups)
{
    ...
    EXPECT_NO_THROW(isce3::io::IGroup grp =
                            fic.createGroup(std::string("groupVector")));
    EXPECT_NO_THROW(isce3::io::IGroup grp =
                            fic.createGroup(std::string("groupValarray")));
    ...
    EXPECT_NO_THROW(isce3::io::IGroup grp = fic.createGroup(
                            std::string("groupX/groupXX/groupXXX")));
...
TEST_F(IH5Test, createSimpleDatasetFromVectorBuffer)
{
    ...
    // Create simple dataset in groupVector, stored as 1D array from a
    // std::vector buffer
    ...
    // Create simple string dataset in groupVector, stored as 1D array from a
    // std::vector buffer
    std::vector<std::string> v3(10 * 10);
    for (int i = 0; i < 100; i++)
        v3[i] = std::to_string(i + 1);
...
TEST_F(IH5Test, createSimpleDatasetFromValarrayBuffer)
{
    ...
    // Create simple dataset in groupValarray, stored as 1D array from a
    // std::valarray buffer
    ...
    // Create simple string dataset in groupValarray, stored as 1D array from a
    // std::valarray buffer
...
TEST_F(IH5Test, createSimpleDatasetFromRawPointer)
{
    ...
    // Create simple dataset in groupRawPointer, stored as 1D array from a
    // std::valarray buffer
    ...
    // Create simple string dataset, stored as 1D array from a raw pointer
    // buffer
...
TEST_F(IH5Test, createGroupAttributes)
{
    ...
    for (int i = 0; i < 10; i++)
        attv2.push_back(std::to_string(i));
...
TEST_F(IH5Test, createFloat16Dataset)
{
    ...
    dset = grp.createDataSet<isce3::io::float16>(
            std::string("vFloat16_nbit"), dims, 1);
    dset.write(v1);
    ...
    dset = grp.createDataSet<isce3::io::float16>(
            std::string("vFloat16_nbit_deflate"), dims, 1, 0, 9);
    dset.write(v1);
