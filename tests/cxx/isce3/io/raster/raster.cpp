...
TEST_F(RasterTest, createGeoTiffFloat)
{
    ...
    isce3::io::Raster lat =
            isce3::io::Raster(latFilename, nc, nl, 1, GDT_Float32, "GTiff");
...
TEST_F(RasterTest, createVRTDouble_setGetValue)
{
    ...
    isce3::io::Raster lon =
            isce3::io::Raster(lonFilename, nc, nl, 1, GDT_Float64, "VRT");
    float a;
    ...
    for (uint y = 0; y < lon.length(); ++y) {    // for each line
        for (uint x = 0; x < lon.width(); ++x) { // for each column
            lon.setValue(
                    y, x, y, 1); // lon stores the line number in each pixel
...
TEST_F(RasterTest, openVRTRasterReadOnlyMode_getValue)
{
    ...
    // ASSERT_EQ (lon.access(), GA_ReadOnly);  // files are opened in readonly
    // mode by default
...
TEST_F(RasterTest, updateGeoTiff_getLineValarray)
{
    ...
    for (uint y = 0; y < lat.length(); ++y) { // for each line
        ...
        for (uint x = 0; x < lat.width(); ++x) // for each column
            ASSERT_EQ(lineOut[x],
                    (double) x); // lat pixels store the column index
    }
}
...
TEST_F(RasterTest, createTwoBandsENVIRaster)
{
    ...
    isce3::io::Raster inc =
            isce3::io::Raster(incFilename, nc, nl, 2, GDT_Int16, "ENVI");

    ASSERT_EQ(exists(incFilename), true);
}

// Populate first band of ENVI Raster with setBlock (blocks can't overflow
// image)
TEST_F(RasterTest, setBlockBandOneENVIRaster)
{
    ...
    float a;
    uint nXBlocks = floor(
            (float) inc.width() / (float) nbx); // number of blocks along X
    uint nYBlocks = floor(
            (float) inc.length() / (float) nby); // number of blocks along Y

    for (uint y = 0; y < nYBlocks; ++y) {
        for (uint x = 0; x < nXBlocks; ++x) {
            block = x * y; // pick a value
            inc.setBlock(block, x * nbx, y * nby, nbx, nby,
                    1); // block must be within band
            inc.getValue(a, x * nbx + ceil(nbx / 2.), y * nby + ceil(nby / 2.),
                    1);          // get block center value
            ASSERT_EQ(a, x * y); // values must be equal
        }
    }
}

// Populate second band of ENVI Raster with setBlock (blocks can't overflow
// image)
...
TEST_F(RasterTest, checkGetLineVRT)
{
    ...
    for (size_t i = 0; i < vrt.length(); ++i) {
        ...
        ASSERT_EQ(line.sum(),
                i * vrt.width()); // lon has row-number in each pixel
    }
}
...
TEST_F(RasterTest, addRasterToVRT)
{
    ...
    ASSERT_EQ(
            vrt.numBands(), refNumBands); // must be five due to previous tests
