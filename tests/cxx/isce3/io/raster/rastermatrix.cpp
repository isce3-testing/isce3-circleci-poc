...
public:
    ...
    void initTestRaster(const std::string& filename)
    {
        ...
        isce3::io::Raster inc =
                isce3::io::Raster(filename, nc, nl, 1, GDT_Float32, "GTiff");
        uint nXBlocks =
                floor((float) inc.width() / (float) nbx); // number of blocks
        uint nYBlocks =
                floor((float) inc.length() / (float) nby); // number of blocks
        ...
        for (uint y = 0; y < nYBlocks; ++y) {
            for (uint x = 0; x < nXBlocks; ++x) {
                ...
                inc.setBlock(
                        mat, x * nbx, y * nby, 1); // block must be within band
            }
        }
    }
};
...
TEST_F(RasterTest, setGetBlockEMatrix2D)
{
    ...
    inc.getBlock(blockmat, 0, 0, 1);     // read upper-left sub-block
    inc.getBlock(chunkmat, nbx, nbx, 1); // read chunk from band 1
    ASSERT_EQ(blockmat.sum(),
            block_mat_sum); // block sum must be equal to nbx*nby
