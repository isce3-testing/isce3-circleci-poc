...
TEST(RadarGridTest, fromSwath)
{
    ...
    ASSERT_NEAR(grid.wavelength(), isce3::core::speed_of_light / 5.331004416e9,
            1.0e-5);
    ASSERT_NEAR(grid.rangePixelSpacing(), 7.803973670948287, 1.0e-7);
...
TEST(RadarGridTest, fromParametersSingeLook)
{
    ...
    isce3::product::RadarGridParameters grid(
            10.0, 0.06, 1729.0, 800000., 10.0, LookSide::Left, 24000, 6400, t0);
...
TEST(RadarGridTest, fromParametersMultiLook)
{
    ...
    // Create radar grid from product
    isce3::product::RadarGridParameters grid(
            10.0, 0.06, 1729.0, 800000., 10.0, LookSide::Left, 8000, 1600, t0);
...
TEST(RadarGridTest, fromParametersUpsample)
{
    ...
    ASSERT_NEAR(
            (upsampled_grid.refEpoch() - t0).getTotalSeconds(), 0.0, 1.0e-10);
...
TEST(RadarGridTest, fromParametersCrop)
{
    ...
    // Crop
    isce3::product::RadarGridParameters mlgrid =
            grid.offsetAndResize(400, 500, 2000, 800);
    ...
    ASSERT_NEAR((mlgrid.refEpoch() - grid.refEpoch()).getTotalSeconds(), 0.0,
            1.0e-10);
...
TEST(RadarGridTest, cropSame)
{
    ...
    // Crop
    isce3::product::RadarGridParameters mlgrid =
            grid.offsetAndResize(0, 0, grid.length(), grid.width());
