...
TEST_F(GeometryTest, RdrToGeoLat)
{
    ...
    for (size_t ii = 0; ii < 20; ++ii) {
        ...
        for (int kk = 0; kk < 2; kk++) {
            ...
            isce3::core::cartesian_t xyz = {ellipsoid.a() *
                                                    std::cos(geocentricLat) *
                                                    std::cos(expectedLon),
                    ellipsoid.a() * std::cos(geocentricLat) *
                            std::sin(expectedLon),
            ...
            // Run rdr2geo
            int stat = isce3::geometry::rdr2geo(tinp, rng, 0.0, orbit,
                    ellipsoid, dem, targetLLH, 0.24, sides[kk], 1.0e-8, 25, 15);
...
TEST_F(GeometryTest, GeoToRdrLat)
{
    ...
    for (size_t ii = 1; ii < 20; ++ii) {
        ...
        for (int kk = 0; kk < 2; kk++) {
            ...
            isce3::core::cartesian_t sat_xyz = {(ellipsoid.a() + hsat) *
                                                        std::cos(expectedLon) *
                                                        std::cos(lat0),
...
derived for a satellite flying at constant velocity and radius over a line of
constant latitude.

The target is assumed to lie on a sphere with radius equal to Ellipsoid's major
axis.
...
A point $\vec{S}$ on a sphere at a height $h$ above the major axis characterized
by Geocentric Latitude ($\lambda$), Longitude ($\psi$) can be expressed in ECEF
coordinates as follows:
...
Let the location of target ($\vec{T}$) be represented by geocentric latitude
...
$$ \left( \left( a + h_s \right) \cdot \cos \lambda_s  -   a \cdot \cos \lambda
\right)^2$$
$$ + \left( \left( a +h_s \right) \cdot \sin \lambda_s - a \cdot \sin \lambda
\right)^2 = R^2$$

Leading to

$$\cos \left( \lambda - \lambda_s \right) = \frac{1}{2} \cdot \left[
\frac{a+h_s}{a} + \frac{a}{a+h_s} - \frac{R}{a} \cdot \frac{R}{a+h_s} \right] $$
