...
struct PerimeterTest :
    public ::testing::TestWithParam<std::tuple<LookSide, int, int>> {
    ...
    // Constructor
    PerimeterTest()
        : t0("2017-02-12T01:12:30.0"), grid(15.0, 0.06, 1000., 800000., 1000.,
                                               LookSide::Right, 10000, 80, t0)
    {}
...
TEST_P(PerimeterTest, Normal)
{
    ...
    isce3::geometry::Perimeter perimeter =
            isce3::geometry::getGeoPerimeter(grid, orbit, proj);
    ...
    for (int ii = 0; ii < nPtsPerEdge; ii++) {
        ranges.push_back(grid.slantRange(
                (ii / (nPtsPerEdge - 1.0)) * (grid.width() - 1)));
        times.push_back(grid.sensingTime(
                (ii / (nPtsPerEdge - 1.0)) * (grid.length() - 1)));
    }
...
TEST_P(PerimeterTest, DateLine)
{
    ...
    // Compute perimeter
    isce3::geometry::BoundingBox box =
            isce3::geometry::getGeoBoundingBox(grid, orbit, proj);
...
INSTANTIATE_TEST_SUITE_P(PerimeterTests, PerimeterTest,
        testing::Values(std::make_tuple(LookSide::Right, 1, 1),
