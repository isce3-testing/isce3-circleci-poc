...
double error_bound(long m, long n, long fft_size)
{
    double x = sqrt(1.0 - n * 1.0 / fft_size);
    return 4 * M_PI * (sqrt(m) + m) * sqrt(x) * std::exp(-2 * M_PI * m * x);
}

void compare_output(const isce3::signal::NFFT<double>& p,
...
void test_nfft(size_t m, size_t nf, size_t fft_size)
{
    // Generate test data.
    const size_t nt = 256;
...
void test_adjoint_nfft(size_t m, size_t nf, size_t fft_size)
{
    // Generate test data.
    ...
    for (size_t i = 0; i < nf; ++i) {
        ...
        if (_DBG_NFFT)
            printf("data %25.16e %25.16e %25.16e\n", times[i], std::real(xt[i]),
                    std::imag(xt[i]));
    }
...
{
    ASSERT_THROW(
            isce3::signal::NFFT<double>(1, 9, 32), isce3::except::LengthError);
}
