...
TEST(Filter, constructAzimuthCommonbandFilter)
{
    ...
    // Get the Doppler polynomial and use it for both refernce and secondary
    // SLCs
    auto dop1 = isce3::core::avgLUT2dToLUT1d<double>(
    ...
    // beta parameter for the raised cosine filter used for constructing the
    // common azimuth band filter
    double beta = 0.25;
    ...
    filter.constructAzimuthCommonbandFilter(dop1, dop2, commonAzimuthBandwidth,
            prf, beta, refSlc, refSpectrum, ncols, blockRows);
...
TEST(Filter, constructBoxcarRangeBandpassFilter)
{
    ...
    filter.constructRangeBandpassFilter(rangeSamplingFrequency,
            subBandCenterFrequencies, subBandBandwidths, refSlc, refSpectrum,
            ncols, blockRows, filterType);
