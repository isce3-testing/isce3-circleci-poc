...
int main()
{
    ...
    // verify: go through all the tables and verify that they contain the
    // correct target means we need the detected tiles, so make room for the
    // results
    ...
    cudaError_t status = cudaMemcpy(
            ampResults, rArena, ampFootprint, cudaMemcpyDeviceToHost);
    // stop the clock
    timer.stop();
    ...
    if (status != cudaSuccess) {
        ...
        error << pyre::journal::at(__HERE__)
              << "while retrieving the amplitudes of the tiles: " << description
    ...
    // show me
    tlog << pyre::journal::at(__HERE__)
         << "moving the detected tiles to the host: " << 1e3 * ampDuration
         << " ms"
         << ", at " << ampRate << " Gb/s" << pyre::journal::endl;
    ...
    if (status != cudaSuccess) {
        ...
        // complain
        error << pyre::journal::at(__HERE__)
              << "while retrieving SATs: " << description << " (" << status
              << ")" << pyre::journal::endl;
        // and bail
        throw std::runtime_error(description);
    }
    ...
    // the total number of cells in the stats array: 2 floats per placement per
    // pair
    ...
    // copy the results over
    status = cudaMemcpy(
            statResults, tgtStats, statFootprint, cudaMemcpyDeviceToHost);
    // stop the clock
    timer.stop();
    // if something went wrong
    if (status != cudaSuccess) {
        ...
        // complain
        error << pyre::journal::at(__HERE__)
              << "while retrieving the hyper-grid of average amplitudes for "
                 "the target tiles: "
    ...
    // show me
    tlog << pyre::journal::at(__HERE__)
         << "moving the hyper-grid of averages to the host: "
         << 1e3 * statDuration << " ms"
         << ", at " << statRate << " Gb/s" << pyre::journal::endl;
