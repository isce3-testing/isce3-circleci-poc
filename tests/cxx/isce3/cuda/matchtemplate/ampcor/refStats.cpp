...
int main()
{
    ...
    channel << pyre::journal::at(__HERE__)
            << "test: adjusting the amplitudes of the reference tile to zero "
               "mean"
            << pyre::journal::endl;
    ...
    // show me
    tlog << pyre::journal::at(__HERE__)
         << "computing the statistics of the reference tiles: "
    ...
    cudaError_t status =
            cudaMemcpy(tiles, rArena, rFootprint, cudaMemcpyDeviceToHost);
    // stop the clock
    timer.stop();
    ...
    if (status != cudaSuccess) {
        ...
        error << pyre::journal::at(__HERE__)
              << "while retrieving the detected tiles: " << description << " ("
    ...
    // show me
    tlog << pyre::journal::at(__HERE__)
         << "moving the detected tiles to the host: " << 1e3 * rDuration
         << " ms"
         << ", at " << rRate << " Gb/s" << pyre::journal::endl;
    ...
    // show me
    tlog << pyre::journal::at(__HERE__)
         << "moving reference tile statistics to the host: " << 1e3 * sDuration
         << " ms"
         << ", at " << sRate << " Gb/s" << pyre::journal::endl;
    ...
    for (auto pid = 0; pid < pairs; ++pid) {
        ...
        if (std::abs(mean) > tolerance) {
            ...
            pyre::journal::error_t error("ampcor.cuda");
            // complain
            error << pyre::journal::at(__HERE__) << "mean mismatch at tile "
                  << pid << ": " << mean << " != 0" << pyre::journal::endl;
            // bail
        ...
        // show me
        channel << pyre::journal::at(__HERE__) << "tile " << pid << ": "
        ...
        if (mismatchVar > 10 * tolerance) {
            ...
            pyre::journal::error_t error("ampcor.cuda");
            // complain
            error << pyre::journal::at(__HERE__) << "variance mismatch at tile "
                  << pid << ": "
                  << "computed: " << computedVar
                  << ", expected: " << expectedVar << pyre::journal::endl;
            // bail
