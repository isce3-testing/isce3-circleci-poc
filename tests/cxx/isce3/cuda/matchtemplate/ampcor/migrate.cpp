...
int main()
{
    ...
    status = cudaMemcpy(
            refined, refinedArena, footprint, cudaMemcpyDeviceToHost);
    // if something went wrong
    if (status != cudaSuccess) {
        // get the error description
        std::string description = cudaGetErrorName(status);
        ...
        // complain
        error << pyre::journal::at(__HERE__)
              << "while harvesting the refined tiles: " << description << " ("
    ...
    timer.stop();
    // show me
    tlog << pyre::journal::at(__HERE__) << "harvesting the refined tiles "
         << 1e3 * timer.read() << " ms" << pyre::journal::endl;
    ...
    if (status != cudaSuccess) {
        ...
        // complain
        error << pyre::journal::at(__HERE__)
              << "while harvesting the nudged locations: " << description
              << " (" << status << ")" << pyre::journal::endl;
        // bail
        throw std::runtime_error(description);
    }
    ...
    timer.stop();
    // show me
    tlog << pyre::journal::at(__HERE__) << "harvesting the nudged locations "
         << 1e3 * timer.read() << " ms" << pyre::journal::endl;
    ...
    for (auto pid = 0; pid < pairs; ++pid) {
        ...
        // show me the tile
        channel << pyre::journal::at(__HERE__) << "pair (" << row << "," << col
        ...
        for (auto idx : tgtRefined.layout()) {
            ...
            if (idx[0] >= expDim || idx[1] >= expDim) {
                ...
                if (std::abs(tgtRefined[idx]) > tolerance) {
                    ...
                    pyre::journal::error_t error("ampcor.cuda");
                    // complain
                    error << pyre::journal::at(__HERE__) << "pair " << pid
                          << ": mismatch at (" << idx << "): "
                ...
                if (actual != expected) {
                    ...
                    pyre::journal::error_t error("ampcor.cuda");
                    // complain
                    error << pyre::journal::at(__HERE__) << "pair ("
                          << pid / placements << "," << pid % placements
                          << ")"
                             ": mismatch at ("
                          << idx << "): "
                          << "expected: " << expected << ", got: " << actual
                          << " from tgt[" << base + idx << "]"
                          << ", expDim: " << expDim << pyre::journal::endl;
                    // and bail
                    throw std::runtime_error("verification error!");
                }
            }
