...
int main()
{
    ...
    channel << pyre::journal::at(__HERE__)
            << "test: refining the coarse dataset" << pyre::journal::endl;
    ...
    auto status = cudaMemcpy(
            refined, refinedArena, refinedFootprint, cudaMemcpyDeviceToHost);
    // stop the clock
    timer.stop();
    ...
    if (status != cudaSuccess) {
        ...
        error << pyre::journal::at(__HERE__)
              << "while retrieving the refined tiles: " << description << " ("
    ...
    // verify that all the cells that would have been occupied by the target
    // tiles are still zero
        ...
        for (auto idx : tgt.layout()) {
            ...
            if (std::abs(cell) > tolerance) {
                ...
                error << pyre::journal::at(__HERE__)
                      ...
                      << pyre::journal::newline << "at [" << pid << ";" << idx
                      << "]" << pyre::journal::endl;
                // and bail
    ...
    // show me
    tlog << pyre::journal::at(__HERE__)
         << "verifying that the refined target slots were untouched: "
         << 1e3 * duration << " ms" << pyre::journal::endl;
    ...
    // show me
    tlog << pyre::journal::at(__HERE__)
         << "reallocating the refined tile arena: " << 1e3 * timer.read()
    ...
    // show me
    tlog << pyre::journal::at(__HERE__)
         << "synthesizing locations for the refined target tiles: "
    ...
    if (status != cudaSuccess) {
        ...
        // complain
        error << pyre::journal::at(__HERE__)
              << "while allocating memory for the locations of the correlation "
                 "maxima: "
    ...
    if (status != cudaSuccess) {
        ...
        // complain
        error << pyre::journal::at(__HERE__)
              << "while moving the locations of the correlation maxima to the "
                 "device: "
    ...
    // show me
    tlog << pyre::journal::at(__HERE__)
         << "moving maxima locations to the device: " << 1e3 * timer.read()
         << " ms" << pyre::journal::endl;
    ...
    // show me
    tlog << pyre::journal::at(__HERE__)
         << "moving the second set of refined tiles to the host: "
         << 1e3 * duration << " ms"
         << ", at " << rate << " Gb/s" << pyre::journal::endl;

    // start the clock
    timer.reset().start();
    // verify that all the cells that would have been occupied by the reference
    // tiles are still zero
    for (auto pid = 0; pid < pairs; ++pid) {
        // find the starting point of this target tile
