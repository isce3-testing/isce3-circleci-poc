...
int main()
{
    ...
    cudaError_t status =
            cudaMemcpy(results, rArena, rFootprint, cudaMemcpyDeviceToHost);
    // stop the clock
    timer.stop();
    ...
    for (auto cell = 0; cell < cells; ++cell) {
        if (std::abs(arena[cell]) != results[cell]) {
            ...
            pyre::journal::error_t error("ampcor.cuda");
            // complain
            error << pyre::journal::at(__HERE__) << "mismatch at cell " << cell
                  << ": "
        ...
        for (auto pid = 0; pid < pairs; ++pid) {
            ...
            for (auto idx = 0; idx < tgtDim; ++idx) {
                for (auto jdx = 0; jdx < tgtDim; ++jdx) {
                    channel << results[pid * cellsPerPair + refDim * refDim +
                                       idx * tgtDim + jdx]
                            << " ";
                }
                channel << pyre::journal::newline;
